...
| 卡片圆角 | `--r-4`(20px) | **新的 `--r-6`(42px)+ 超椭圆**(见下面那条,改了四轮) |
...
⚠️⚠️ **记忆卡片的圆角最后定在「第六档 42px + 超椭圆」**(2026-09-03,**改了四轮才对**)。
...
- **`--r-6: 42px` 是 2026-09-03 给尺度表新加的第六档**,⚠️ **不是散值**,五档的规矩没破(仍一档比一档大);
  ⚠️ **目前只有记忆卡片用它**,别顺手拿去套抽屉/登录框(那些仍是 `--r-5`)。
- ⚠️ **换档就要换那张环**:描边宽度 = `40 ÷ 半径px`,42px → `stroke-width='0.95'`,
  `.bucket-row::before` 因此单独一条,和别的卡片不共用。
