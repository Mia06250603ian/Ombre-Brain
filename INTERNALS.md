...
### 三个颜色旋钮(都在 `turbulence.html` 顶部 `<style>` 的 `:root` 里)

| 变量 | 管什么 | 现在的值 |
|---|---|---|
| `--drift-signal` | 高亮的字 + 凑近时那些**连线** | 深 `205,105,111` / 浅 `122,52,55` |
| `--drift-trace` | **往下坠的那些光迹** | 同上。**2026-09-03 所有者要求从 `signal` 里拆出来**,好单独换色;改它不影响连线和高亮 |
| `--drift-ink-boost` | 平时那些字符的**浓度**(乘在原版透明度上) | 深 `1` / 浅 **`1.5`** |

⚠️ **`--drift-ink-boost` 为什么浅色要 1.5**:原版那套透明度(`.28~.45`)是给深底设计的,
深色下正好;**同样的值压在浅底上会淡一截**。1.5 是所有者看了 1.0 / 1.5 两版真效果之后定的
(2026-09-03)。**别按「和深色统一」的理由改回 1** —— 那是没看图的人才会做的决定。
...
```bash
bash tests/turbulence-e2e/run.sh    # 47 项浏览器演练(2026-09-03 全绿)
```
