...
> 最后更新：2026-08-31(新增第 1.8 节「后台 /dashboard 的外观」;此前各节的日期以节内标注为准)
...
**改的只有外观和排布,后端一行没动**:没有新接口、没有新字段、`server.py` 的 `/dashboard` 路由
(仍是「读文件、发出去」)一个字未改,七个标签页与全部功能原样保留。
...
3. **记忆桶从表格行改成卡片**,顶栏加了一颗「记忆银河」按钮(见下)。
4. **iOS 那种毛玻璃**(顶栏/标签栏/卡片/药丸/详情抽屉),见下面《玻璃》一节。
...
**顶栏、标签栏、详情抽屉** —— 就这三处。做法是半透明底 +
`backdrop-filter: saturate(180%) blur(20px)`(抽屉 30px)+ 上缘一道 `inset 0 1px 0` 的高光。

⚠️ **卡片、药丸、面板一律实色,这是当天来回过一轮之后的结论,别再改回去。**
~~2026-08-31 先做成「整体加玻璃」(卡片/药丸/搜索框都半透明,并在 `body` 上画了
`--wash-a/-b` 两团极淡底光给它们当透视素材)~~ —— **所有者当天看了真机截图说「不像玻璃,很奇怪」,已撤销。**
**根因不是参数没调好,是这个做法本身不成立**:
- 玻璃之所以像玻璃,是**透出了后面的东西**。卡片底下是一整片平底色,什么都没有 ——
  现场算过:深色下卡片合成出来 `#1d1d1c`,而实色卡片本来就是 `#1a1918`,**差 3/255,肉眼等于没差**;
  上缘高光 `rgba(255,255,255,.08)` 同样看不见。
- 为了给它「可透的东西」而在页面上画光晕,那几团光**没有来由** —— 眼睛会问「这儿为什么会亮」,
  结果比不做还怪。所有者试过把光晕饱和度调成 0(纯灰)仍然觉得怪,于是整个撤掉。
- **顶栏那三处不一样**:内容真的从它们底下滑过去,玻璃在那儿是有来由的,所以留着。

**测试钉着这条**:`.header`/`.tabs` 必须有 `backdrop-filter` 且底是半透明;
`.bucket-row`/药丸必须**不透明且没有 blur**;`body` 的 `background-image` 必须是 `none`。
...
- **画布里的颜色一律 `cssVar()` 现读**(记忆网络、Breath 那两处)。写死的话深色模式下画出来还是浅色那套。
...
```bash
bash tests/dashboard-ui/run.sh      # 真浏览器演练 96 项(2026-08-31 现场数),深浅色各跑一遍
