...
### 详情卡上那颗标签:苹果做法,别改回描边胶囊

所有者拿她朋友那版的截图来比,原话「**不要那么有安卓味,按苹果 ui 设计走**」。
她指的是详情卡抬头那颗分类标签(`恋爱` 那种)。改法钉在这儿:

| | 之前(安卓味) | 现在(苹果做法) |
|---|---|---|
| 位置 | 正文**底下** | **标题正下方**,在 `#card header` 里 |
| 分类标签 | 1px 描边胶囊、9px 灰字 | **不描边**;底是 `rgba(--drift-signal, .13)`,字就是 `rgb(--drift-signal)` 本身 |
| 日期 | 虚线描边的**第二颗胶囊** | **不是标签了**,就是一句 `--drift-faint` 的次要文字,和标签并排 |

紧接着所有者又说「**我不要那个框**」—— 她朋友那版的卡上**一个描边框都没有**,我们原来三处全套着:

| | 之前 | 现在 |
|---|---|---|
| 关联列表 | 每条一个描边圆角框、两列 | **纯文字行**,一行一条:左边灰色关系名、右边加粗标题 |
| 右上角 ✕ | 套了个方框 | 光秃秃一个符号(34px 还留着,那是**手指的点击范围**,不是框) |
| 「在面板里打开」 | 描边空心胶囊 | **和「恋爱」那颗标签同一套**(见下) |

⚠️ **正文那一块是例外,它有框** —— 所有者随后补的:「**正文的部分是有一个和底色相近的玻璃质感框的**」。
关键在「**和底色相近**」:它不是描边,是 `--drift-sheet`(浅 `rgba(255,255,255,.62)` / 深 `rgba(255,255,255,.045)`)
**把卡片底再提亮一档**的一层玻璃 + `blur(10px)`,只够把正文和下面的关联列表分开,不够到像另一张卡。
**别给它加 `border`** —— 一描边就变回她砍掉的那三处框了。

**「在面板里打开」是真按钮,不是装饰**(所有者问过):它是个 `<a>`,`href` 指到
`/dashboard#bucket=<id>`,点了直接在后台展开这一条。

样式是她一句话定死的:「**改成和恋爱那个按钮一样的,黑白两版本都改成那个样的**」——
**就是标签那套:`rgba(--drift-signal,.13)` 的底 + `rgb(--drift-signal)` 的字,不描边**,
深浅两套自动跟着 signal 走。演练里直接拿它和标签**逐字段比对**(底、字色必须一模一样)。

⚠️ **这颗按钮我连试三版都被她否了,别再自己发挥**:signal 实心底 + 近黑字 → 否;
中性灰底 + 白字 + 收窄不通栏 → 「改的好奇怪」,否。**最终就是和标签同一套,通栏。**
⚠️ 关联行右边那格必须是 `minmax(0,1fr)`,**不能只写 `1fr`**(同本节上面那条,长标题会撑破卡)。

**安卓味的病根**是「一个标签堆三层装饰」(描边 + 底 + 高饱和实心)。
苹果只留两件事:**同色极淡底 + 强调色的字**,以及**只有该是标签的东西才做成标签**。
颜色**直接取 `--drift-signal`**(上一节她亲手拖的那个),**不另配色** —— 再配一个就又是两套色系了。
`tests/turbulence-e2e/` 里 D 段钉了三条(在抬头里 / 不描边 / 底是半透明),别绕过去改回去。

⚠️ **「在面板里打开」靠 `dashboard.html` 认的 `#bucket=<id>` 锚点**(同日为它加的)——
删了那个锚点,这颗按钮就只能跳到列表页,点开哪条还得自己找。
