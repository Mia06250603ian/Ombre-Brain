...
| | 原来 | 现在(C 档) |
|---|---|---|
| 卡片内边距 | `16px 18px` | **`13px 15px`**(C 档 11/14 上线后她在真机上改回 **B 档**) |
| 卡片圆角 | `--r-4`(20px) | **`--r-5`(26px)+ 超椭圆**(预览里一度到过 42,**真机上被否**,见下面那条) |
| 卡片间距 `.bucket-list` | `12px` | **`10px`**(同上,C 档 8 → B 档 10) |
| 内容区左右留白 `.content` | `20px` | **`14px`**(她:「横向长度拉长一点点」) |

⚠️ **「胖」和「方」要一起治**:卡片越大,同样的圆角越显方 —— 只收紧不提圆角、
或只提圆角不收紧,都不是她要的样子。⚠️ **圆角没有新造数值**,仍在那五档之内(挪到「大面板」档)。
### ⚠️⚠️ 预览不等于实机 —— 这一页最贵的一条教训(2026-09-03)

**所有者的原话**:「**圆角改回去救命 你的预览和实机差很多**」。
09-03 下午整批外观改动是靠**容器里 headless chromium 的截图**在她和我之间来回定的,
上线之后她在**真 iPhone**上一看,**当场要求把圆角从 42 改回 26**。

**为什么会差**(三条都验过,别再拿预览当结论):
1. **形状路径就不是同一条**:容器里的 chromium 141 **认原生 `corner-shape: squircle`**,
   而 **Safari 不认、走的是我们自己画的 mask 兜底** —— 两条曲线在数学上一样,渲染不一样。
2. **屏幕不一样**:她是 3 倍屏的真机,截图是模拟的 @3x;**字体也不是同一套**
   (容器里连不上 Google Fonts,退回系统字)。
3. **尺寸感受不一样**:截图是贴在电脑屏上看的一小张图,手机是拿在手里、离眼睛 30cm。

**所以规矩改成**:⚠️ **截图只用来「排除明显错的」,不用来「定稿」。**
**凡是尺寸/圆角/间距这类要用眼睛定的,先按一个保守值上线,让她在真机上看了再调** ——
一次部署几分钟,比在预览里来回猜四轮便宜得多(09-03 圆角那件事就来回了四轮,最后还是被真机推翻)。

⚠️⚠️ **记忆卡片的圆角**:09-03 在预览里一路调到「第六档 42px + 超椭圆」,
**上线后被真机否掉,已改回 `--r-5`(26px)、第六档已删**。下面这段过程留着是为了拦住重蹈覆辙:
**这四轮值得记住,因为每一轮都是同一个误解**:
`20px 普通圆弧`(她:太胖、好安卓)→ `26px 超椭圆`(她:「**这个圆角好安卓**」)→
`26px 普通圆弧`(她:还是安卓)→ **`42px 超椭圆`**(定稿)。
**反直觉但看得见的那条**:同一个数值下,**超椭圆是紧贴角切的、弧收得快,看着比普通圆弧更方**。
所以**「上超椭圆」必须同时把半径给够** —— 26 那两版就是栽在这儿,不是形状选错,是半径没跟上。

- ~~**新加的第六档 `--r-6: 42px`**~~ —— **已删,回到 `--r-5`(26px)**,环也换回 `stroke-width='1.54'`
  那张(和 `.todo-card` 共用)。**别再加回 42。**
- ⚠️ **换档就要换那张环**:描边宽度 = `40 ÷ 半径px`(26px → `1.54`,42px → `0.95`)。
- ⚠️ 中途 `.bucket-row` 被摘出超椭圆名单又加回来过,**两条超椭圆路径的演练探针已改用 `.todo-card`**
  —— 那样更稳:探针不该押在一个还在反复调的元件上。
- 演练另两条守着它:「兜底路上记忆卡片也被切成了超椭圆」「记忆卡片那一档够大(≥40px)」。
...
```bash
bash tests/dashboard-ui/run.sh      # 真浏览器演练 253 项(2026-09-03 现场数),深浅色各跑一遍
