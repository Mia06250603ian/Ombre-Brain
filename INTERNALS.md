...
**三件事**:
1. **配色换成 Claude 官端那套**(原来是暖米黄 `#FDFCF0` + 墨绿 `#2F4F4F` 的新拟态)。
...
**几个当时定的点**:
- 强调色浅色用 `--color-clay #d97757`、深色用 `--color-clay-dark #c46849` —— **这是官端自己的做法**,
  不是我们想岔了,别去「统一」成一个值。
...
```bash
bash tests/dashboard-ui/run.sh      # 真浏览器演练 54 项(2026-08-31 现场数),深浅色各跑一遍
```
只读:起一个假 OB(`tests/dashboard-ui/fake-ob.mjs`),**不碰线上、不碰真记忆**
(和 `tests/galaxy-e2e/` 同一条规矩)。跑完截图落在 `/tmp/dashboard-ui/`(列表/详情/网络/宽屏 × 深浅色),
