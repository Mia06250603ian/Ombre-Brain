...
| 卡片圆角 | `--r-4`(20px)+ 超椭圆 | **`--r-5`(26px)+ 普通圆弧**(见下面那条) |
...
⚠️⚠️ **记忆卡片最后走的是「普通圆弧」,不是超椭圆**(2026-09-03 所有者选的)。
她看了 **26 超椭圆 / 26 普通圆弧 / 34 超椭圆 / 42 超椭圆** 四张真图,选了**普通圆弧那张**。
**反直觉但看得见**:同一个数值下,**超椭圆是紧贴角切的、弧收得快,看着比普通圆弧更方** ——
她说「这个圆角好安卓」正是这个,不是半径不够。
所以 `.bucket-row` **同时从原生 `corner-shape` 那张名单和 mask 兜底那段里摘掉了**,
Safari 上也就是普通的 26px 圆角、不需要 mask、也不需要那圈描出来的环。
⚠️ **别再把它加回超椭圆名单**;**别的卡片(便利贴/信箱/配置块)仍走超椭圆,没动** ——
两条超椭圆路径的演练探针因此**换成了 `.todo-card`**,不再拿 `.bucket-row` 当样本。
⚠️ 演练另加两条守着它:「记忆卡片是普通圆弧」「圆角没被 mask 归零」。
