...
`<a href="/dashboard">`,横椭圆玻璃,和顶栏那两颗一个样子。
**所有者 2026-09-03 上线后点名要的**(「乱流加一个出口」)。

**为什么必须有**:这一页可以被**加到手机主屏**,那种模式下 Safari 那圈壳没了、
**连返回按钮都没有** —— 没这颗就只能杀掉重开。
⚠️ 用 `<a>` 不用 JS 跳转:点了才走,不带来任何请求(同 1.8 顶栏那两颗的理由)。
演练钉着四条:在、指向 `/dashboard`、是 `<a>`、有 `aria-label`(它只有图标)。

⚠️ **`/galaxy` 也没有出口,同一个毛病** —— 2026-09-03 报给所有者了,她没说要加之前别自作主张动。
