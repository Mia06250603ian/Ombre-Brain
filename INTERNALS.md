...
| | 现在 | 为什么不是别的 |
|---|---|---|
| **面** | **深色:白纱** `rgba(255,255,255,.07)` / **浅色:纯白实色** `#fff` | ⚠️ **两套现在不是同一套做法,别去"统一"**。深色下白纱那条仍然成立(~~52% 灰块~~ 已撤销 —— 合成出来 `#1d1d1c`,和实色卡片 `#1a1918` **差 3/255,肉眼等于没差**,所有者当场说「不像玻璃,很奇怪」)。**浅色 2026-09-03 改成纯白实色**:她拿一张 iOS app 的截图说「**我想要的是类似于这种的 iOS app 质感**」,那种靠**灰底 + 纯白卡片的落差**分层,半透明会把底色吃进来、白得不干净 |
| **边** | **深色:一圈均匀的淡描边**(10% 白)/ **浅色:不描边**(`transparent`,2026-09-03 改) | ~~上缘一道 `inset 0 1px 0` 的高光线~~ **已撤销** —— 所有者原话:「**卡片上方那个线有点太粗了不像玻璃感**」。2 倍屏上它就是一条又粗又硬的白杠。**参照物是 Claude Code 手机端自己的面板**(她截图给的):通体一圈淡边,没有亮线 |
| **底光** | 两套都只剩**一丝几乎看不见的中性光**(0.6% / 0.5%) | ⚠️⚠️ **2026-09-03 当天来回改过两轮,别照任何一版旧文改回去**:①原是「1%、纯灰」(她 08-31 拿滑杆定的,当时试过带色的说不要);②当天她拿**朋友那版**来问玻璃质感,改成**蓝 13% + 紫 10% 的三团彩光** —— **玻璃感的来源不是把 blur 调大,是玻璃背后得有东西可透**;③**当天她又拿一张 iOS app 截图说要那种质感**,于是彩光**撤掉**。⚠️ **「玻璃」和「iOS 平面」是两个互斥的方向**:前者靠透光,后者靠灰白落差。别把彩光加回来又指望它像 iOS。现在留一丝只为让顶栏的模糊有东西可糊 |
...
- **测试钉着这几条**:「卡片没有上缘高光线」(`box-shadow` 里不许出现 `inset`)、
  「页面有一层极淡的底光」(≤0.02,两套都是)、
  **「浅色卡片是纯白实色,不是半透明」**、**「浅色卡片不描边」**(两条 2026-09-03 新增),
  以及**磨砂那组分两档**:浅色只查顶栏和标签栏,深色连卡片和药丸一起查。
  ⚠️ ~~当天中途还加过「浅色底光必须真的有颜色」~~ —— 那是配彩光那一版的,**已随彩光一起撤回**。
- **阴影 2026-09-03 定稿:极轻的中性影**(`rgba(17,19,21,.04)`)。~~中途试过带蓝的柔影~~,
  那是配彩色底光的;彩光撤了之后蓝影会在灰底上显脏。iOS 那种卡片阴影几乎看不见,
  **分层交给底色落差,不交给阴影**。
- **卡片圆角 2026-09-03 由 16px 改成 20px**(五处一起改的),更接近 iOS。
