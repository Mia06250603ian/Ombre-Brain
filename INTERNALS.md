...
| | 现在 | 为什么不是别的 |
|---|---|---|
| **面** | **白纱**:深色 `rgba(255,255,255,.07)` / 浅色 `rgba(255,255,255,.66)`(⚠️ 浅色 2026-09-03 由 `.50` 提到 `.66`:底光加了颜色之后太透会让正文发花。**「白纱不是灰块」这条规矩没变**) | ~~深色下用 52% 灰块~~ **已撤销** —— 合成出来 `#1d1d1c`,而实色卡片本来就是 `#1a1918`,**差 3/255,肉眼等于没差**(2026-08-31 现场算的),所有者当场说「不像玻璃,很奇怪」 |
...
| **底光** | **浅色:蓝 13% + 紫 10%,三团**(2026-09-03 改)/ **深色:1%、纯灰,两团**(没动) | 玻璃要有东西可透,平底色上模糊了个寂寞。⚠️⚠️ **浅色那半 2026-09-03 由所有者本人推翻,别照旧文改回去** —— ~~原文「1%、纯灰(饱和度 0),她试过带色的不要,别调大别加颜色」~~ 已撤销。**起因**:她拿朋友那版的截图来问「像我朋友这样的玻璃质感是不是做不出来」。**关键认识:玻璃感的来源不是把 blur 调大,是玻璃背后得有东西可透** —— 1% 纯灰上再怎么模糊都是白盖白。**由两团加到三团**:只有两个角有光的话,往下滚就滚出了有颜色的区域,卡片又变回白盖白。**想退回她 08-31 那版**:浅色 `--wash-*` 换回 `rgba(17,19,21,0.010)` / `(…,0.008)`,并把三团改回两团 |

- ⚠️ **`background-color` 仍是官端那个纯色**,底光只走 `background-image` ——
  测试按底色对账,别改成 `background` 简写把底色一起吃掉。
- **选中的药丸刻意不透明**:上面压着字,透了就糊。
- **测试钉着三条**:「卡片没有上缘高光线」(`box-shadow` 里不许出现 `inset`)、
  「页面有一层底光」(**浅色最大 alpha ≤ 0.16、深色 ≤ 0.02**;上限是防止以后有人把它调成一锅粥)、
  以及 2026-09-03 新增的「**浅色的底光必须真的有颜色**」(三通道不全等)——
  最后这条拦的正是「顺手统一回灰」那类改动。
- **阴影 2026-09-03 改成带一点蓝的柔影**(`rgba(52,74,138,…)`):硬的灰影压在有颜色的底上显脏。
