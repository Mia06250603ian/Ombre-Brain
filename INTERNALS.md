...
> 最后更新：2026-09-01(新增第 1.10 节「记忆网络页为什么会打不开」与 1.11 节「便利贴页」;此前各节的日期以节内标注为准)
...
### 标签页:现在是九个(2026-09-01)

记忆桶 / Breath 模拟 / 记忆网络 / 信箱 / **便利贴** / 配置 / 导入 / **回收站** / 设置。

⚠️ ~~原文写的是「标签页一个没动、七个」~~ —— **外观那一轮确实一个没动**,
但**同一天后面新增了「回收站」**(2026-08-31),变成八个;
**2026-09-01 又加了「便利贴」**(见 1.11),现在是九个。**这个数每加一页都要来改。**
...
## 1.10 「记忆网络」页为什么会打不开,以及那次提速(2026-09-01)
...
### 外观:一张小票(2026-09-01 定稿,同一天改过两轮)

**现在这版**:一张收银小票 —— 一行一条、右边是记下的日期、底下 `TOTAL / 待做 / 已完成`
三行合计、下缘一排锯齿撕口。所有者拿一张小票的图来定的。样式在 `dashboard.html` 的
`.receipt*` 一节;**页面结构、接口、数据一个没动,换的只是这一层皮**。

⚠️ **同一天被撤销的两版,别改回去**(规矩 5:旧结论留在原地):
①**贴纸墙 + 明黄 `#fdf0b8` / 天蓝 `#dbeefd`** —— 所有者原话「好丑的颜色」;
②**贴纸墙 + 淡纸色**(A/B/C 三版真实效果里她选的 B) —— 做完她看了图,说要小票那种。
**两版都不是「做错了」,是她看到真东西之后改了主意**;所以下面这条教训仍然成立、而且更值钱了。

**三条别踩**:
1. **颜色和外观别靠猜**。1.8 之前那轮 dwell UI 留下的教训是「做几版真实效果给她看,
   或者给滑杆让她自己调」——本次三版对照图一次就定了 B,B 做完她又要小票,
   **每一轮都是看图才定的,没有一轮是文字描述能定下来的**。
2. **手写体走系统自带的字体栈**(`Snell Roundhand` / `Segoe Script` / `Brush Script MT`),
   **刻意不引 Google Fonts** —— 这个后台目前零外部请求,不为一行装饰破例
   (浏览器演练里钉了一条「手写体没走外部字体」)。⚠️ **测试容器里没有这些手写字体**,
   截图里那行会退化成衬线体;**她的 iPhone 上是 `Snell Roundhand`**,以手机为准。
