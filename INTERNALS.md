...
### ⚠️ 浅色 2026-09-03 由暖改冷(本节以下写「官端灰阶」的地方,只对深色成立了)

**所有者原话:「我是想改成冷色调一点,包括那个灰色按钮之类的」。**

| | 改之前(暖) | 改之后(冷) |
|---|---|---|
| 底色 `--bg` | `#faf9f5` | **`#f7f7f9`** |
| 主字 `--text` | `#141413` | **`#111315`** |
| 淡字 / 强调色当文字 | `#5e5d59` | **`#535a64`** |
| 描边 `--border` | `#e8e6dc` | **`#dee1e6`** |
| **强调色 `--accent`**(她点名的「那个灰色按钮」) | `#999999` | **`#8d97a5`** |

**做法**:`:root` 里**新增一套 `--cool-*`**,**明度逐档照搬官端、一个不动**,只把色相搬到 215°、
按深浅给一点饱和度;然后**浅色的角色表改指这套**。⚠️ **官端那套暖灰一行没删**,
**深色模式仍然整套用它,一个数没动**。**想退回暖的**:把浅色角色表里的 `--cool-*` 换回
`--color-gray-*` 即可,别删那段冷灰。

**为什么坚持明度不动**:本节下面那些对比度的账(白字压强调色 2.85、`#343434` 压深色卡片
1.48 之类)全是按明度算的。**明度不动 = 那些账原样成立**,不用重算。2026-09-03 逐笔对过:
白字压强调色 2.85 → **2.96**、正文压底 17.5 → **17.4**、淡字压底 6.26 → **6.51**,
**没有一笔变坏**。

⚠️ **`--net-dynamic` / `--net-feel`(Breath 评分条那两个)刻意没跟着改** —— 它们是官端的
clay / fig,是**语义色不是灰阶**,她也没提。要改再说。
...
| | 现在 | 为什么不是别的 |
|---|---|---|
| **面** | **两套都是白纱**:深色 `rgba(255,255,255,.07)` / 浅色 `rgba(255,255,255,.58)` | ~~深色 52% 灰块~~ **已撤销**(合成 `#1d1d1c`,和实色卡片 `#1a1918` **差 3/255,肉眼等于没差**,她当场说「不像玻璃,很奇怪」)。⚠️ ~~2026-09-03 中途把浅色改成过**纯白实色**(去追 iOS 平面质感)~~ —— **也已撤销**,她原话「**我就是想要玻璃质感啊你改改没了**」:**实色卡片是没有玻璃的**。`.58` 是权衡:再透正文发花,再实看不出透光 |
| **边** | **一圈均匀的淡描边**:深色 10% 白 / 浅色 6% 黑 | ~~上缘一道 `inset 0 1px 0` 的高光线~~ **已撤销** —— 她原话「**卡片上方那个线有点太粗了不像玻璃感**」,2 倍屏上是一条又粗又硬的白杠;参照物是 Claude Code 手机端自己的面板。⚠️ ~~09-03 中途去掉过浅色的描边(追 iOS 无边白卡)~~ **已撤销**:那圈边是玻璃的边缘,不是装饰。**「不许有高光线」这条一直有效** |
| **底光** | **两套都是:零饱和,强度看得见** —— 浅色纯灰 5% / 3.8%,深色纯白 4.5% / 3.5%,各三团 | ⚠️⚠️ **2026-09-03 浅色来回改了三轮才定,深色随后按同一套流程跟上,别照任何一版旧文改回去**:①原「1%」(她 08-31 拿滑杆定的)—— **太淡,玻璃没东西可透,白盖白**;②改成「蓝 13% + 紫 10%」的彩光 —— 她说「**我并不想要这么高的饱和度**」;③一度整个撤成近乎没有(去追 iOS 平面)—— 她说「**我就是想要玻璃质感啊你改改没了**」。**定稿的想法:玻璃要的是「背后有东西可透」,不等于「背后必须有颜色」** —— **纯灰的深浅变化一样能透出层次**。⚠️ **别加颜色,也别调回 1%**(那等于没有玻璃)。**三团**:只有两个角有光的话,往下滚就滚出去了 |

- ⚠️ **`background-color` 仍是官端那个纯色**,底光只走 `background-image` ——
  测试按底色对账,别改成 `background` 简写把底色一起吃掉。
- **选中的药丸刻意不透明**:上面压着字,透了就糊。
- **测试钉着这几条**:「卡片没有上缘高光线」(`box-shadow` 里不许出现 `inset`)、
  **「浅色底光够得上『有东西可透』」**(强度 0.02~0.09 —— **太淡=没玻璃,太浓=显脏**)、
  **「底光是中性灰,没有饱和度」**(三通道相等)、「底色饱和度为 0」、
  以及「磨砂」那组(顶栏/标签栏/卡片/药丸**两套都查**)。
  ⚠️ **当天撤销过的断言,别照旧文加回来**:~~「底光 ≤0.02」~~、~~「底光必须真的有颜色」~~、
  ~~「浅色卡片是纯白实色」~~、~~「浅色卡片不描边」~~、~~「磨砂浅色只查顶栏和标签栏」~~。
- ⚠️ **底光那两团用的是 `rgba(19,19,19)`,不是别处那个 `rgba(17,19,21)`** ——
  后者**自己带一点蓝**,不是纯灰。2026-09-03 演练那条「底光是中性灰」当场抓到的。
- **阴影是极轻的中性影**(`rgba(17,19,21,.04)`)。~~中途试过带蓝的柔影~~ —— 那是配彩光的,
  彩光撤了之后蓝影会在灰底上显脏。
- **卡片圆角 2026-09-03 由 16px 改成 20px**(五处一起改的)。
...
⚠️ **`--glass-card` 那 7% 白纱没动** —— 底光提上来之后它自己就读得出来了,不用再加厚。
...
1. **颜色和外观别靠猜**。1.8 之前那轮 dwell UI 留下的教训是「做几版真实效果给她看,
   ...
   没有一轮是文字描述能定下来的**。
2. **手写体走系统自带的字体栈,不为一行装饰再多引一种字体。**
   ⚠️ **2026-09-03 更正:~~原文那句「这个后台目前零外部请求」是错的~~,别照它下判断。**
   `dashboard.html` **第 21 行一直在从 Google Fonts 拉 Inter + Cormorant Garamond**
   (见 1.8「字体没换」那条,两处自相矛盾);这个后台**从来就不是零外部请求的**。
   **规矩本身仍然成立**(别为装饰再加字体),错的只是给它配的那个理由。
   ⚠️ **这句错话害过人**:2026-09-03 我照它跟所有者说「你的后台从不联外网」,当场说错。
   **真正零外部请求的是 `/turbulence`**(见 1.12,那儿有测试钉着),不是这个后台。
   ⚠️ **测试容器里没有手写字体**,截图里会退化成衬线体;
