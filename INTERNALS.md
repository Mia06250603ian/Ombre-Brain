...
三样东西凑出来的,**少一样就不像**:①半透明底 ②`backdrop-filter: saturate(180%) blur(20px)`
③上缘一道 `inset 0 1px 0` 的高光。挂在:顶栏、标签栏、卡片、药丸、搜索框、详情抽屉(它模糊 30px)。

- **玻璃底下要有东西才看得出来**。顶栏是内容滑过去;卡片底下本来是纯色,所以 `body` 上浮了一层
  极淡的 `--wash-a/-b` 底光(`background-image`,**`background-color` 仍是官端那个纯色** ——
  测试按它对账,别改成 `background` 简写把底色一起吃掉)。
...
**几个当时定的点**:
- **筛选药丸的表情**:**只有「未解决」和「已消化」去掉**,钉选 / Feel / 归档 保留
  (~~先做成整排都去掉~~,所有者当场纠正)。⚠️ **卡片上那颗图标是另一回事,别跟着删** ——
  它标的是每条记忆的状态(📌钉选 / 🫧feel / 🌿已消化 / 💤已解决 / 💭普通)。
...
星图那边是只读的,细节见下面 1.9 节。

### 怎么验

```bash
bash tests/dashboard-ui/run.sh      # 真浏览器演练 76 项(2026-08-31 现场数),深浅色各跑一遍
```
