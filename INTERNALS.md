...
⚠️ **一件已知的代价,报给所有者了**:~~↺ 那颗是唯一「一下回到 1.00×」的路~~,删了之后
**手机上捏乱了没有一键复位**(桌面滚轮推回去就行)。她说先删,**没让加替代**——
真要补,该做的是**双击画布复位**,别把按钮加回来。

### 怎么验

```bash
bash tests/turbulence-e2e/run.sh    # 98 项浏览器演练(2026-09-03 现场数,全绿)
```
