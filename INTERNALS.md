...
| **强调色 `--accent`**(她点名的「那个灰色按钮」) | `#999999` | ⚠️ **又换回 `#999999`** —— 见下面那条 |

⚠️ **强调色那一格转了一圈回到原点,别当成"没改过"或"漏改了"**:
`#999999`(她 08-31 定的中性灰)→ 09-03 上午随「整体改冷」变成 `#8d97a5`
→ **同日她说「浅色的那个按钮的颜色能不能也换成没有饱和度的灰」,于是又回到 `#999999`**。
**明度一直是 60,从头到尾没动过,变的只有饱和度。** 深色那颗仍是冷灰 `#2f3339` ——
**她只说了浅色那颗**。测试钉着「浅色那颗按钮零饱和」(三段十六进制必须相等)。
...
**测试钉着三条**:「卡片上那颗图标还在」(改成查 `svg.ico`,~~原来用 emoji 正则判~~)、
