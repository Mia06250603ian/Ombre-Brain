...
| **正文里的域** `.bucket-row .dchip` | ~~一串纯文字 `自省, 心理`~~ | **小胶囊**,`2px 6px` / 11px(两字约 36px) | 两字约 34pt |

⚠️⚠️ **三条底线,收窄时一条都不许破**:
1. **右上角那两颗仍必须是横椭圆,不许收成正圆** —— 她 2026-09-03 拿图指名要过
   「不要这种圆形,想要这种椭圆」(~~此前做成过 34×34 正圆,已撤销~~)。演练钉「宽 > 高 + 6」。
2. **域胶囊不许上颜色** —— 她朋友那版是彩色标签,但这一页是她定死的**整页零彩色**;
   彩色会当场破掉那条,零彩色那条演练也会红。材质走 **g3 微玻璃**(手册里 g3 本来就写着「小标签」)。
3. **`.domain` 这个类名一个字不许改** —— 它现在是容器、里面每个域一颗 `.dchip`;
   `renderBuckets()` 被搜索结果等处共用,改类名会连带搞坏别处。
   ⚠️ **回收站那份底行仍是纯文字**(要连 type 和副本数一起写),没跟着改。
⚠️ **踩过一个坑**:`.dchip` 第一版写 `max-width:46%`,**百分比是相对那个被挤扁的容器算的**,
于是全被截成「社…」。改成按字宽给上限(`10em`)+ 容器 `flex:1 1 auto` 才对。
