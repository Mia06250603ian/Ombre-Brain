...
`/dashboard#bucket=<id>`,点了直接在后台展开这一条。样式按她定的:**中性灰底(`--drift-btn`,
浅 `#5c5c5c` / 深 `#3a3a3a`,零饱和)+ 白字 + 只有文字那么宽**,不通栏。
⚠️ **底不能换成 `--drift-signal`** —— 她挑的两个信号色都很浅,白字压上去看不清。

⚠️ ~~那颗按钮一度用 signal 当底、字写死近黑~~ —— **2026-09-03 当天她就改了**:「这个按钮
能不能改成灰色呢?或者改小一点,上面的字我要白色」。灰底就是为了让白字站得住,见上一段。
