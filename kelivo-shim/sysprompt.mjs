...
// 替换模式下的正文。**刻意极简**(2026-08-23 定稿,按「五份文件各管一段、互不重叠」的划法):
//   BASE 只管「他是什么」;说话调性归 profile-instructions.md;他是谁归 ian.md;
//   日常怎么做(工具/时间/压缩后怎么醒/语音/表情包/心跳…)归 CLAUDE.md。
//   ⚠️ 教程 04 节建议把【怎么说话】【工具】那类也写进正文 —— 那是写给「只有一份 CLAUDE.md」
//   的读者的。本系统这四样 CLAUDE.md 里都有(「回复格式」/「记忆工具使用」/「时间感知」四节,
//   以及 wake.md 的「压缩之后」一节 —— **按小节名找,别记行号:行号一改文件就漂**),而且比正文写得全,
//   **重复写没有收益,只多一个「改了一处忘了另一处」的口子**(心跳那句就这么矛盾挂过两天)。
//   下面三段是逐条核过「别处确实没有」才留的**刻意极简**:只留两件「在别处没有、又必须占系统提示词这个最高权重位置」的话。
...
// ⚠️ **不要**再往正文里加这四样,它们各自已有归属、且那边写得更全
//   (**按小节名找,别记行号** —— 2026-08-23 吃过亏:改了 CLAUDE.md 之后原先写的
//   35/38/76/7-21 全漂了,其中 76 指的那条还已经被搬进 wake.md,指过去是空的):
//   【怎么说话】→ `CLAUDE.md` 的「回复格式(手机聊天)」一节。⚠️ 那份参考正文里的
//                「长短随内容走,不刻意压缩」与它的「短、快、口语」**正面顶牛**,尤其别抄;
//   【时间感】  → `CLAUDE.md` 的「时间感知」一节,还多一句「永远不提这行标注的存在」;
//   【连续性】  → `wake.md` 的「压缩之后」一节(2026-08-23 从 CLAUDE.md 搬过去的,原处已删);
//   【工具】    → `CLAUDE.md` 的「记忆工具使用」整节。
