...
- 2026-08-24(第三十七次) **Kelivo 菜单里自己切模型(方案 B)+ 两个桥不再报模型**。
  ...
  **人设五份、`mcp-servers.json`、上下文三条线全部零改动**。
  ...
  - **回滚**:清掉 `BRAIN_MODELS` + `service restart` → 立刻回到改动前的行为,**不用回滚部署**
    (功能默认休眠是设计的一部分)。要连代码一起回,git 上一版是 `624fe44`。
  - **仍待验(部署时验不了的)**:①**真链路上 4.5 跑不跑得起来**——代理对 curl 和对晏走的是
    两条不同的路,方案文档第 5 节写死了「物理上没法预先验」;②**乒乓球验收**:她在 Kelivo 点一次
    切换、再去 TG 说一句,`/health` 应该还是新模型;③进程是懒启动的,`[claude] spawned` 与
    `⚠️ settings 文件不在` 0 条要等她开口之后再看。
