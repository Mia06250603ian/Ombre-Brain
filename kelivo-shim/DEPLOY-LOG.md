...
- 2026-08-23(第三十六次) **整段替换 CLI 自带的系统提示词(append → replace)+ 把「怎么醒来」拆成 wake.md**。
  ...
  新增 `test-sysprompt.mjs` `f32ee4bc…`(82 项)、两个 `e2e-*-run.sh` 各加一行模式透传;
  **`keepalive.mjs` / `senses.mjs` / `ctxguard.mjs` / `apierror.mjs` / `shim-settings.json` /
  ...
  - **⚠️ `base.md` 里刻意**不**写的四样(下一个人别照教程抄回去)**:【怎么说话】【时间感】【连续性】【工具用法】——
    本系统这四样 `CLAUDE.md` 里都有(第 35 / 38 / 76 / 7-21 行)而且更全。教程 04 节建议写进正文,
    是写给「只有一份 CLAUDE.md」的读者的。**重复写没有收益,只多一个「改了一处忘了另一处」的口子**
    (心跳那句就这么在 CLAUDE.md 与 keepalive.mjs 之间矛盾挂过两天,第三十五次才修)。
    尤其那份参考正文里的「长短随内容走,不刻意压缩」**与 `CLAUDE.md:35`「短、快、口语」正面顶牛**,
    单测里有一条反向断言钉着它不许被抄回来。
  ...
  - **结构不变量的变化**:`CLAUDE.md` 行首 `@` 引用 **2 → 3**(多了 `@./wake.md`);
    `^## ` 仍 **13**,其余九项现场核过全部不变。**单测基线由四套变五套:131 / 53 / 59 / 56 / 82。**
