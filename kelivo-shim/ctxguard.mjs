...
//   archiveEveryTokens<=0 = 关闭**增量**(归档之后的周期性催档),但**硬线本身那一次仍然要催**:
//   只要上次归档发生在硬线之前(last < hard),硬线这一档就还没被满足过,到线照催。
//   ⚠️ 2026-08-10 的 bug 就出在这里:线上 every=0,软线存完日记①把 lastArchiveTokens 记上,
//   旧写法于是让 161500 永久失效(实测一个窗口 28 条消息、日志 `fire hard` 零条)。
//   改环境变量时记住:every 管「归档之后每涨多少再催」,硬线管「到这个水位必须有一次归档」,两者正交。
//
...
export function ctxDecide({ contextTokens, softTokens, hardTokens, archiveEveryTokens = 0, softFired, lastArchiveTokens = 0, trusted = true, finalTokens = 0, finalFired = false }) {
  ...
  if (hard > 0) {
    if (last > 0) {
      if (every > 0) {
        if (t >= Math.max(hard, last + every)) return { level: "hard" };
      } else if (t >= hard && last < hard) {
        // archiveEveryTokens<=0 关掉的是「归档之后的周期性增量」,**不是硬线本身那一次**。
        // 上次归档发生在硬线之前(典型:软线那次日记①)= 硬线这一档还没被满足过,到线仍要催。
        // 2026-08-10 实测:线上 every=0,软线 155396 存完日记①后 lastArchiveTokens 被记上,
        // 161500 从此永久静音——一个窗口 28 条消息一次没催过(日志 `fire hard` 零条)。
        return { level: "hard" };
      }
