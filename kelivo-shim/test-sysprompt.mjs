...
ok(base.length < 400, `内置正文要短(现 ${base.length} 字符);它取代的是 26,894 字符的自带提示词,别把省下的额度又填回去`);
// 别往正文里加「别假装调用工具/别编造工具结果」:2026-08-23 实测 2.1.215 与 2.1.241 的
// 自带提示词里**都没有**这类句子,写进去是新增规矩,不是补回丢失的地板。
ok(!/编造|假装/.test(base), "内置正文里不许夹带自带提示词本来就没有的新规矩");
