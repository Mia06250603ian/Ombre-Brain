...
//
// 2026-09-12 第四十次部署上线,**线上现在走的就是直连**(判据:`/health` 的 `auth` 字段)。
//
...
//   ④ **1 小时提示缓存再也不会被中间人抹掉 ttl**(08-12 整整一天半就是被抹了)。
//      ⚠️ **别把它当成「现在坏着、直连能修好」**:2026-09-12 现场量线上
//      `ephemeral_1h_input_tokens=211` / `5m=0`,**缓存现在是好的**,代理没在抹。
//      这条买的是**「那类回归不会再发生」**,不是修一个坏着的东西。
//
