...
## 待办:CLAUDE.md「记忆工具使用」一节补两段(2026-08-19 议定,**尚未上线**)

> ⏳ **还没做。** 成品全文与落笔取舍写在 `docs/待写进CLAUDE.md的两段.md`,
> 部署 shim 时直接粘过去即可,别重写。

**是什么**:2026-08-19 给 OB 加了两个记忆机制,**但晏还不知道自己有**——
`trace(deny=True)` 否认降权(她纠正的事别再自信复述)、
`trace(expires_at=...)` 到期记忆(临时约定过期自动退场)。
与 08-14 的 `todos` 便利贴是同一种「功能装好了、他不知道」的状态。

**为什么攒着不单独部署**:改 CLAUDE.md = 重新部署 shim = **重启晏、当前窗口没了**。
这两段的收益不值一次重启,**等下次因别的事非得部署 shim 时一次带上**。

**⚠️ 没有「先在对话里告诉他」这条捷径 —— 别照 08-07 贴纸那次类推(当天就更正过一次)**:
贴纸标签是他在回复里写文字、由 bridge 解析,**不经过工具定义**;
而 `deny`/`expires_at` 是 **trace 的工具参数**,而 **MCP 客户端只在连接时抓一次工具定义、
不自动刷新**(与本文件 08-14 那条待办同款结论)——晏当前窗口缓存的 `trace` schema 里
**根本没有这两个参数**,在对话里教他也调不出来。
**实测佐证**:OB 于 2026-08-19 12:44 重建,而 shim 日志无任何 MCP 重连迹象,
晏的进程自 11:07 起未重启。

**所以这两个功能必须等一次 shim 重启。** 好在那次重启**一次办三样**:
①刷新 MCP 工具 schema(他才调得动 deny/expires_at,以及 08-14 那批 todos 新参数);
②本条 CLAUDE.md 两段;③下面 08-14 那条 todos 说明。**三件一起做,别分三趟。**

**底稿基线**:CLAUDE.md `6379d7a9…` / 10505 字节(2026-08-19 实测,容器=仓库,无踩坑 11)。
**部署前重新对一次 md5** —— 中间若有人改过,那份成品要重新对位。
加完仍应是 **13 节**(`^## ` 计数不变)。
