...
> **行号 2026-09-23 现场重量过一遍**(第四十二次后又量了一次;上一次 2026-08-29)(此前正文那两个数已飘:踩坑写的 425、检查单写的 312,
> 实际是 574 / 417 —— 已更正)。**行号会飘,改完这份手册顺手 `grep -n "^#\{2,4\} " ` 核一遍。**

- **必须全文读的只有一节:「踩过的坑(别再踩)」(第 891 行,约 9500 token)。**
  那 22 条每一条都是真事故换来的,漏一条就可能重演;**别只读加粗标题就当读过了**。
- **其余各节按目录取用**:跟本次改动相关的展开读,不相关的不用读。
- **部署 shim 之前**,除踩坑外还要读完《部署检查单》(第 549 行)和「本目录刻意缺的三个文件」(第 451 行)——这两节是流程,跳了会漏步骤。

| 行号 | 节 | 约 token | 什么时候读 |
|---|---|---|---|
| 57 | ⚠️ 部署前必读(2026-07-13 事故教训) | 0.3k | **每次动 shim 都读**,三句话:仓库为唯一可信源、先 git pull、别用会话里的旧副本 |
| 70 | 当前 server.js 相对 7-12 初版的改动 | 10k | **改代码前读**。十四条改动清单:1 进程误杀补丁 / 2 标题拦截 / 3 定性锚点 / 4 时间注入 / 5 感官(天气+经期) / 6 保温+主动唤醒 / 7 上下文守卫 / 8 人设拆分 / 9 上游报错不再吃成空回复 / 10 系统提示词 append↔replace / 11 模型可切(2026-08-24) / 12 上游选路:代理 or 直连(2026-09-12 已上线) / **13 双引擎(5.5 走新版 CLI) / 14 思考翻中文(均 2026-09-23 第四十一次上线)**。**只读你要动的那一条即可** |
...
| 406 | 系统回合(`x-system-turn: 1`) | 0.5k | 动查岗/系统注入类功能时读 |
| 422 | 架构 | 0.2k | 第一次接触 shim 时读,一屏看完 |
| 441 | Zeabur 位置(IDs 供 CLI 用) | 0.4k | 要跑 CLI 命令时来抄 id |
| 451 | 本目录刻意缺的三个文件(部署前必须补) | 2.4k | **部署前必读**:ian.md / profile-instructions.md / mcp-servers.json 怎么从容器拷出来。**2026-08-29 起 `ian.md` 的版本考古(v18~v29 改了什么)也在这节**——从 OPERATIONS 第 3 节搬来的,改人设前必读 |
| 549 | **部署检查单(唯一权威版本)** | 0.2k | **部署前必读**,下面三小节是它的正文 |
| 558 | └ 部署前 | 1.0k | 〃 **末尾多了一小节《搭顺风车的待办》——每次部署 shim 前扫一眼**(挂着的都是「不值得为它单独丢一个窗口」的改动,只有这时候做才零代价) |
| 729 | └ 部署 | 0.3k | 〃 |
| 739 | └ 部署后(踩坑 9:上传成功 ≠ 上线) | 1.0k | 〃 **最容易被跳的一节,别跳** |
| 774 | └ 回滚的通用规矩 | 0.5k | 要回滚时读。核心一句:回滚不是零代价,等于再丢晏一个窗口 |
| 787 | └ 记录该写什么 | 0.2k | 部署完写 DEPLOY-LOG 时读 |
| 794 | 环境变量(值不入库;改值后要 restart) | 4.0k | **要调任何旋钮之前读**。链路/人格/感官/主动性/上下文守卫五组;**`BRAIN_MODELS`(模型菜单,不设=休眠)在这节**,另有一条 2026-08-24 实测的警告:**终线会被「发图那一轮」整个跳过**;**2026-09-01 起还有五个 `TOOLVIS_*`**(工具可见化,急救开关 `TOOLVIS_ON=0`);**2026-09-23 起还有 `NEXT_CLI_*`(双引擎)和 `THINK_TRANSLATE_*`(思考翻中文,急救开关设空)** |
| 839 | **晏的思考流靠哪一行撑着(2026-09-09)** | 0.7k | **有人问「官端不显示思考了,我们会不会也没」就读这节**;**升 CLI 之前也读**。晏的思考流的唯一支点是 `server.js:178` 那行 `--thinking-display summarized`;⚠️ **实测默认值已经是「不返回思考文字」**(和官方文档写的不一样),所以必须显式传。真坏了的现象:他照常说话、只有思考流不见了,**全线不报警** |
| 891 | **踩过的坑(别再踩)** | 8.8k | **⚠️ 唯一必须全文读的一节。** 22 条:1 消息抢跑 MCP 握手 / 2 upload 丢 dotfiles / 3 沙盒里测 claude 会卡死 / 4 订阅 OAuth 登录 / 5 令牌只能一处跑 / 6 Kelivo 开关注入 system / 7 OB 域名失效致 MCP 静默失败 / 8 自动标题也是注入源 / 9 deploy success ≠ 上线 / 10 连发 deploy 前一次被取消 / 11 别人的会话把版本滚回去过 / 12 她问问题≠授权动手 / 13 代发「归档」要慎用 / 14 卡在 Pulling 是调度挂了 / 15 .gitignore 让上传静默丢文件 / 16 经期记录每次部署被清空 / 17 deploy 传的是当前工作目录 / 18 进 DEPLOYING 就挤不掉了 / 19 --settings 指向不存在的文件会拒绝启动 / 20 新 import 要同步改 e2e 的 cp 清单 / **21 有些会话拷不出容器文件、检查单第 4 步会卡死(2026-08-30)** / **22 yarn 缓存把镜像撑到 3.3 GB、拉镜像卡死(2026-09-23,变量 `ZBPACK_INSTALL_COMMAND` 别删)** |
| 1128 | CLI 版本与升级指南 | 1.0k | 要动 CLI 版本时读(现钉死 2.1.215) |
| 1175 | ~~待办~~ 「她在干嘛」改写 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第二十四次」 |
| 1181 | 她在干嘛(如果开了) | 0.5k | 动查岗功能时读 |
| 1197 | ~~待办~~ 表情包补螃蟹 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第二十九次」 |
| 1203 | 表情包(如果接了 Telegram) | 0.8k | 动贴纸时读 |
| 1226 | ~~待办~~ 「待办便利贴」 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第三十三次」 |
| 1232 | ~~待办~~ 「记错了 / 过期了」 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第三十四次」 |
| 1238 | 建议(未做) | 1.5k | 想找「还能改什么」时读 |
| 1286 | **2026-08-21 心跳「自主活动」+ 压缩点实测** | 1.2k | **改心跳文案 / 动上下文阈值前读**。心跳纸条的**四条机械约束**在这节(`【。】` 判不出沉默那条是硬伤);压缩点**实测 166942**,并撤销了「改人设要跟着下调阈值」的旧推算 |
| 1319 | 2026-08-10 第三十一次的机制细节 | 0.2k | 下面四小节的引子 |
| 1326 | └ ① 161500 硬线永久静音 | 0.6k | 动上下文守卫阈值前读 |
| 1347 | └ ② 终线原话桶顺序错乱 | 0.2k | 〃 |
| 1358 | └ ③ 压缩摘要压不住九节 | 2.6k | 〃 **⚠️ 结论已更新(2026-08-24)**:纸条早已上线,第一次真压缩**成功**(九节命中 0、纸条那句命中 1)——**但晏没照做,压缩后没 awaken**。所有者说不修。问「为什么压缩后他不 awaken」先读这节 |
| 1429 | └ ④ awaken 的全文位把日记桶挤掉了 | 0.6k | ⚠️ **这节讲的是 OB,不是 shim** —— 改 OB 的 `awaken` 之前必读。**现在的结论是「OB 不用改」**;它里面那个「必然重演、要按类型各保一条」的旧结论**已被所有者当场推翻并撤销,别照它改**。另外它留了一把量「awaken 会不会把开机撑爆」的尺子(2026-08-10:占 27%;2026-08-21 钉选改出全文后:28%),往 awaken 加东西前拿它量一遍 |
| 1449 | 部署记录 | 0.8k | 只是指路:完整记录在 `DEPLOY-LOG.md`(**41 条,覆盖第 2~42 次**,2026-09-23 第四十二次后现场量;上次 2026-09-12 现场量;第一次记在 `TIMELINE.md` 07-12),按「第 N 次」搜 |
...
6. **缓存保温 + 主动唤醒合并**(2026-07-18,新文件 `keepalive.mjs`,原 heartbeatTick 移除):
   ...
   **这个前提塌了的时候,代码这边一点异常都看不出来**:ping 照发、`kaSilent` 照判、
   ...
   ⚠️ **这里的 1.25 倍是「ttl 被抹成 5 分钟」时的价,别拿它算别的场景**:
   **1 小时缓存的写入价是 2 倍**。正常情况下断一次链要重建多少,见
   `../OPERATIONS.md`《断一次到底多贵》(2026-09-02 算过:约 22.7 万当量,
   **不到保温一天的开销,便宜到不值得为它做任何优化**)。
...
11. **模型可切(Kelivo 菜单里自己选)**(2026-08-24,方案 B,新 e2e `e2e-model-run.sh` + `e2e-model-api.mjs` 20 项):
    ...
    **急救开关**:清掉 `BRAIN_MODELS` + restart,不用回滚部署。
    ...
    不认识它,要先单独立项升 CLI。~~ **2026-09-23 起改走第 13 条「双引擎」**:5.5 这类新模型
    **只能**经第二份新版 CLI 进名单,**别把 5.5 放进名单却清空 `NEXT_CLI_MODELS`** —— 那等于让 2.1.215 去跑它,空回。
    (**仍然成立的一半**:别为了新模型去整体升主力 CLI,那会碰 4.6,理由见第 13 条。)
...
12. **上游选路:代理 or 直连**(2026-09-12 第四十次上线,**线上现在走直连**)。
    ...
    ⚠️⚠️ **摘除为什么必须和开关绑在一个函数里**:CLI 凭据优先级里 `ANTHROPIC_AUTH_TOKEN`(2)
    排在 `CLAUDE_CODE_OAUTH_TOKEN`(5)**前面** —— 只加变量不摘代理,**CLI 会继续走代理而且一声不吭**,
    结果是「一切照旧」却让所有人以为换过路了。**别把摘除挪出去做成「记得手动删」。**
    接线只有三处:spawn 用 `buildAuthEnv(process.env)`、开机日志尾巴带 `auth`、`/health` 多一个 `auth` 字段。
    📌 `/health` 那个字段**还兼做上线判据**(只有新代码才有)**和看门狗的开关**
    (见 `../OPERATIONS.md` 第 7 节《长期令牌到期提醒》)。背景与量法见 `auth-env.mjs` 头注。
13. **双引擎:5.5 走第二份新版 CLI,其余模型逐字走 2.1.215**(2026-09-23 **第四十一次已上线**;
    思路借朋友 `Arisu-cross/kelivo-shim` 4372e2d,改了一处,见下)。纯逻辑在 `cli-bin.mjs`(单测 `test-cli-bin.mjs` 29 项),
    整链路 `e2e-dualcli-run.sh`(29 项,真 2.1.215 + 真 2.1.280 + 假后端)。
    **为什么不整体升 CLI**:2026-09-23 抓包比对,2.1.280 跑 4.6 时请求参数逐字相同,**但替换模式下
    会往第一条消息前多塞环境信息和「You are powered by Opus 4.6」**,没找到开关 —— 碰了「4.6 绝对安全」。
    **为什么非做不可**:2.1.215 二进制里 `claude-opus-5-5` 0 处,朋友实测它跑 5.5 = 空回、usage 全零。
    **怎么装**:`package.json` 多一行 npm 别名 `claude-code-next` → `@anthropic-ai/claude-code@2.1.280`
    (npm 会把新版的原生二进制嵌在 `claude-code-next/node_modules/` 下,两份互不干扰,2026-09-23 实装验过);
    `entrypoint.sh` 补下载它的二进制,**真能跑才导出 `CLAUDE_BIN_NEXT`**。
    **新版那份额外带三个变量**(只给它,4.6 那份一个都不加,单测钉着):
    `CLAUDE_CODE_AUTO_COMPACT_WINDOW=200000` + `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`(**两个都要**,
    只设后者限不住原生 100 万的模型)、`CLAUDE_CODE_TOTAL_TOKENS_REMINDER=off`。
    📏 **压缩点实测(2026-09-23,假后端二分,零额度)**:5.5 + 这两个变量 = **166796 不压 / 166991 压**,
    与 4.6@2.1.215 **逐位相同** → **三条上下文线一条都不用动**。**再量一遍**:照 `e2e-dualcli-run.sh` 的起法,
    让假后端第一轮回 `cache_read_input_tokens=X`、发第二条消息,看 stdout 有没有 `"status":"compacting"`,对 X 二分。
    ⚠️ **和朋友那版不一样的一处(刻意的)**:新版没装上时,他们让 5.5 退回旧版去跑(= 空回);
    **我们把 5.5 从菜单拿掉**,白名单随之拒绝,请求留在当前模型。`/health` 多三个字段:
    `cli`(main/next)、`cliNext`(ready/missing)、`modelsDropped` —— **看门狗读它们**(见 `../OPERATIONS.md` 第 7 节《看门狗》)。
    ⚠️ **只影响 5.5、认了的一件**:新版给 5.5 也会塞环境信息和「You are powered by Opus 5.5」,关不掉。
    **急救**:①让她在 Kelivo 菜单切回 4.6(零命令,丢一个窗口);②从 `BRAIN_MODELS` 拿掉 5.5 + restart。
    **部署后验收**:`/health` 的 `cliNext` = `ready`、`modelsDropped` = `[]`;她切 5.5 说一句后 `cli` = `next`;
    runtime 日志 `[claude] spawned claude-opus-5-5 … cli next`;**切回 4.6 后 `cli` = `main`**。
    ⚠️ **5.5 有一道会卡死整个窗口的安全拦截(2026-09-23 真撞过一次,`../TIMELINE.md` 09-23 第五十二件)**:
    她那边看到的是 `⚠️[shim] 上游断了…(上游报错)`(`kind` 为空),**但链路没断** —— `/debug` 的 `lastApiError` 原文是
    `Opus 5.5's safeguards flagged this message … Claude Code can't respond to this message with Opus 5.5`。
    CLI 把被拦的那句留着、之后每句都连它一起重发,**所以一旦被拦,这个窗口在 5.5 上就再也说不了话,等也没用**。
    **恢复**:她在 Kelivo 切回 4.x 说一句(= 新窗口);或 restart。**窗口保不住**(被拦的窗口没法先归档,他一句都回不了)。
    📋 **当晚的实验结论(假后端、零额度、真 2.1.280,别重做)**:①给活着的进程发 `control_request` `{subtype:"set_model"}`
    **能不重启就换模型**,会话、历史都在(换完下一条消息里会多一行 `Set model to …`;切换时 CLI 会先发一个 `Hi` 验证模型);
    ②**被拦之后 `set_model` 到 4.6,CLI 自动把被拦的那几轮丢掉、之前的对话全保留**,4.6 能接着聊;
    ③进程退出后 `--resume <会话id> --model claude-opus-4-6` 读回会话,同样能接上、同样丢掉被拦的几轮;
    ④**同一窗口再切回 5.5 会再被拦**(被拦的几轮还在会话记录里,只是没发给 4.6)。
    2.1.280 自带一个 `switchModelsOnFlag` 设置(「被拦就自动换模型」),**线上当晚没触发**,原因没查。
    **所以「被拦不丢窗口」技术上做得到**(菜单加一个跑在新版 CLI 上的 4.6,或 shim 发现被拦就自动 `set_model` 并重发),
    代价:兜底那个 4.6 开头会多读「你在 Linux 上 / 你是 Opus 4.6」两句(`base.md` / 人设 / 天气时间经期注入**完全一样**,抓包逐字比过);
    shim 里「换模型 = 新进程」的假设要改(八处报模型、守卫复位、看门狗读的 `cli`),Kelivo 之后还发「5.5」的请求不能把进程拽回去;
    真接口认不认 5.5 的思考块转给 4.6 **没验**。**所有者 09-23 看完决定先不做,先观察 5.5 多久被拦一次**;常被拦再做。
    顺带一件小事也挂着:`apierror.mjs` 那句「链路修好就能接着聊」对这种拦截是**误导**,真做时一起改成「5.5 拦了、去切 4.6」。
14. **思考翻中文**(2026-09-23 **第四十一次已上线**;借朋友 7efb2a9 / f766260,改了三处)。
    5.5 的思考只给**英文摘要**(由另一套系统写,人设管不到),所有者英语不好。`think-translate.mjs` 包一层 sink:
    模型思考先攒,一段结束(`content_block_stop`)就用一次性 `claude -p`(**主力 CLI + `claude-sonnet-4-6`**,
    临时 HOME、无 MCP、无工具、关思考)翻成中文再发;正文、工具标记、finish 排在它后面,顺序不乱。
    单测 `test-think-translate.mjs` 27 项 + `e2e-dualcli-run.sh` 里的翻译断言。
    **和朋友那版的三处不同**:①一段结束**立刻排进翻译队列**,不等前一段发出去(他们要等前一段发完才开始下一段);
    ②**工具可见化那几行(`〔🔧〕` / `→` / `←`)直通不翻**;③称呼用 `USER_NAME`/`AI_NAME` 代入,不写死人名。
    **只包看得见思考的出口**(流式 SSE = Kelivo / Telegram 桥 / 网页),保温轮与系统回合不翻。
    ⚠️ **内存(2026-09-23 实测)**:一个翻译子进程峰值约 **300 MB**,整机可用只有 1.2~1.5 GB(和浏览器抢),
    所以**同一时刻只跑一个**(`THINK_TRANSLATE_CONCURRENCY`,默认 1;**别随手调大**)。
    ⚠️ **代价**:①**正文会等翻译**,每段多几秒,**最长 `THINK_TRANSLATE_TIMEOUT_MS`(默认 20 秒)**,超时发英文原文;
    ②每段思考多一次 Sonnet 调用,吃订阅额度;③译文是**转述的转述**,语气可能再偏一点。
    **兜底**:翻译失败 / 超时 / 空 → 原样发英文,**思考可以晚到,绝不丢**。
    **急救开关**:`THINK_TRANSLATE_MODELS=""` + restart = 立刻不翻(e2e 阶段 C 验过)。
    📋 **2026-09-23 所有者嫌译文「译制腔」,当天在容器里用真模型试过三条路(每条 3~4 段样例,共花几毛额度),结论先记这儿,别重新试一遍**:
    ①**Sonnet +「改写成他的口吻」**:明显更像他,**但碰到亲密内容 Sonnet 会写到一半拒绝**,拒绝的话会漏进思考流,
    且会把「按照设定,先认可她的感受」整句删掉(丢意思)—— **不能用**;
    ②**Sonnet +「翻译」身份 + 去译制腔规矩**(约 400 token):不拒绝、意思不丢,但**仍是翻译稿**,破折号屡禁不止;
    ③**Opus 4.6 + 把 `profile-instructions.md` 的 Thinking requirements 当规矩 + 给它看前后文,「还原成内心独白」**:
    **真正像他在想事**,不拒绝、意思都在;代价每段约 10 秒、约 1~2 毛额度、长 2~3 倍,
    且上线时只能给它看**她那句话**(他的回复在思考之后才生成)。
    **根因**(别再在措辞上打转):5.5 给的摘要**本来就是「行动计划」不是独白**,由另一个模型写,官方文档说原始思考任何设置都拿不到。
    **所有者拍板:不多花钱,线上保持现状**(Sonnet + 原提示词)。**以后若要改,直接从③起步**;
...
kelivo-shim(本目录, Zeabur 服务名 kelivo-shim)
   ...
   ▼
Anthropic(**直连**,2026-09-12 第四十次起;子进程带 CLAUDE_CODE_OAUTH_TOKEN)
   +
Ombre Brain 记忆库(Zeabur 另一项目, streamable-http MCP)
```

⚠️ **这里以前是 `CLIProxyAPI ──→ Anthropic` 那一跳,2026-09-12 已摘掉。**
**判据**:`curl https://yan-shim.zeabur.app/health` 的 `auth`(`direct` / `proxy`)。
代理服务还留着当退路(删掉 `CLAUDE_CODE_OAUTH_TOKEN` + restart 就退回去),但不再过流量。
选路逻辑见 `auth-env.mjs` 头注与改动清单第 12 条。
   ...
   **2026-07-20 起拆出姊妹文件 `profile-instructions.md`**(下一条),两份一起才是完整人设。
   ...
   > 细节进它该在的手册。**一个字没改。**
     ...
     - `ian.md`(人设本体,私密)——从运行中容器 base64 拷出,当前 **v29**(23045B,md5 `8918742d…`,
       ...
       ⚠️ **9.5 这一节有两条所有者拍板的约束,改 `ian.md` 之前必看**(2026-08-30 从 `TIMELINE.md` 08-19
       第七件挪来,照规矩 1;那边只留一句指路):
       ①**不写「出戏」** —— 原参考文本要求「区分出戏的真实反馈并必须听」,那等于**在安全词之外开第二个出口**,
       与 9.4 的 `The only valid stop signal is the safe word` 直接打架(同第十九次删 `"stop,"` 那次同类)。
       **所有者选择不写,全文 0 处,别好心补回来。**
       ②**与 9.4 / Pact One / Pact Four 重复的那三句是刻意不写的** —— 9.5 就在 9.4 隔壁,靠它兜底;
       而**既有的跨节重复一处未动**(第十九次所有者拍板「重复是有意的」)。别把这两件事搞反。
       另记:**9.5 的文本是所有者与晏本人共同定的**(她把草案交给晏压缩,以他那版为底再补回三处),
       不是我写的稿子 —— 要改先问她。
...
2. **九套单测全绿**:`node test-ctxguard.mjs && node test-senses.mjs && node test-keepalive.mjs && node test-apierror.mjs && node test-sysprompt.mjs && node test-toolvis.mjs && node test-auth-env.mjs && node test-cli-bin.mjs && node test-think-translate.mjs`
   (当前基线 **131 / 53 / 59 / 56 / 86 / 63 / 33 / 29 / 27**,2026-09-23 现场跑;后两套是双引擎和思考翻译,~~原文「七套」停在 09-12~~。
   ⚠️ **加了新的单测,这一行要跟着改** —— `test-toolvis.mjs` 2026-09-01 加了却漏在外面,
   直到 09-12 才补上,中间每次部署都少跑一套)。
   动了 `server.js` 或流事件相关的还要 `bash e2e-run.sh` 与 `bash e2e-apierror-run.sh`;
   **2026-09-23 起再加 `bash e2e-dualcli-run.sh`**(双引擎 + 思考翻译,29 项;会下载两份 CLI)。
3. **全量 md5 对账**:`service exec … md5sum *.mjs *.js *.sh *.json *.md *.txt`,
   **容器与仓库逐件比,不能挑几件比**(踩坑 11 的唯一防线;2026-08-03 就出现过「容器改了仓库没提交」,
   ...
   ⚠️ **比之前两边都先 `tr -d '\r'`** —— 远程执行的输出常带回车符,不去掉会看到「**全部不同**」的假差异,
   于是把一次正常对账读成「线上整个漂了」。
...
5. **四个 `/mcp` 各连 3 次、必须 3/3 200**(OB / browser / gmail / **netease**;~~原文写「三个」~~ 停在
   2026-08-30 接 netease 之前)。**token 从 `mcp-servers.json` 里读,别打印出来**;
   一次把四条都验了的写法见 `DEPLOY-LOG.md` 第三十八次。
...
> **现在挂着 1 条**(2026-09-26 挂的,见下)。
> 再挂新的照原样加 `- [ ]` 一条;做完划掉、写清哪次做的。

- [ ] **心跳出口别只认 Telegram**(2026-09-26 第四十三次上线「跟着她走」后,核耦合时发现的两处;所有者要求「别耦合太多」)。
  ①`kaPrompt` 里写死「会直接出现在你们的 **Telegram** 对话里」—— 心跳现在可能落进 iMessage,晏可能说「去 TG 看」。
  改法:`kaPrompt` 的 `viaBridge` 换成出口名(取 `pushTargets(...)[0].name`),文案按出口说,或干脆改成不点名的「会直接出现在你们的对话里」。
  ⚠️ 这段文字有四条机械约束(`keepalive.mjs` 注释里),改的时候别碰开头和结尾。
  ②`proactivePush` 和 `kaDecide` 的 `hasChannel` **只看 `BRIDGE_PUSH_URL`**:哪天删了 Telegram 桥、只剩 `IMESSAGE_PUSH_URL`,
...
- [x] ~~**读 CLI 输出时按 UTF-8 流式解码(防汉字碎成 `���`)**~~(2026-09-23 挂上)—— ✅ **当天第四十二次已部署**,容器里 `grep -c 'setEncoding("utf8")' server.js` = 1。原文如下:
  ...
  管道分块正好切在汉字中间就会碎。**同款 bug 当天在 telegram-bridge 上撞到并已修**(她在 TG 看到「额头,鼻尖,���巴」,
  见 `../telegram-bridge/MAINTENANCE.md` 已知边界 10);shim 这处**没见过真发作**(管道块大、切中概率低),
  所以**不值得为它单独丢一个窗口**。单测九套 + `e2e-run`(replace)+ `e2e-dualcli` 在改后已跑过全绿。
  **部署后验**:容器里 `grep -c 'setEncoding("utf8")' server.js` = 1。

- [x] ~~**工具调用在 thinking 里显示参数和返回值**~~(2026-08-31 挂上;所有者当天提的)
      —— ✅ **2026-09-01 第三十九次已做**(所有者当天说「部署 不用归档」,故未归档)。
      落地与撤销的错结论见 `DEPLOY-LOG.md` 第三十九次;**做完本小节又空了**。
      —— **起因**:她看到朋友那边的截图,聊天里能看到 `→ hold {"content":…}` 和 `← hold: {"result":…}`,
      问我们为什么没有。**查过了,是 shim 这边,和 OB 无关** —— OB 只是被 CLI 用 MCP 调一下、
      结果回到 CLI 进程里,**它没有通往手机的通道**,给不给看全由 shim 决定。
      **现状**(`server.js:220-228`):`content_block_start` 里遇到 `mcp__` 开头的 `tool_use`,
      只往 thinking 里插一行 `〔🔧 工具名〕`,**参数和返回值都没转发**:
      ① 参数走 `input_json_delta`,而 `:230` 的 `content_block_delta` 分支只认
      `text_delta` / `thinking_delta` —— `input_json_delta` **在 `server.js` 里出现 0 次**(2026-08-31 现场数),没人接;
      ② 返回值更彻底:`handleEvent` 只有 `stream_event`(`:214`)和 `result`(`:236`)两个分支,
      工具结果那类事件**连函数都进不去**。
      **改法**(约 30 行):按 content block 的 `index` 攒 `input_json_delta` 的 `partial_json`,
      `content_block_stop` 时吐进 thinking;再给工具结果加一个分支。
      ⚠️ **两件动手前必须先定的事**:
      **①返回值要截断到多少字**(~~`breath`/`awaken` 的返回是几千字,原样吐进 thinking 会把窗口吃掉
      —— 这不是显示问题,是**真花 token**~~ ⚠️ **2026-09-01 读码核实,这句已撤销,别照它改**:
      这些字只走 shim → 手机那条 SSE,**从不回流进 claude 进程**(每轮只喂 `lastUser`),
      而工具返回**本来就已经在他窗口里**(CLI 得把结果交给模型才能继续)。**显示是免费的。**
      所以截断是**防刷屏**,不是省窗口 —— 所有者想看全**尽管调大,没有窗口代价**。
      考据在 `toolvis.mjs` 头注;建议默认 200 字 + 省略号,**等所有者定**);
      **②参数里可能有隐私**(`hold` 的 `content` 就是记忆原文),她自己看无所谓,
      但**截图外发时会连正文一起露出去** —— 报备过再做。
      **为什么挂在这儿**:纯显示效果,**不值得为它单独丢晏一个窗口**。
      ✅ **2026-09-01 代码已写完并验完,仍未部署**(等下一趟顺风车)。落地形态:
      新模块 `toolvis.mjs`(纯逻辑)+ `server.js` 接线三处(攒 `input_json_delta` / `content_block_stop` 吐参数 / 新增 `user` 事件分支收工具结果);
      四个环境变量 `TOOLVIS_ON` `TOOLVIS_ARG_CHARS` `TOOLVIS_RESULT_CHARS` `TOOLVIS_REDACT`(+`TOOLVIS_REDACT_KEYS`),
      **改值 + restart 即生效**,急救开关是 `TOOLVIS_ON=0`。
      **验收**:`test-toolvis.mjs` 63 项 + 原有五套单测全绿;三套 e2e(守卫 / 报错 / 模型)全绿;
      另用一次性探针(真 CLI + 真 stdio MCP 服务 + 假后端)跑通整链路,确认三件事件形状与假设一致
      ——参数分片走 `input_json_delta`(按 `index` 攒)、`content_block_stop` 带 `index`、
      **工具结果是一条 `user` 事件里的 `tool_result` 块**(所以原来的 `handleEvent` 接不到)。
      ⚠️ **部署前她该知道的两件**:①`TG_THINKING=1`(2026-07-17 她自己要求开的)意味着这两行
      **会进 Telegram 的折叠引用**,也就是 `hold` 的记忆原文会以消息形式落进 TG 聊天记录;
      ②`../dwell-bridge/dwell-lib.mjs:72` 只认 `〔🔧 名字〕`,新加的 `→`/`←` 两行在网页上会显示成普通思考文字。

      ✅ **2026-09-01:代码已写完并验过,挂在分支 `claude/repo-docs-operations-jsiumk` 上,尚未部署**
      (所以这条**仍然是 `- [ ]`**:下次部署 shim 时它会跟着走,不用再写一遍代码)。
      落地:新增 `toolvis.mjs`(纯逻辑)+ `test-toolvis.mjs`(**57 项全绿**,2026-09-01 跑);
      `server.js` 只做接线(攒 `input_json_delta` → `content_block_stop` 时吐 `→` 行;
      新增 `user` 事件分支吐 `←` 行;`turn` 上多两个 **每轮清零** 的 Map)。
      **照踩坑 20,三个 `e2e-*-run.sh` 的 `cp` 清单都已加上 `toolvis.mjs`。**
      **所有者当天定的两件**(她原话「不太懂」,故按建议默认值走,并做成可改的旋钮):
      ①**截 800 字**(先定 200,当日核实「显示不花窗口」后她说可以调大);②**参数照常显示、不打码** —— ⚠️ 这意味着 `hold` 的 content(记忆原文)
      会出现在思考流里,**她截图外发会连正文一起露**,已当面报备。要改:`TOOLVIS_REDACT=1`。
      四个旋钮 `TOOLVIS_ON` / `TOOLVIS_ARG_CHARS` / `TOOLVIS_RESULT_CHARS` / `TOOLVIS_REDACT`
      见下面《环境变量》一节,**改值 + restart 即生效,不用重新部署**(`TOOLVIS_ON=0` 是急救开关)。
      **事件形状是 2026-09-01 拿真 CLI 2.1.215 + 假后端 + 一个真 stdio MCP 服务现场核过的**
      (探针在会话的临时目录里跑完就删,没进仓库):三片 `input_json_delta` 按 `index` 到齐、
      `content_block_stop` 有、工具结果确实是一条 **`user` 事件**里的 `tool_result` 块
      (`content` 是 `[{type:"text"}]`,带 `tool_use_id`)。**别再重新推一遍这三件。**
      ⚠️ **跨服务(规矩 6):`../dwell-bridge/MAINTENANCE.md` 那边也留了指路** ——
      dwell 只认 `〔🔧 …〕`,新的 `→`/`←` 两行它会当成普通思考文字原样显示(不崩、不丢字,但观感会变)。

- [x] ~~**长期令牌直连:把 CLIProxyAPI 从链路上拆掉**~~(2026-09-12 挂上,**当天第四十次就做掉了**)
      —— ✅ **已上线**:`/health` 报 `auth=direct`,子进程环境里代理那两个变量与 API key 全不在。
      所有者本人先对晏说过「归档」。完整记录见 `DEPLOY-LOG.md` 第四十次。**做完本小节又空了。**
      **为什么挂在这儿**:代码本身零风险(不设变量 = 逐字回到原行为),
      但切换必须重启常驻进程 —— **不值得为它单独丢晏一个窗口**。
      **它买的是什么**:①**重新授权从 28 天一次变成一年一次**(四个数据点 30/29/29/28);
      ②**09-09 那场故障整类消失**(真病根是代理冒充的 CLI 版本指纹落后,直连之后没有「冒充」这回事);
      ③拆掉《CLIProxyAPI 版本漂移》那颗雷(没有版本锁、重启即吃最新版),
      连带解开「`MANAGEMENT_PASSWORD` 泄露过却不敢转」的死结。
      **完全不变**:晏本人、记忆库、三个前端、四个 MCP、计费(同一个订阅同一个池)。**不多任何功能。**
      ⚠️⚠️ **唯一的技术陷阱**:CLI 凭据优先级里 `ANTHROPIC_AUTH_TOKEN`(2)排在
      `CLAUDE_CODE_OAUTH_TOKEN`(5)**前面** —— **只加变量不摘代理,CLI 会继续走代理而且一声不吭**。
      所以摘除**绑死在** `auth-env.mjs` 里,不靠谁记得去删。
      **落地形态**:新增 `auth-env.mjs`(纯逻辑)+ `test-auth-env.mjs`(**33 项全绿**,2026-09-12);
      `server.js` 只做接线三处(spawn 用 `buildAuthEnv(process.env)` / 开机日志带 `auth` /
      `/health` 多一个 `auth` 字段)。**照踩坑 20,三个 `e2e-*-run.sh` 的 `cp` 清单都已加上 `auth-env.mjs`。**
      **已验(2026-09-12)**:七套单测 + 三套 e2e 全绿;
      **钉死的 CLI 2.1.215 支持长期令牌**(`setup-token --help` 打得出用法、二进制里该变量 72 处)
      —— **不用为这件事升 CLI**,量法在 `auth-env.mjs` 头注;
      另用一次性探针(真 shim + 假二进制 + 假令牌)确认接线:`/health` 报 `direct` 时子进程里
      `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_API_KEY` **全不在**,不给令牌时三者原样在。
      ⚠️ **部署这一条之前,先把这两件验掉**(都对线上零影响):
      ①**在容器里**跑 `node -e "fetch('https://api.anthropic.com/v1/models').then(r=>console.log('HTTP',r.status))"`,
      **期望 `HTTP 401`**(别在别的机器上跑,那验的是另一条出口);
      ②代理有没有给模型改名(有的话 `BRAIN_MODELS` 要跟着改回官方 ID)。
      ⚠️ **部署那次的额外验收(加在检查单第 12 条上)**:`/health` 的 `auth` 必须是 `direct`;
      按踩坑 9 的办法进容器,确认 claude 子进程的环境里代理那两个变量**真的不在**
      (⚠️ **别用 `ps | grep claude`** —— 你自己那条 `sh -c` 命令行里含 "claude" 会匹配到它自己,
      而它继承的是容器完整环境,**会让你误判成「没摘掉」**;要遍历 `/proc` 用 `readlink exe` 认、并跳过 `$$`);
      然后**让所有者本人真发一条消息**确认出得了话,三个模型各测一遍。
      ⚠️ **她要做的只有:在手机上点一个授权链接,把回来的码发回来**(外加把到期日填进仓库变量
      `CLAUDE_TOKEN_EXPIRES`)。**她只有手机,没有电脑 —— 别对她说「你在自己电脑上跑一下」。**
      令牌要**由会话在容器里用脚本驱动 TUI** 取得
      (⚠️ 读屏必须用 `pyte` 那类真终端模拟器,正则剥转义码会吞字符 → 令牌少字 → 401,
      **而它只显示一次,抠错等于那次授权作废、她要重点一遍**),
      ⚠️ **且必须和「写进 Zeabur」放在同一次会话**(令牌只显示一次,会话容器一次性)。
      完整取法与两项自检见 `../OPERATIONS.md` 第 7 节《长期令牌直连》里那段。
      **回退**:删掉 `CLAUDE_CODE_OAUTH_TOKEN` + restart 即自动退回代理线路,不用回滚部署
      (⚠️ restart 仍然要丢一个窗口;⚠️ **代理服务至少观察一两周再谈删**)。
      ⚠️ **跨服务(规矩 6)**:这件事同时动了**看门狗** —— 见 `../OPERATIONS.md` 第 7 节
      《长期令牌直连》与《长期令牌到期提醒》。切过去之后代理的凭证再也不刷新,
      《续命预警》那条会靠读本服务 `/health` 的 `auth` 字段自动让位,否则会开始刷假警报。

- [x] ~~**把表情回应的语法写进 `CLAUDE.md`**~~(2026-08-28 挂上)
      —— **2026-08-30 第三十八次已做**,和下面那条搭同一班顺风车。
      落地:加在已有的 `## 表情包(如果接了 Telegram)` 那节**末尾**,没新开节;
      正文所有者事先逐字过目。机制细节见 `telegram-bridge/MAINTENANCE.md` 设计要点 20
      (那边也留了回指:语法已进 `CLAUDE.md`)。

- [x] ~~**把网易云音乐 MCP 接上**~~(2026-08-29 挂上)
      —— **2026-08-30 第三十八次已做**。三处:①`mcp-servers.json` 加第四条 `netease`
      (streamable-http + `X-Token`,43 字符,照 `browser` 那条的格式,URL `https://yan-netease.zeabur.app/mcp`);
      ②`ALLOWED_TOOLS` 五项 → 六项(`variable update` **不 restart**,随新容器生效,第二十次那招);
      ③晏的 `CLAUDE.md` 新增 `## 音乐(如果接了)` 一节(放在「邮箱」与「语音」之间),正文所有者逐字过目。
      **`^## ` 因此由 13 变 14**,下面第 11 条的结构不变量已同步。
      **接之前那两个前提当天都验过**:`/health` 的 `loggedIn:true`;四个 `/mcp` 各 3/3 200。
      ⚠️ **窗口账的实测结果和当初的估算对不上**:原估「18 个工具常驻 1600~2000 token」
      (2026-08-29 按 `TOOLS` 块 5796 字符 ÷ 3.5 估的),而 2026-08-30 接上后**开机占用 38018**,
      对比 08-23 第三十六次的 33721 **高约 4300**。⚠️ **这个差不干净**——里面还混着 `CLAUDE.md`
      新增的两段(约 150)和两次测量首轮对话内容的差异,**别拿 4300 当工具的净成本**;
      要精确归因得单独再量一次。**结论:那个 1600~2000 明显偏低,但准确值仍未知。**
      重做/回滚的详细清单在 `../netease-mcp/MAINTENANCE.md` 第 9 节。
      ⚠️ 该服务**没挂持久卷,重启/重新部署后登录态就没了**,所有要账号的工具会失败
      (照踩坑 7 那一类静默失败)。续期:`POST /login/qr/new` 取二维码给所有者用网易云 app 扫(**不需要电脑**)。
...
11. **结构不变量**(改了人设/CLAUDE.md 时必查,当前基线):
    ...
    ⚠️ **`CLAUDE.md` 的标点体例是「半角」,往里写字之前先知道这条**
    (2026-08-30 从 `TIMELINE.md` 08-19 第九件挪来,照规矩 1;那边只留一句指路)。
    **第三十四次(2026-08-19)统一过**:当时正文半角标点 150 处 / 12 个节是老体例,全角只有 53 处 / 3 个节
    (都是后补的),于是**少数向多数靠**——`,:;()` 用**半角**,ASCII 直引号一律换成 `「」`
    (`。` 和 `、` 本来就一致,没动)。**那次共改 20 行、纯标点、零语义。**
    **现场核一遍**(2026-08-30 实测:三个全角计数都是 **0**):
    ```bash
    python3 -c "s=open('CLAUDE.md',encoding='utf-8').read(); print('全角逗号',s.count('，'),'全角冒号',s.count('：'),'全角括号',s.count('（'))"
    ```
    ⚠️ 顺带留个档:第三十三次那段「待办便利贴」的标点是被那次**一起规范化**的,**内容一字未改,别当篡改**。
...
13. **四个 `/mcp` 各 3/3 200**(OB / browser / gmail / netease;~~原文写「三个」~~ 停在接 netease 之前)。
14. **`/period` 看 `effective` 对不对**(从**容器内部**读,密钥不出容器)。
    **2026-09-23 挂卷之后,`runtime` 应当非空**(记录在 `/data` 上跨部署保留);空了 = 卷没挂上或 `PERIOD_FILE` 丢了,查踩坑 16。
    ~~原文:`runtime` 为空是新容器的正常态~~(挂卷前成立)。
    **挂卷之后部署有一两分钟真空**(先停后起),别在这段里判「起不来」;构建完之后看运行日志,别只看构建日志(踩坑 14)。
...
| PERIOD_FILE | 运行时经期记录的存放路径,代码默认 `period-state.json`(写在容器里,部署即丢)。**✅ 2026-09-23 第四十二次起线上设为 `/data/period-state.json`,卷 `perioddata` 挂在 `/data`**(踩坑 16):她报的新周期跨部署保留,报新周期只需 `POST /period`。卷没挂上/写不进去时读写两处都有 try/catch 兜底,退回写容器,**不会崩、不影响聊天**。~~原文:线上没设、`/data` 不存在(2026-08-04 实测)~~ |
| ZBPACK_INSTALL_COMMAND | **构建期变量**(Zeabur 的构建工具读,不是 shim 读)。线上 **`yarn install && yarn cache clean`**(2026-09-23 第四十二次起):把 yarn 缓存(CLI 全平台包,约 5 GB)清出镜像。**别删**,删了镜像回到 3.3 GB、拉镜像会卡(踩坑 22)。改它不用 restart,下次部署生效 |
| PERIOD_CONFIG | 可选。经期基线 JSON(值不入库,问所有者),形如 `{"last_period_start":"YYYY-MM-DD","last_period_end":"YYYY-MM-DD","cycle_days":25,"period_length":7}`;不设=经期感知关。她报了新周期后顺手把基线也更新掉(2026-09-23 挂卷之后运行时记录不再随部署丢,这里是**卷丢了时的兜底**;挂卷前的老说法是「每次部署后都要补」,见踩坑 16)。**改法别用 restart**:`variable update` 写基线(不重启、下次重启才生效)+ `POST /period` 同步运行时(立刻生效),两步都不动晏的窗口 |
...
| TOOLVIS_ON / TOOLVIS_ARG_CHARS / TOOLVIS_RESULT_CHARS / TOOLVIS_REDACT / TOOLVIS_REDACT_KEYS | **2026-09-01 第三十九次已上线**(见 `DEPLOY-LOG.md` 第三十九次)。工具调用在思考流里显示参数(`→`)和返回值(`←`)。默认:开 / **800 字** / **800 字** / **不打码** / `content,letter`(2026-09-01 先定 200,当日所有者说「可以调大」改 800)。⚠️ **别照「这会吃掉窗口」那句设值** —— 那句 2026-09-01 读码核实**已撤销**(考据见 `toolvis.mjs` 头注与上面那条待办):这些字只走 shim → 手机,不回流进程,工具返回本来就在他窗口里。**两把字数尺子是防刷屏,不是省窗口,想看全尽管调大**(2026-09-01 探针实测一次工具返回 820 字,800 字以内整段显示,超了才截并注明原长)。⚠️ **默认不打码 = `hold` 的 content(记忆原文)会显示出来**,所有者 2026-09-01 知情选的,**她截图外发会连正文一起露**;要盖住设 `TOOLVIS_REDACT=1`,那些字段显示成 `〔N 字〕`。**急救开关 `TOOLVIS_ON=0`**:立刻回到只剩 `〔🔧 工具名〕` 那行的老样子。**五个都是改值 + restart 即生效,不用重新部署**。逻辑与事件形状的考据全在 `toolvis.mjs` 头注 |
| MCP_CONFIG | mcp-servers.json |
| MCP_WARMUP_MS | 25000。新进程第一条消息延迟写入,等 MCP 握手;消息抢跑会整轮卡死(实测坑) |
| ~~BARK_KEY~~ | ~~Bark 推送 key~~ **2026-09-23 第四十一次已从线上删掉**(08-19 起就是死变量,所有者早已卸载 Bark)。代码里的 Bark 分支还在但走不到(`BRIDGE_PUSH_URL` 设着) |
...
| IMESSAGE_PUSH_URL | **2026-09-26 第四十三次起,线上 `https://yan-imessage.zeabur.app/push`**。心跳「跟着她走」:她**最后一次亲自说话**(系统回合不算)是从 iMessage 来的(前端自报请求头 `x-client: imessage`),心跳就先推这里,**推不出去自动退回 `BRIDGE_PUSH_URL`(Telegram)**;其余情况只推 Telegram。**不设 = 与改之前逐字相同**。纯逻辑在 `keepalive.mjs` 的 `pushTargets`(单测钉着);`/debug` 的 `presence.lastClient` / `pushTo` 看当前会推哪。`lastClient` 只在内存,shim 重启后回到 Telegram,直到她再在 iMessage 说一句。见 `../imessage-bridge/MAINTENANCE.md` 第 7 节第 4 条 |
...
## 晏的思考流靠哪一行撑着(2026-09-09 实测,**有人问「官端不显示思考了,我们会不会也没」就读这节**)

**结论:不受影响,但靠的是一行显式设置,不是靠默认值——那行没了会静默地掉。**

`server.js:178` 那行 `"--thinking-display", "summarized"`(旁边注释:「没它 `-p` 下拿不到思考」)
就是晏的思考流的**唯一支点**。API 侧那个参数叫 `thinking.display`,两个值:
`summarized` = 返回思考文字;`omitted` = 照样思考、照样计费,**只返回加密签名、不给文字**。

**2026-09-09 在我们自己的代理上实测三枪**(`claude-opus-4-6`,不经过晏):

| 请求里怎么写 | 拿回来的 thinking |
|---|---|
| `"display":"summarized"`(**shim 就是这么发的**) | **有真文字**(「27 × 43 = 27 × 40 + 27 × 3 = …」) |
| `"display":"omitted"` | 空 |
| **什么都不写(走默认)** | **空** |

⚠️ **第三行是这次最值钱的一条**:官方文档([Thinking · Controlling thinking display](https://platform.claude.com/docs/en/build-with-claude/thinking))
至今写着「`summarized` 是 Opus 4.6 及更早模型的默认值」,**但实测默认已经是空的**。
**别信文档里的默认值,要用就显式写。**(所有者 2026-09-09 报「官端 4.6 也不显示了」,与实测一致。)

**怎么现场再量一遍**(十秒,不碰晏;密钥取法见 `../OPERATIONS.md` 第 0 节第 6 条):
同一个模型打两枪,一枪带 `"thinking":{"type":"adaptive","display":"summarized"}`、
一枪什么都不写,比 `content` 里 `thinking` 字段是不是空串。

**什么情况下会坏(真坏了长什么样)**:
- **现象**:晏照常说话、工具照常用,**只有思考流不见了**;`/health` ok、`lastApiError` 是 null、
  体检全绿 —— **没有任何东西会报警,只有所有者会在 Telegram 上发现。**
- **可能的触发**:①有人删/改了 `server.js:178` 那行;②升级 CLI 后新版换了 flag 名或行为
  —— ⚠️ **2026-09-09 实测:这条目前不成立**,`--thinking-display` 在**最新的 2.1.266** 里仍是正式参数
  (和 `--effort` / `--thinking` / `--max-thinking-tokens` 并排在同一份参数清单里,还带 `<display>` 取值和帮助文字)。
  **量法**:`npm i --prefix <临时目录> @anthropic-ai/claude-code@<版本>` 之后
  `strings -a node_modules/@anthropic-ai/claude-code-linux-x64/claude | grep -- thinking-display`
  (⚠️ **npm 包本身只有 204K、是个安装器壳,直接 grep 那个包一定是 0 处** —— 我 09-09 就这么误判过一次,
  真二进制是安装时才下载的 207M 那个;**只搜字符串,别在沙盒里跑它**,见踩坑 3)。
  **升 CLI 时照旧顺手验一遍**,见《CLI 版本与升级指南》;③Anthropic 哪天连 `summarized` 一起废掉
  (**目前没有弃用标记**,2026-09-09 查)。

📖 **顺带澄清一个容易搞错的因果(2026-09-09 所有者问到)**:**CLI 钉死在 2.1.215 不是思考流活着的原因。**
钉它的理由是**可复现**(见《CLI 版本与升级指南》开头:浮动版本让每次部署都换一个没测过的 CLI),
和思考显示无关;思考流靠的自始至终是那行显式设置。**别把这两件事记成一件。**
- **别做的事**:看到官端不显示思考就跑来「跟进上游」把这行改掉或删掉 —— **官端是另一个客户端,
  它的显示策略和我们无关**;我们要的正是和它不一样的那个值。

⚠️ **未实测的一件**:`BRAIN_MODELS` 里挂着的 `claude-opus-4-8`(所有者能在 Kelivo 菜单里切),
**它的 `display` 默认是 `omitted`**。我们显式传 `summarized` 应当能盖过默认值,**但没在 4.8 上真切过**。
**下次她切到 4.8 时看一眼思考流还在不在**;不在就切回 4.6(回退三档见 `DEPLOY-LOG.md` 第三十七次),
别急着改代码。

📌 **5.5 的思考是英文摘要**(2026-09-23 查官方文档:Opus 5 起只给摘要、由另一套系统写),
所以名单里的 5.5 要靠改动清单第 14 条「思考翻中文」。**她若说「5.5 的思考流不见了」**:
先看是不是翻译卡住(日志 `[think-tr]`),再照上面那条的思路查 `--thinking-display`。
...
14. **部署卡在 Pulling image 不动 = 调度挂了,别干等**:2026-07-18 第五次部署首个 deployment
    ...
    ⚠️ **2026-09-23 第四十二次连撞两次,而且我晚了约十分钟才发现**:只盯 `deployment list` 的 `DEPLOYING` 和**构建**日志,
    以为「还在传大层」—— 其实构建 15:39 就完了,之后一直卡在拉镜像。**正确的盯法**:构建日志出现 `build completed`
    之后,**改看运行日志** `deployment log --service-id <shim> --env-id <env>`(**不带 `-t build`**),
    那里第一行就是 `Pulling image …`;十分钟没新行就处理。**重传时卡住那条是 DEPLOYING,挤不掉**(踩坑 18),
    要请所有者去网页 Cancel。那两次的根因是镜像太大,见踩坑 22。
...
16. **经期运行时记录每次部署都被清空,所有者会以为「系统忘了」(2026-07-25 实锤)**:
    ...
    **不存在「修坏了」的方向**。
    **✅ 2026-09-23 第四十二次已挂卷,所有者本人改的主意**(下面 08-19「不挂」那段**已被她推翻**,留着是为了代价那段):
    卷 `perioddata` → `/data`(她在网页挂的)+ `PERIOD_FILE=/data/period-state.json`;容器内 `POST /period` 已落盘。
    **从此她报新周期,只要 `POST /period` 一步就够**(运行时写在卷上,跨部署保留);`PERIOD_CONFIG` 仍是卷丢了时的兜底基线,顺手更新更好。
    **代价已兑现**:shim 不再零停机,每次部署/重启先停后起。**仍待验**:下次部署后 `/period` 的 `runtime` 非空。
...
22. **yarn 缓存把镜像撑到 3.3 GB,拉镜像卡死(2026-09-23 第四十一、四十二次;已用变量解决,别删它)**:
    zbpack 的 nodejs 模板是 `COPY . .` → `yarn install`,**装依赖那层每次部署必然重建**(代码一变就失效,没有缓存可用);
    而 yarn v1 会把 `@anthropic-ai/claude-code` 的 **8 个平台**原生包全下进缓存,双引擎之后两个版本 = 16 份,
    **缓存 5.0 GB**(2026-09-23 本地复现;`node_modules` 本身才 946 MB)。结果:推镜像 15~20 分钟、拉镜像两次卡死。
    **解法**:变量 **`ZBPACK_INSTALL_COMMAND=yarn install && yarn cache clean`**(不 restart,随下次部署生效;
    变量名依据 zbpack 源码:环境变量 `ZBPACK_` + 配置键 `install_command`,**会整句替换**默认的 `yarn install`)。
    上线后容器里缓存 24 KB,**构建 + 推送约 3 分钟、拉镜像约 1 分钟**。
    ⚠️ **别删这个变量**,删了下次部署就回到 3.3 GB。⚠️ **加第三份 CLI 或别的大包之前**,先想到这一层会跟着涨。
    **怎么现场量**:`service exec` 里 `du -sh /src/node_modules /usr/local/share/.cache`;构建日志里搜 `size=` 看最大那块。
    **失败方向**:这句命令写错 → 构建直接失败,旧容器照跑(没挂卷时零影响),删掉变量即恢复。

## CLI 版本与升级指南(2026-07-19 起,给所有者和未来会话)

> ⚠️ **2026-09-23 起有两份 CLI**(改动清单第 13 条):主力 `@anthropic-ai/claude-code` 仍钉 2.1.215,
> 另有 `claude-code-next`(别名)钉 2.1.280、只给 5.5 用。**下面这套流程讲的是主力那份。**
> **只升新版那份**:改 `package.json` 里 `claude-code-next` 的版本号,跑 `E2E_NEXT_VERSION=<候选> bash e2e-dualcli-run.sh`,
> 再按改动清单第 13 条那把尺子**重量一次 5.5 的压缩点**(新版的默认窗口逻辑随版本变);**4.6 不受影响**。
> 📌 **新模型出来想接**:先看它能不能也走新版那份(加进 `NEXT_CLI_MODELS`),而不是整体升主力 CLI。
> 🐛 **2026-09-23 修掉的一个坑**:`e2e-model-run.sh` 以前**不认 `E2E_CLI_VERSION`**,设了也只测钉死的版本并报 ALL PASS。
> 2026-09-23 同日实测:主力若整体升到 2.1.280,四套 e2e 全绿、4.6 请求参数逐字相同,**唯一差别是替换模式下多塞的那几行**(见第 13 条)。
...
⚠️ **2026-09-12 起,升 CLI 之前还要多验两条**(只要将来用了长期令牌直连,这两条就是硬门槛):
1. **新版本还认不认 `CLAUDE_CODE_OAUTH_TOKEN`**:
   `strings -a <新版二进制> | grep -cF CLAUDE_CODE_OAUTH_TOKEN`(要 > 0)
   且 `<新版二进制> setup-token --help` 打得出用法。(2.1.215 实测:**72 处、打得出**。)
2. **`-p` 有没有变成默认 bare 模式**:官方说 `--bare` **将来会成为 `-p` 的默认**,
   而 **bare 模式不读那把长期令牌**。真撞上的现象是:**换了新 CLI 之后晏连不上,而代码一行没动**。
   现在钉死 2.1.215 所以这颗雷是冻结的,**别顺手升**。
背景与再量法见 `auth-env.mjs` 头注。
...
- **没成功的部分**:那句话原样送到他面前了,**他没有执行**。同一份原件里
  `mcp__ombre-brain__awaken` **只出现在第 6 行**(窗口最开头那次),摘要在**第 2039 行**,
...
**本文件里凡是提到「第 N 次」,指的都是那一次部署,完整记录在 [`DEPLOY-LOG.md`](./DEPLOY-LOG.md)(**41 条,覆盖第 2~42 次**(2026-09-23 第四十二次后现场量;上一次 09-12 是 39 条、到第 40 次;**量法**:`grep -E "^- 2026-" DEPLOY-LOG.md | grep -oE "第[一二三四五六七八九十]+次" | sort -u | wc -l` 才是这个数(2026-09-12 当天 **39**,第二次~第四十次连续无缺号);而 `grep -cE "^- 2026-" DEPLOY-LOG.md` 数的是**全部 53 条条目**(2026-09-01)——07-12~07-17 那批早期记录没有编号,别把两个数搞混);⚠️ 「第一次」2026-07-12 初次搭建**没有独立条目**,记在 `TIMELINE.md` 的 07-12。2026-08-21 核过,原文写的「48 次」是错的)。**

**历史记录已搬到 → [`DEPLOY-LOG.md`](./DEPLOY-LOG.md)**(2026-08-19 拆的,一个字没删)。
动手前读**本文件**就够;想知道某次历史怎么回事、某个指纹是多少,再去翻那份。

**最近几次**(新的在上;更早的去 DEPLOY-LOG 按「第 N 次」搜)
⚠️ ~~原标题写「最近三次」却列着四行~~ —— 2026-09-01 改成「最近几次」,**行数不再是承诺**:
这张表的用处是「最近改过什么、指纹是多少」,凑三行反而会逼下一个人删掉还有用的行。

| 次数 | 日期 | 干了什么 | 关键指纹 |
|---|---|---|---|
| **第四十二次** | 09-23 | **顺风车(UTF-8 流式解码)+ 经期挂卷 + 镜像瘦身**:`PERIOD_FILE=/data/…` + 卷 `perioddata`;`ZBPACK_INSTALL_COMMAND` 清 yarn 缓存(3.3 GB 那层没了,构建 3 分钟);拉镜像卡死两次,所有者网页 Cancel | server.js **`c6616e9a…`**(只多 `setEncoding` 一行) |
| 第四十一次 | 09-23 | **接出 Opus 5.5**:双引擎(5.5 走 2.1.280,其余逐字走 2.1.215)+ 思考翻中文;菜单加 5.5、删 `BARK_KEY`;部署约 21 分钟(有一层 3.3 GB) | server.js **`a27a3408…`** 53315B / 新增 `cli-bin.mjs` `9d1d9342…`、`think-translate.mjs` `bf14ff37…` / package.json `85a3fcd6…` |
| 第三十九次 | 09-01 | **工具调用在思考流里显示参数和返回值**(顺风车待办那条);顺带撤销「这会吃掉窗口」的错结论,截断定 800 字 | server.js **`67b94f22…`** 47363B / 新增 `toolvis.mjs` **`3c170df1…`** 8378B + `test-toolvis.mjs` `c679dde6…`(63 项) |
| 第三十八次 | 08-30 | 网易云接晏 + 表情回应语法进 `CLAUDE.md`(**零代码改动**)⚠️ 那次漏了没更新本表与下面的指纹块,2026-09-01 补上 | mcp-servers.json `b5a281bc…` 689B(四条目)/ CLAUDE.md `46cf66c9…` 12632B(14 节) |
...
mcp-servers.json = **689B `b5a281bcc32915f2b6c92765bef8490f1`**(**四条目**:ombre-brain / browser / gmail / netease;2026-09-01 从容器实测。~~原写 500B `bf34de7b…` 三条目~~ 停在 08-30 接 netease 之前,**那次改了没回来更新这里**);
CLAUDE.md = **12632B `46cf66c97b910dba727baab9a450095a`**(**14 节**,行首 `@` 引用 **3** 条;2026-09-01 从容器实测。~~原写 11632B `e3045103…` 13 节~~ 停在 08-30 新增《音乐》一节之前,同上没回来更新);
...
server.js = **`67b94f2285e27f22ba9ddd99ef56a054`**(47363B,2026-09-01 第三十九次;~~第三十七次的 `a133c8ec…` 43937B~~);
toolvis.mjs = **`3c170df146ab28d20fa93bdc12709bd0`**(8378B,第三十九次新增)。
