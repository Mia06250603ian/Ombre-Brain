...
- 2026-07-20 **守卫职责重定义:只提醒存 OB、永不换窗(改动清单 7 第三次改版+改动清单 6 注)。
  已改码、单测 88+53+52 全绿、e2e(真 2.1.215 二进制+假后端,含压缩循环剧本)全绿,
  ⚠️ 尚未部署**——部署时走全套流程(md5 对账、拷 ian.md/mcp-servers.json、三 /mcp 验 200、
  所有者归档+授权),并注意:① CLAUDE.md 本次有改动(上下文管理节),别拿容器旧版覆盖;
  ② 新环境变量 CTX_ARCHIVE_EVERY_TOKENS/CTX_OBSERVE 有代码默认值,Zeabur 不加也能跑;
  ③ 所有者未拍板是否先开观察模式(CTX_OBSERVE=1),部署前问一句。部署后把本条改成正式记录。
