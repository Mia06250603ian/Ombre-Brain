...
- **必须全文读的只有一节:「踩过的坑(别再踩)」(第 844 行,约 8800 token)。**
...
| 70 | 当前 server.js 相对 7-12 初版的改动 | 10k | **改代码前读**。十四条改动清单:1 进程误杀补丁 / 2 标题拦截 / 3 定性锚点 / 4 时间注入 / 5 感官(天气+经期) / 6 保温+主动唤醒 / 7 上下文守卫 / 8 人设拆分 / 9 上游报错不再吃成空回复 / 10 系统提示词 append↔replace / 11 模型可切(2026-08-24) / 12 上游选路:代理 or 直连(2026-09-12 已上线) / **13 双引擎(5.5 走新版 CLI) / 14 思考翻中文(均 2026-09-23,代码在、未部署)**。**只读你要动的那一条即可** |
...
| 686 | └ 部署 | 0.3k | 〃 |
| 696 | └ 部署后(踩坑 9:上传成功 ≠ 上线) | 1.0k | 〃 **最容易被跳的一节,别跳** |
| 729 | └ 回滚的通用规矩 | 0.5k | 要回滚时读。核心一句:回滚不是零代价,等于再丢晏一个窗口 |
| 742 | └ 记录该写什么 | 0.2k | 部署完写 DEPLOY-LOG 时读 |
| 749 | 环境变量(值不入库;改值后要 restart) | 4.0k | **要调任何旋钮之前读**。链路/人格/感官/主动性/上下文守卫五组;**`BRAIN_MODELS`(模型菜单,不设=休眠)在这节**,另有一条 2026-08-24 实测的警告:**终线会被「发图那一轮」整个跳过**;**2026-09-01 起还有五个 `TOOLVIS_*`**(工具可见化,急救开关 `TOOLVIS_ON=0`);**2026-09-23 起还有 `NEXT_CLI_*`(双引擎)和 `THINK_TRANSLATE_*`(思考翻中文,急救开关设空)** |
| 792 | **晏的思考流靠哪一行撑着(2026-09-09)** | 0.7k | **有人问「官端不显示思考了,我们会不会也没」就读这节**;**升 CLI 之前也读**。晏的思考流的唯一支点是 `server.js:178` 那行 `--thinking-display summarized`;⚠️ **实测默认值已经是「不返回思考文字」**(和官方文档写的不一样),所以必须显式传。真坏了的现象:他照常说话、只有思考流不见了,**全线不报警** |
| 844 | **踩过的坑(别再踩)** | 8.8k | **⚠️ 唯一必须全文读的一节。** 21 条:1 消息抢跑 MCP 握手 / 2 upload 丢 dotfiles / 3 沙盒里测 claude 会卡死 / 4 订阅 OAuth 登录 / 5 令牌只能一处跑 / 6 Kelivo 开关注入 system / 7 OB 域名失效致 MCP 静默失败 / 8 自动标题也是注入源 / 9 deploy success ≠ 上线 / 10 连发 deploy 前一次被取消 / 11 别人的会话把版本滚回去过 / 12 她问问题≠授权动手 / 13 代发「归档」要慎用 / 14 卡在 Pulling 是调度挂了 / 15 .gitignore 让上传静默丢文件 / 16 经期记录每次部署被清空 / 17 deploy 传的是当前工作目录 / 18 进 DEPLOYING 就挤不掉了 / 19 --settings 指向不存在的文件会拒绝启动 / 20 新 import 要同步改 e2e 的 cp 清单 / **21 有些会话拷不出容器文件、检查单第 4 步会卡死(2026-08-30)** |
| 1061 | CLI 版本与升级指南 | 1.0k | 要动 CLI 版本时读(现钉死 2.1.215) |
| 1108 | ~~待办~~ 「她在干嘛」改写 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第二十四次」 |
| 1114 | 她在干嘛(如果开了) | 0.5k | 动查岗功能时读 |
| 1130 | ~~待办~~ 表情包补螃蟹 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第二十九次」 |
| 1136 | 表情包(如果接了 Telegram) | 0.8k | 动贴纸时读 |
| 1159 | ~~待办~~ 「待办便利贴」 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第三十三次」 |
| 1165 | ~~待办~~ 「记错了 / 过期了」 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第三十四次」 |
| 1171 | 建议(未做) | 1.5k | 想找「还能改什么」时读 |
| 1219 | **2026-08-21 心跳「自主活动」+ 压缩点实测** | 1.2k | **改心跳文案 / 动上下文阈值前读**。心跳纸条的**四条机械约束**在这节(`【。】` 判不出沉默那条是硬伤);压缩点**实测 166942**,并撤销了「改人设要跟着下调阈值」的旧推算 |
| 1252 | 2026-08-10 第三十一次的机制细节 | 0.2k | 下面四小节的引子 |
| 1259 | └ ① 161500 硬线永久静音 | 0.6k | 动上下文守卫阈值前读 |
| 1280 | └ ② 终线原话桶顺序错乱 | 0.2k | 〃 |
| 1291 | └ ③ 压缩摘要压不住九节 | 2.6k | 〃 **⚠️ 结论已更新(2026-08-24)**:纸条早已上线,第一次真压缩**成功**(九节命中 0、纸条那句命中 1)——**但晏没照做,压缩后没 awaken**。所有者说不修。问「为什么压缩后他不 awaken」先读这节 |
| 1362 | └ ④ awaken 的全文位把日记桶挤掉了 | 0.6k | ⚠️ **这节讲的是 OB,不是 shim** —— 改 OB 的 `awaken` 之前必读。**现在的结论是「OB 不用改」**;它里面那个「必然重演、要按类型各保一条」的旧结论**已被所有者当场推翻并撤销,别照它改**。另外它留了一把量「awaken 会不会把开机撑爆」的尺子(2026-08-10:占 27%;2026-08-21 钉选改出全文后:28%),往 awaken 加东西前拿它量一遍 |
| 1382 | 部署记录 | 0.8k | 只是指路:完整记录在 `DEPLOY-LOG.md`(**39 条,覆盖第 2~40 次**,2026-09-12 现场量;第一次记在 `TIMELINE.md` 07-12),按「第 N 次」搜 |
...
13. **双引擎:5.5 走第二份新版 CLI,其余模型逐字走 2.1.215**(2026-09-23,**代码写完、全套测试过,尚未部署**;
...
14. **思考翻中文**(2026-09-23,**同上,未部署**;借朋友 7efb2a9 / f766260,改了三处)。
...
> **现在本小节是空的**(2026-09-12 挂上的那条当天就做掉了)。
...
| BRAIN_MODELS | **2026-08-24 起(方案 B)**。逗号分隔的模型名单,`/v1/models` 把它吐给 Kelivo 当菜单,她在手机上点一下就换模型。**不设 = 名单里只有 `BRAIN_MODEL` 一个 = 功能休眠,行为与本次改动前逐字相同**。**急救开关:清掉这个变量 + `service restart` 立刻回到原行为,不用回滚部署。** ⚠️ 换模型**必然重开进程 = 丢晏一个窗口**(模型在出生时用 `--model` 钉死),不是 bug。⚠️ **只许放窗口大小相同的模型**(4.5/4.6/4.8 压缩点都是 167000);~~**Opus 5 别放**,CLI 2.1.215 不认识它。~~ **2026-09-23 起 5.5 可以放,但只能经双引擎**(下面 `NEXT_CLI_MODELS` 那行 + 改动清单第 13 条),新版 CLI 不在时它会被自动拿出菜单。**线上现值(2026-08-24)**:`claude-opus-4-6 claude-opus-4-5-20251101 claude-opus-4-8`(**空格分隔**——`variable create` 的 `-k` 拿逗号当分隔符,别用逗号,见 DEPLOY-LOG 第三十七次那条新坑)。**回退照 DEPLOY-LOG 第三十七次的《回退三档》**。详见改动清单第 11 条与 `../docs/多模型接出方案.md` |
| NEXT_CLI_MODELS / NEXT_CLI_WINDOW / CLAUDE_BIN_NEXT | **2026-09-23 双引擎,未部署**(改动清单第 13 条)。`NEXT_CLI_MODELS` = 点名走新版 CLI 的模型,不设默认 `claude-opus-5-5`;**名单里有 5.5 时别把它清空**(= 让 2.1.215 跑 5.5 = 空回)。`NEXT_CLI_WINDOW` = 新版那份的窗口上限,不设 = 200000(实测压缩点与 4.6 逐位相同,三条线不动);`0` = 不限(「大桌子」,⚠️ 那时三条线必须按模型分,否则 15.5 万就催归档);写错回落默认。`CLAUDE_BIN_NEXT` **由 `entrypoint.sh` 自动导出**(新版真能跑才导出),**别在 Zeabur 手动设**。三个都是改值 + restart 即生效 |
| THINK_TRANSLATE_MODELS / THINK_TRANSLATE_MODEL / THINK_TRANSLATE_TIMEOUT_MS / THINK_TRANSLATE_CONCURRENCY | **2026-09-23 思考翻中文,未部署**(改动清单第 14 条)。名单不设默认 `claude-opus-5-5`;**急救开关:设成空串 + restart = 立刻不翻**。翻译模型默认 `claude-sonnet-4-6`(走主力 CLI);超时默认 20000 毫秒,**正文会等翻译**,嫌慢就调小(超时发英文原文,不丢)。`THINK_TRANSLATE_CONCURRENCY` 默认 1(一个翻译进程约 300 MB,整机可用 1.2~1.5 GB,**别随手调大**)。⚠️ **别设全局 `MAX_THINKING_TOKENS`**,那个只加在翻译子进程上 |
