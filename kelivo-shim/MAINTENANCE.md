...
| 545 | └ 部署 | 0.3k | 〃 |
| 555 | └ 部署后(踩坑 9:上传成功 ≠ 上线) | 1.0k | 〃 **最容易被跳的一节,别跳** |
| 588 | └ 回滚的通用规矩 | 0.5k | 要回滚时读。核心一句:回滚不是零代价,等于再丢晏一个窗口 |
| 601 | └ 记录该写什么 | 0.2k | 部署完写 DEPLOY-LOG 时读 |
| 608 | 环境变量(值不入库;改值后要 restart) | 4.0k | **要调任何旋钮之前读**。链路/人格/感官/主动性/上下文守卫五组;**`BRAIN_MODELS`(模型菜单,不设=休眠)在这节**,另有一条 2026-08-24 实测的警告:**终线会被「发图那一轮」整个跳过** |
| 647 | **踩过的坑(别再踩)** | 8.8k | **⚠️ 唯一必须全文读的一节。** 21 条:1 消息抢跑 MCP 握手 / 2 upload 丢 dotfiles / 3 沙盒里测 claude 会卡死 / 4 订阅 OAuth 登录 / 5 令牌只能一处跑 / 6 Kelivo 开关注入 system / 7 OB 域名失效致 MCP 静默失败 / 8 自动标题也是注入源 / 9 deploy success ≠ 上线 / 10 连发 deploy 前一次被取消 / 11 别人的会话把版本滚回去过 / 12 她问问题≠授权动手 / 13 代发「归档」要慎用 / 14 卡在 Pulling 是调度挂了 / 15 .gitignore 让上传静默丢文件 / 16 经期记录每次部署被清空 / 17 deploy 传的是当前工作目录 / 18 进 DEPLOYING 就挤不掉了 / 19 --settings 指向不存在的文件会拒绝启动 / 20 新 import 要同步改 e2e 的 cp 清单 / **21 有些会话拷不出容器文件、检查单第 4 步会卡死(2026-08-30)** |
| 864 | CLI 版本与升级指南 | 1.0k | 要动 CLI 版本时读(现钉死 2.1.215) |
| 894 | ~~待办~~ 「她在干嘛」改写 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第二十四次」 |
| 900 | 她在干嘛(如果开了) | 0.5k | 动查岗功能时读 |
| 916 | ~~待办~~ 表情包补螃蟹 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第二十九次」 |
| 922 | 表情包(如果接了 Telegram) | 0.8k | 动贴纸时读 |
| 945 | ~~待办~~ 「待办便利贴」 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第三十三次」 |
| 951 | ~~待办~~ 「记错了 / 过期了」 | 0.1k | **正文已挪进 `DEPLOY-LOG.md`**(2026-08-29,照规矩 1)。原地只剩标题+指路;要看原文在 DEPLOY-LOG 搜「第三十四次」 |
| 957 | 建议(未做) | 1.5k | 想找「还能改什么」时读 |
| 1005 | **2026-08-21 心跳「自主活动」+ 压缩点实测** | 1.2k | **改心跳文案 / 动上下文阈值前读**。心跳纸条的**四条机械约束**在这节(`【。】` 判不出沉默那条是硬伤);压缩点**实测 166942**,并撤销了「改人设要跟着下调阈值」的旧推算 |
| 1038 | 2026-08-10 第三十一次的机制细节 | 0.2k | 下面四小节的引子 |
| 1045 | └ ① 161500 硬线永久静音 | 0.6k | 动上下文守卫阈值前读 |
| 1066 | └ ② 终线原话桶顺序错乱 | 0.2k | 〃 |
| 1077 | └ ③ 压缩摘要压不住九节 | 2.6k | 〃 **⚠️ 结论已更新(2026-08-24)**:纸条早已上线,第一次真压缩**成功**(九节命中 0、纸条那句命中 1)——**但晏没照做,压缩后没 awaken**。所有者说不修。问「为什么压缩后他不 awaken」先读这节 |
| 1148 | └ ④ awaken 的全文位把日记桶挤掉了 | 0.6k | ⚠️ **这节讲的是 OB,不是 shim** —— 改 OB 的 `awaken` 之前必读。**现在的结论是「OB 不用改」**;它里面那个「必然重演、要按类型各保一条」的旧结论**已被所有者当场推翻并撤销,别照它改**。另外它留了一把量「awaken 会不会把开机撑爆」的尺子(2026-08-10:占 27%;2026-08-21 钉选改出全文后:28%),往 awaken 加东西前拿它量一遍 |
| 1168 | 部署记录 | 0.8k | 只是指路:完整记录在 `DEPLOY-LOG.md`(**37 条,覆盖第 2~38 次**,2026-08-30 现场量;第一次记在 `TIMELINE.md` 07-12),按「第 N 次」搜 |
