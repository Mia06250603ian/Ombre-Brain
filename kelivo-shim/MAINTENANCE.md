...
## 待办:CLAUDE.md「记忆工具使用」节加一句「记错了 / 过期了」(2026-08-19 议定,**所有者说明天再弄,待下次部署 shim 时顺手做**)
...
**⚠️ 2026-08-19(晚)状态:成品已按体例落进仓库的 `kelivo-shim/CLAUDE.md`,但【尚未部署】。**
仓库那份现在是 **11340B md5 `6cf1b2052f52f5bcb0917d362da63f5c`**(仍 13 节),
线上容器仍是第三十三次的 **10850B `86ee28f0…`** —— **下一次部署 shim 时它会随构建一起上线**,
不用再改一遍;真要撤销就把这一段删掉(git 里有)。四套单测改动前后各跑一遍,均 **131/53/52/56 全绿**。
**一处体例改动,照第三十三次「待办便利贴」那次的先例报备**:下面这份成品用的是半角逗号 + 反引号,
而 CLAUDE.md「记忆工具使用」节的四段(含它紧邻的「待办便利贴」)**通篇是全角标点、工具调用不加反引号**
(实测:该节 13/15/17/19 四行全角逗号 3/3/2/4 处、半角 0 处;反引号只有第 13 行的 `[seal:…]` 用了)。
落进去的是**跟邻居一致的全角版**,机械约束(`deny` / `undeny` / `expires_at` / `YYYY-MM-DD` /
「一次只能改一个桶」)一字未动。想要逐字照抄的半角版也行,说一声改回去即可。
**前提已复验(2026-08-19 晚)**:线上 OB 的 `trace` 工具定义里 `deny` / `undeny` / `expires_at` /
`include_dormant` 四个参数**确实全在**(直接跟 `/mcp` 握手拉 `tools/list` 验的),
而改动前的 CLAUDE.md 里 `deny` / `expires_at` **0 处** —— 印证「他调得动、只是不知道何时该用」。
