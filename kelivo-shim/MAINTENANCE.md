...
- [ ] **工具调用在 thinking 里显示参数和返回值**(2026-08-31 挂上;所有者当天提的)
      ...
      **所有者当天定的两件**(她原话「不太懂」,故按建议默认值走,并做成可改的旋钮):
      ①**截 200 字**;②**参数照常显示、不打码** —— ⚠️ 这意味着 `hold` 的 content(记忆原文)
...
| TOOLVIS_ON / TOOLVIS_ARG_CHARS / TOOLVIS_RESULT_CHARS / TOOLVIS_REDACT / TOOLVIS_REDACT_KEYS | **2026-09-01 起(代码已写、尚未部署,见上面《搭顺风车的待办》那条)**。工具调用在思考流里显示参数(`→`)和返回值(`←`)。默认:开 / **200 字** / **200 字** / **不打码** / `content,letter`。⚠️ **别照「这会吃掉窗口」那句设值** —— 那句 2026-09-01 读码核实**已撤销**(考据见 `toolvis.mjs` 头注与上面那条待办):这些字只走 shim → 手机,不回流进程,工具返回本来就在他窗口里。**两把字数尺子是防刷屏,不是省窗口,想看全尽管调大**(2026-09-01 探针实测一次工具返回 820 字,默认截到 200 + 「(共 820 字)」)。⚠️ **默认不打码 = `hold` 的 content(记忆原文)会显示出来**,所有者 2026-09-01 知情选的,**她截图外发会连正文一起露**;要盖住设 `TOOLVIS_REDACT=1`,那些字段显示成 `〔N 字〕`。**急救开关 `TOOLVIS_ON=0`**:立刻回到只剩 `〔🔧 工具名〕` 那行的老样子。**五个都是改值 + restart 即生效,不用重新部署**。逻辑与事件形状的考据全在 `toolvis.mjs` 头注 |
