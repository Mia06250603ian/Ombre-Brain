...
| 1076 | 部署记录 | 0.8k | 只是指路:完整记录在 `DEPLOY-LOG.md`(35 条,覆盖第 2~36 次;第一次记在 `TIMELINE.md` 07-12),按「第 N 次」搜 |
...
| CTX_ARCHIVE_EVERY_TOKENS | 2026-07-20 起。硬线首归后,窗口每再涨这么多 token 催一次增量归档,代码默认 25000;**线上实际是 `0`**(2026-08-23 进容器读环境变量实测;~~此前手册两处都写着「线上现设 5000」,与「宁可多存也别被压缩蒸掉」那句取向说明一起,都是失真的~~)。**`0` 不等于整套失效**:第三十一次修过之后,`0` 只关「归档之后的周期性增量催促」,**硬线那一次照常触发**(判据 `lastArchiveTokens < hardTokens`);修之前 `0` 会连硬线一起哑掉,那才是 2026-08-10「161500 永久静音」的根因 |
