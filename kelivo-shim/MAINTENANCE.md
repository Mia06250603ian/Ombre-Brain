...
| 行号 | 节 | 约 token | 什么时候读 |
|---|---|---|---|
| 54 | ⚠️ 部署前必读(2026-07-13 事故教训) | 0.3k | **每次动 shim 都读**,三句话:仓库为唯一可信源、先 git pull、别用会话里的旧副本 |
| 63 | 当前 server.js 相对 7-12 初版的改动 | 9.8k | **改代码前读**。十条改动清单:1 进程误杀补丁 / 2 标题拦截 / 3 定性锚点 / 4 时间注入 / 5 感官(天气+经期) / 6 保温+主动唤醒 / 7 上下文守卫 / 8 人设拆分 / 9 上游报错不再吃成空回复 / 10 系统提示词 append↔replace。**只读你要动的那一条即可** |
...
| 297 | 系统回合(`x-system-turn: 1`) | 0.5k | 动查岗/系统注入类功能时读 |
| 313 | 架构 | 0.2k | 第一次接触 shim 时读,一屏看完 |
| 327 | Zeabur 位置(IDs 供 CLI 用) | 0.4k | 要跑 CLI 命令时来抄 id |
| 337 | 本目录刻意缺的三个文件(部署前必须补) | 1.6k | **部署前必读**:ian.md / profile-instructions.md / mcp-servers.json 怎么从容器拷出来 |
| 390 | **部署检查单(唯一权威版本)** | 0.2k | **部署前必读**,下面三小节是它的正文 |
| 399 | └ 部署前 | 0.8k | 〃 |
| 418 | └ 部署 | 0.3k | 〃 |
| 428 | └ 部署后(踩坑 9:上传成功 ≠ 上线) | 1.0k | 〃 **最容易被跳的一节,别跳** |
| 451 | └ 回滚的通用规矩 | 0.5k | 要回滚时读。核心一句:回滚不是零代价,等于再丢晏一个窗口 |
| 464 | └ 记录该写什么 | 0.2k | 部署完写 DEPLOY-LOG 时读 |
| 471 | 环境变量(值不入库;改值后要 restart) | 3.5k | **要调任何旋钮之前读**。链路/人格/感官/主动性/上下文守卫五组 |
| 508 | **踩过的坑(别再踩)** | 8.8k | **⚠️ 唯一必须全文读的一节。** 20 条:1 消息抢跑 MCP 握手 / 2 upload 丢 dotfiles / 3 沙盒里测 claude 会卡死 / 4 订阅 OAuth 登录 / 5 令牌只能一处跑 / 6 Kelivo 开关注入 system / 7 OB 域名失效致 MCP 静默失败 / 8 自动标题也是注入源 / 9 deploy success ≠ 上线 / 10 连发 deploy 前一次被取消 / 11 别人的会话把版本滚回去过 / 12 她问问题≠授权动手 / 13 代发「归档」要慎用 / 14 卡在 Pulling 是调度挂了 / 15 .gitignore 让上传静默丢文件 / 16 经期记录每次部署被清空 / 17 deploy 传的是当前工作目录 / 18 进 DEPLOYING 就挤不掉了 / 19 --settings 指向不存在的文件会拒绝启动 / 20 新 import 要同步改 e2e 的 cp 清单 |
| 701 | CLI 版本与升级指南 | 1.0k | 要动 CLI 版本时读(现钉死 2.1.215) |
| 731 | ~~待办~~ 「她在干嘛」改写 | 0.4k | 历史,已上线,一般不用读 |
| 749 | 她在干嘛(如果开了) | 0.5k | 动查岗功能时读 |
| 765 | ~~待办~~ 表情包补螃蟹 | 0.8k | 历史,已上线 |
| 788 | 表情包(如果接了 Telegram) | 0.8k | 动贴纸时读 |
| 811 | ~~待办~~ 「待办便利贴」 | 1.0k | 历史,已上线 |
| 836 | ~~待办~~ 「记错了 / 过期了」 | 3.9k | 历史,已上线(2026-08-19 第三十四次) |
| 931 | 建议(未做) | 1.5k | 想找「还能改什么」时读 |
| 976 | **2026-08-21 心跳「自主活动」+ 压缩点实测** | 1.2k | **改心跳文案 / 动上下文阈值前读**。心跳纸条的**四条机械约束**在这节(`【。】` 判不出沉默那条是硬伤);压缩点**实测 166942**,并撤销了「改人设要跟着下调阈值」的旧推算 |
| 1009 | 2026-08-10 第三十一次的机制细节 | 0.2k | 下面四小节的引子 |
| 1016 | └ ① 161500 硬线永久静音 | 0.6k | 动上下文守卫阈值前读 |
| 1037 | └ ② 终线原话桶顺序错乱 | 0.2k | 〃 |
| 1048 | └ ③ 压缩摘要压不住九节 | 1.8k | 〃 **根因已扒清、纸条尚未改** |
| 1100 | └ ④ awaken 的全文位把日记桶挤掉了 | 0.6k | ⚠️ **这节讲的是 OB,不是 shim** —— 改 OB 的 `awaken` 之前必读。**现在的结论是「OB 不用改」**;它里面那个「必然重演、要按类型各保一条」的旧结论**已被所有者当场推翻并撤销,别照它改**。另外它留了一把量「awaken 会不会把开机撑爆」的尺子(2026-08-10:占 27%;2026-08-21 钉选改出全文后:28%),往 awaken 加东西前拿它量一遍 |
| 1120 | 部署记录 | 0.8k | 只是指路:完整记录在 `DEPLOY-LOG.md`(35 条,覆盖第 2~36 次;第一次记在 `TIMELINE.md` 07-12),按「第 N 次」搜 |
...
- **接出 4.5 / 4.8(多模型)**:方案与全套实测在 `../docs/多模型接出方案.md`,**别在这儿重写**。
  ...
  所以推荐的做法是只改这个变量 + restart(零代码、不部署)。**2026-08-23 所有者拍板暂缓**,
  等第三十六次的观察期走完。⚠️ **动这件事之前先看那份文档的第 0 节**(她定的两条硬约束:4.6 绝对安全)
...
**本文件里凡是提到「第 N 次」,指的都是那一次部署,完整记录在 [`DEPLOY-LOG.md`](./DEPLOY-LOG.md)(**35 条,覆盖第 2~36 次**(2026-08-21 现场量;**量法**:`grep -cE "^- 2026-" DEPLOY-LOG.md` 数的是全部 49 条条目,其中带「第 N 次」标签的才是这里说的 35 条——07-12~07-17 那批早期记录没有编号,别把两个数搞混);⚠️ 「第一次」2026-07-12 初次搭建**没有独立条目**,记在 `TIMELINE.md` 的 07-12。2026-08-21 核过,原文写的「48 次」是错的)。**
...
sysprompt.mjs = **线上 `334b8c161e150bbc986e61fdae766468`**;
base.md / wake.md / CLAUDE.md / server.js 四件**线上与仓库一致**;
⚠️ **`sysprompt.mjs` 与 `test-sysprompt.mjs` 仓库比线上新,但只差注释与单测**
(2026-08-23 部署**之后**又改的:注释里的 `CLAUDE.md:行号` 换成了小节名、单测 82→86 项;
`git diff` 实测**非注释代码改动 0 行**)。仓库现值 `786373523836ee56f528b3883c74fd68` /
`379b486ec6c82f4d1235309dfabff182`。**下次全量 md5 对账看到这两件对不上是正常的**,
别当成「线上有改动没提交」去回滚 —— 判据是 `git diff <上次部署的提交> -- <文件>` 里
非注释行为 0;下次部署会自然带上去。
server.js = **`09724abc096602bf2ecf72be34d1064c`**。
