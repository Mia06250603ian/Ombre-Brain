...
| 717 | ~~待办~~ 「她在干嘛」改写 | 0.4k | 历史,已上线,一般不用读 |
...
| 968 | **2026-08-21 心跳「自主活动」+ 压缩点实测** | 1.2k | **改心跳文案 / 动上下文阈值前读**。心跳纸条的**四条机械约束**在这节(`【。】` 判不出沉默那条是硬伤);压缩点**实测 166942**,并撤销了「改人设要跟着下调阈值」的旧推算 |
| 1001 | 2026-08-10 第三十一次的机制细节 | 0.2k | 下面四小节的引子 |
| 1008 | └ ① 161500 硬线永久静音 | 0.6k | 动上下文守卫阈值前读 |
| 1029 | └ ② 终线原话桶顺序错乱 | 0.2k | 〃 |
| 1040 | └ ③ 压缩摘要压不住九节 | 1.8k | 〃 **根因已扒清、纸条尚未改** |
| 1092 | └ ④ awaken 的全文位把日记桶挤掉了 | 0.6k | ⚠️ **这节讲的是 OB,不是 shim** —— 改 OB 的 `awaken` 之前必读。**现在的结论是「OB 不用改」**;它里面那个「必然重演、要按类型各保一条」的旧结论**已被所有者当场推翻并撤销,别照它改**。另外它留了一把量「awaken 会不会把开机撑爆」的尺子(2026-08-10:占 27%;2026-08-21 钉选改出全文后:28%),往 awaken 加东西前拿它量一遍 |
| 1112 | 部署记录 | 0.8k | 只是指路:完整记录在 `DEPLOY-LOG.md`(35 条,覆盖第 2~36 次;第一次记在 `TIMELINE.md` 07-12),按「第 N 次」搜 |
