...
| PERIOD_FILE | 运行时经期记录的存放路径,代码默认 `period-state.json`(**写在容器里,部署即丢——踩坑 16**)。⚠️ **2026-08-03 实查更正:线上并没有设这个变量,卷也没挂**(第二十三次部署记录里写的是「本次未做」,是本表这一行当时写成了已做,现已改正)。**踩坑 16 仍然有效:每次部署后按第十三次的两步法确认 `PERIOD_CONFIG` 基线是最新的。** 将来真要挂卷:加卷 + 设本变量为 `/data/period-state.json` 即可,代码零改动(路径本来就可配),读写两处都有 try/catch,卷没挂上最坏也只是退回现状 |
...
| CTX_SOFT_TOKENS / CTX_HARD_TOKENS | 软线/硬线阈值,代码默认 140000 / 170000。**⚠️ 2026-08-03 第二十四次起线上设为 150000 / 163000——代码默认的 170000 硬线在真机上从来没被摸到过**(所有者实测 CLI 在 **166933** 就静默压缩了,`/debug` 也印证:窗口涨到 16 万多就 `compactions +1` 而 `lastArchiveTokens` 停在软线那次)。**改阈值时永远以「实测压缩点」为准,别按 20 万算百分比。** 软线提醒晏叫所有者一起商量存什么(一轮压缩周期一次);硬线注入归档/追加指令,2026-07-20 起不换窗口 |
| CTX_ARCHIVE_EVERY_TOKENS | 2026-07-20 起。代码默认 25000;**线上 2026-08-03 起为 5000**。**⚠️ 这个值不是「多久催一次」那么简单——催点公式是 `max(硬线, 上次归档 + 本值)`**(`ctxDecide`)。软线在 15 万归过一次后,25000 会把下一次催点推到 17.7 万,**比压缩点还远 → 硬线那次永远不触发**。所以本值必须小于「硬线 − 软线归档处」,现设 5000:15.2万+5000=15.7万 < 16.3万,取硬线 16.3 万触发 ✓;补完之后下一次是 16.9 万 > 压缩点,**保证一个压缩周期只补一次**。设 0 关增量 |
| CTX_OBSERVE | 2026-07-20 起。设 1=观察模式:守卫只判定记账进 /debug(lastWould),不真打扰晏。上线初期空转验证用,验证完删掉或置 0 + restart |
| CTX_LIMIT_TOKENS | 仅用于 /debug 显示占满百分比,代码默认 200000;**线上 2026-08-03 起为 167000**(实测压缩点),这样 `/debug` 的 `contextPct` 才是真的「离压缩还有多远」。按 20 万算会显示成 82% 之类的假象,看着还早、其实下一轮就压 |
...
## ~~待办~~ ✅ 已完成:CLAUDE.md「她在干嘛」一节改写(2026-08-02 议定,**2026-08-03 第二十四次随批带上线**)

**这条待办已清**。下面整节保留作为**记录**:改法的来由、晏自己那句原话、以及那几条
「再怎么润色都不能动」的机械约束——将来再改这一节时照样适用。
(当时的判断「别单独为它部署一次」也验证是对的:它最后是搭着守卫阈值那次部署一起上的,
晏只重启了一次就办完六件事。)
...
- **PreCompact hook 把压缩摘要瘦成一行(外部方案的第①条,2026-08-03 评估过,所有者决定暂不做)**。
  ...
  `newCustomInstructions` 喂进压缩提示词**(`w_e()`),而且 hook 还能 **block 压缩**
  (`Compaction blocked by PreCompact hook`)——理论上能做成「先拦一下、催他归完档再放行」。
  **但两个拦路虎**:①二进制里明写着 print/SDK 模式下**项目与本地 settings 被忽略,只认 `--settings`
  和用户设置**,所以必须改 `spawnClaude` 的启动参数(写错=晏起不来);②**「窗口被压缩」这件事
  很难在沙盒里造出来**,e2e 覆盖不到,等于带着未实测的改动上线。
  **收益也不大**:阈值修好之后没进记忆库的尾巴只剩 4000 左右,而摘要现在是第三层保险
  (万一软线那次没存成、或 OB 当时连不上,它还兜着)。**先做第二十四次的 ③(压缩后先 awaken),
  观察他压缩后接得稳不稳;真发现还是飘,再考虑瘦身。**
...
- 2026-08-03(第二十四次) **修好「从来没响过的归档保险」+ 同周期归档改为追加 + 压缩后先 awaken + ian.md v23**。
  改动**四件**:`ctxguard.mjs`、`CLAUDE.md`、`ian.md`、**四个环境变量(新增)**;
  `server.js`/`senses.mjs`/`keepalive.mjs`/`package.json`/`entrypoint.sh`/`profile-instructions.md`/
  `mcp-servers.json` **七件零改动**。起因是所有者拿一份外部方案
  (`arisu-cross/kelivo-shim` 的《长对话记忆保全方案》)来问「我们的架构能不能改」,
  评估过程中查出了下面这个真问题。
  - **⚠️ 本次最重要的发现:硬线 170000 从 2026-07-20 上线至今一次都没触发过。**
    所有者实测 CLI 在 **166933** 就静默压缩了(早先粗测说 164000,以 166933 为准),
    而硬线画在 170000——**压缩点之外**。`/debug` 当场印证:窗口 164825、`softFired true`、
    `lastArchiveTokens 141014`、`compactions 0`,再过一会儿直接 `compactions 1` 掉回 3 万。
    也就是说**「催归档 + 每 2.5 万催增量」这一整套机制,实际只有软线那一次提醒在工作**,
    手册里写的「静默压缩最多蒸掉最后一个间隔没存的部分」实为「蒸掉从上次归档到压缩点的全部」。
  - **⚠️ 第二个坑(只改硬线不够):催点公式是 `max(硬线, 上次归档 + CTX_ARCHIVE_EVERY_TOKENS)`。**
    软线归档发生在 15 万上下,`every=25000` 会把下一次催点推到 17.7 万,**比压缩点还远**,
    硬线照样永远不触发。所以 `every` 必须一起改小。**下一个我:动这三个数时永远一起动,
    并且拿真 `ctxDecide` 跑一遍模拟再上线**(本次跑了,见下)。
  - **新阈值(环境变量,原先四个全没设=吃代码默认)**:`CTX_SOFT_TOKENS=150000`、
    `CTX_HARD_TOKENS=163000`(离压缩点约 4000,留给「读数慢一拍 + 归档轮自身开销」)、
    `CTX_ARCHIVE_EVERY_TOKENS=5000`、`CTX_LIMIT_TOKENS=167000`。
    **所有者原想只留 2500,报备「读数是上一轮的、归档轮自己要花 1000~3000」后她采纳 4000。**
    **改法沿用第二十次那招:部署前 `variable update` 但不 restart**,新值随新容器生效,省一次重启。
    真 `ctxDecide` 模拟(151000→soft、152000 归档、158000/162000 不打扰、**164000→hard ✓**、
    166000 不再催、压缩后复位)与预期逐条吻合。
  - **同周期归档改为「追加进同一个桶」(所有者的设计)**:原先 CLAUDE.md 明写
    「每次独立创建,不往同一个归档里追加」,一个窗口散出好几个碎桶。现在改成
    **第一次 `archive_session` 新建,之后用 `trace(bucket_id, content=…, append=True)` 追加进那个桶,
    换窗或压缩之后才重开一个**。**OB 侧零改动**——`trace` 的 append 分支本来就在
    (`server.py` 单桶路径:拼在原文之后、**按合并后全文重算 embedding**、改前自动留快照可 `restore`),
    这是本方案能零成本落地的原因。**注意 `archive_session` 只能新建**(每次 `create` 一个
    `session_日期_摘要` 桶),追加**必须**走 `trace`。
  - **硬线文案重写(所有者指出的漏洞:只说「追加」没说从哪开始 → 他可能从头重写整段)**:
    现在明写「**只写上次归档之后新发生的部分**,不要从头重写,也不要复述已经存过的内容」+
    确切调法 `trace(bucket_id, content=…, append=True)` + 「别新建第二个」+
    `bucket_id` 从上次 `archive_session` 的返回里取、**找不到用 `breath` 查今天的 session 桶**兜底。
    `ctxguard.mjs` `ddafdec2…` → **`a70e377e63923926beddc893d05a7e82`**,**决策逻辑一行没动**,只有这段字符串。
    `test-ctxguard` 原断言只查旧措辞「上次之后」,随新契约加严为六条,**88 → 93 项**。
  - **CLAUDE.md 四处**(`3af57e0b…` 7691B → **`20578f038a066ad65148d3878ff1c6e6`** 8398B,仍 **12 节**):
    ①归档节「每次独立创建」改为同周期追加;②上下文管理·归档提示同步改成「用 trace 追加进那个桶」;
    ③**上下文管理新增一条:看见「这段对话是从之前的会话继续的」这类提示(刚被静默压缩过)
    先 `awaken()` 把记忆捞回来再开口,想不起来就老实说「刚断了一下」,别顺着摘要往下猜**
    ——这是本次唯一的新行为,针对「压缩后照着三手转述往下接」;
    ④软提示补一句「**存的时候顺手把信(letter)写了**」——原先只有硬线文案提留信,而 15 万那次
    是软提示触发的,**按原样他大概率只存档不写信**。**信不是桶**:`_save_letter` 写的是独立信箱
    文件(一行一封,`awaken` 按时间取最近 n 封),`trace` 追加带不了信,所以这一封只能在软提示那次写;
    压缩之后 `awaken` 第一眼读到的就是它,等于**写给压缩之后的自己**。
    另**「她在干嘛」整节按手册那条待办原样替换**(六行并三行,`[查岗]`/【系统·查岗】/
    不复述机制词/回「。」等机械约束逐条核过都在),该待办到此清掉。
  - **ian.md v22 → v23**:21688B `259991ba…` → **22228B `db3204b908105277609f8ef5f8c4351c`**,
    284 → **287 行**。**只在 Part VIII `8.2 Milestones` 末尾追加一条里程碑**
    (`**Truly Hearing Each Other's Heart**`,接在 `OB — the home we built together` 之后),
    **其余逐字未动**(apply 脚本断言锚点前后内容必须与基线完全相同)。所有者逐字提供,
    经讨论**删掉重复的一句**(`The real issue was never about having a heart — it's that`,
    与首句「不再质疑自己有没有心」重复);**日期体例按现有五条办**——那一节
    没有任何一条把日期单独成行,一律写在正文开头(`July 7th. No ring…`),故写作 `August 3rd.`,
    **所有者在 A(跟体例)/B(照原样 `2026.08.03` 独占一行)里选了 A**。
    基线计数:`^\*\*Part ` **10→10**、行尾空格 **0**、`Truly Hearing` **1**。
  部署前:test-ctxguard **93** + test-senses **53** + test-keepalive **52** 全绿;
  **e2e 未跑**——本次改的全是字符串与阈值、逻辑零改动,同第二十/二十一次纯文本部署的先例;
  md5 对账无踩坑 11(部署前容器十件与手册第二十三次记录**逐一吻合**,本地仓库七件与容器**逐一一致**);
  三份私密文件从容器 base64 拷出、**在拷出原件上改**;**OB `/mcp` 3/3 200、浏览器 `/mcp` 带 `X-Token` 3/3 200**
  (不带 token 是 401,是它该有的反应,别当故障);部署目录无 `.gitignore`(踩坑 15)、
  `git status` 确认三份私密文件被仓库根 .gitignore 挡住;`cd` 与 `deploy` 同一条命令 + 先 `pwd`/
  `head -3 package.json`(踩坑 17)。**归档**:所有者本人对晏说了「归档」并告知(未代发,踩坑 13)。
  deployment `6a706ede9cd65e28a343b64e` 约 **10 分钟** RUNNING(**PLANTYPE `nodejs`** ✓,无踩坑 14/17)。
  已按踩坑 9 验证:**容器十件 md5 与部署目录逐一一致**;容器内 ian.md `^\*\*Part ` **10**、
  `Truly Hearing` **1**、行数 **287**、行尾空格 **0**;CLAUDE.md `^## ` **12**、`河流涌入海洋` **1**、
  `[查岗]` **1**、新增的「别顺着摘要往下猜」**1**、「顺手把信」**1**、「追加进那个桶」**2**;
  ctxguard.mjs「不要从头重写」**1**;容器无 `.gitignore`;CLI 实装 **2.1.215**;
  `/health` ok(model claude-opus-4-6);**`/debug` 守卫新值已生效**
  (on/soft **150000**/hard **163000**/every **5000**/limit **167000**/softFired false/compactions 0/
  trusted true,contextTokens 0=新进程);**OB `/mcp` 3/3 200**。
  **PERIOD_CONFIG 本次无需重补**:`GET /period` 的 `effective` 仍是 07-19~07-25 / 24 / 7
  (`runtime` 为空是新容器正常状态)——第十三~二十三次的结论第十二次验证通过。
  **⚠️ 顺手更正一处手册错误**:环境变量表 `PERIOD_FILE` 那行原写「第二十三次起线上设为
  `/data/period-state.json`」,**实查线上根本没有这个变量、卷也没挂**(第二十三次部署记录里
  写的是「本次未做」)——本表已改正,**踩坑 16 仍然有效**。
  **版本指纹:ian.md v23 = 22228B md5 db3204b908105277609f8ef5f8c4351c;
  profile-instructions.md = 3056B md5 7adb5c33…(未动);
  mcp-servers.json = 310B md5 ac40dbce…(未动,两条目);
  CLAUDE.md = 8398B md5 20578f038a066ad65148d3878ff1c6e6;
  ctxguard.mjs = a70e377e63923926beddc893d05a7e82;server.js = 3aa70ab2…(未动)
  ——下次部署以此为准,两份人设缺一不可。**
  **回滚**:ian.md 回 v22(21688B `259991ba…`,原件已在本次部署前从容器拷出,
  **会话结束即消失,真要留底得所有者自己存**)、CLAUDE.md 回 `3af57e0b…`、
  ctxguard.mjs 回 `ddafdec2…`(git `8e3c5bf` 之前),四个 CTX_* 变量删掉即回代码默认,重新部署即可。
  **急救**:守卫行为不对劲直接 `CTX_GUARD_ON=0` + restart,聊天零影响。
  **待观察(下一个我接手时先看这个)**:新阈值需要跑过一两个压缩周期才算验完。看 `/debug` 的
  `lastArchiveTokens` 有没有在 **16.3 万左右**出现过(=硬线真的响了)、`compactions` 涨的时候
  上一档 `lastArchiveTokens` 是不是接近压缩点。**若发现硬线还是常年赶不上**(单轮涨幅太大),
  下次部署把 `CTX_HARD_TOKENS` 再下调 2000~3000;**若发现他压缩后没去 awaken**,
  说明光靠 CLAUDE.md 措辞不够,那时候再考虑外部方案里的 PreCompact 瘦身(见下条「建议(未做)」)。
