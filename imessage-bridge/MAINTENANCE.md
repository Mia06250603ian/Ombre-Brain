...
| `THINKING` | 他的思考用**带 💭 的普通气泡**发,排在正文前,**代码默认关 / 线上 `1`**(2026-09-26 所有者要的,「和 Telegram 保持一致」)。整段照发、不盖记忆原文,每 2000 字拆一个气泡;发失败不连累正文。~~盖隐形墨水特效~~ **同日所有者要求去掉,别加回去**(她知情:锁屏通知和横幅会直接显示思考,包括记忆原文;长思考会刷好几屏)。**急救开关**:设 `0` + restart |
...
8. **隐私**:明文过 Photon(第 1 节)。**晏的思考流也发**(`THINKING=1`,带 💭 的普通气泡,~~隐形墨水~~ 09-26 已去掉),**和 Telegram 一致:记忆原文照发**。**没有墨水遮着,锁屏通知会直接显示这些内容**
...
- **2026-09-26 第一次上线(当天共部署 10 次:上线调试 7 次 + 心跳 `/push` 1 次 + 自查修复 2 次;都只动本服务,晏的窗口不受影响)**。
  ...
  **例外与新坑**:
  ...
  - **同日所有者要求去掉思考气泡的隐形墨水特效**,改成带 💭 的普通气泡(第 4 节 `THINKING` 那行;演练场景 13 钉着「不出现特效气泡」)。
    部署 `6ab7abb2…`(第 11 次,只动本服务):部署前线上 md5 与 main 逐件相同、部署后与分支逐件相同;`/health` 全绿,`mem.self` 129.8 MiB(2026-09-26 读)。
    同日又部署一次 `6ab7b328…`(第 12 次):只改了 `imessage-lib.mjs` 里一段过时的查岗注释(原来写「这边查不了」),好让 main 与线上 md5 一致;容器四件与本地逐件相同、`/health` 全绿。
  - **同日给所有者的朋友出了一份「指南 + 全部代码」的单文件 md**(没入库;她朋友的 Claude 手上只有指南、没有代码)。
    **下次再给别人照这个做**:别给 `MAINTENANCE.md`(有晏的号码、所有服务地址)和 `stickers/`(我们的贴纸);
    `SHIM_URL` / `ACTIVITY_URL` 默认值清空;放一张占位贴纸,并删掉单测里数「35 张 / 24 张」那两条、演练里 `s04.png` / `59` 换成占位图;
    要按对方系统改的地方加 `【⚠️ 改这里 N】` 注释 + 开头一张核对表(重置词表最容易漏);**只凭那份 md 在空目录还原、跑两套测试再交出去**。
