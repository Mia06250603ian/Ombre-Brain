...
const GIF_BODY = 150;   // 画布里螃蟹本身缩到 150,四周留透明边。2026-09-26 所有者真机反馈 240 撑满「还是太大」——
                        ...
                        // 留白放**右边**(螃蟹贴左、上下居中):晏的消息靠左显示,居中的话左边空一大块(所有者同日第二次反馈)
...
for (const [tag, file] of Object.entries(reg)) {
  ...
  if (file.endsWith(".webp")) {
    ...
    execFileSync(ffmpeg, ["-hide_banner", "-loglevel", "error", "-y", "-c:v", "libvpx-vp9", "-i", src,
      "-filter_complex", `fps=${GIF_FPS},scale=${GIF_BODY}:${GIF_BODY}:flags=neighbor,format=rgba,pad=${GIF_SIDE}:${GIF_SIDE}:0:(oh-ih)/2:color=0x00000000,split[a][b];[a]palettegen=max_colors=48:reserve_transparent=1[p];[b][p]paletteuse=dither=none:alpha_threshold=128`,
