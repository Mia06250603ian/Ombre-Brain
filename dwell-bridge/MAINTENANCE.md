...
## 手感校准:2026-09-10 所有者定的五个数(**已合进 main,未部署、未真机确认**)
...
**停在哪**:**PR #3 已由所有者本人合进 main**(`8088ef7`,2026-09-10;五个值已在 `origin/main` 上核对过)。
**剩下的两件**:①**尚未在真机上确认**(手册那条「预览不等于实机」照旧算数);②**尚未部署,线上还是旧样子**。
**上线**照本手册第 5 节跑 `fetch-frontend.sh` + 部署,**Zeabur 那一步要所有者本人点**。
⚠️⚠️ **部署时 `fetch-frontend.sh` 千万别带分支名** —— 见下面第二次部署记录里那条已撤销的说明。
带了 `claude/pill-and-brandline`(手册原来钉的那个)**今天这五个数一个都不会上去**
(2026-09-10 现场量:该分支仍在远端、且不含 `3219c93`)。**不带参数默认就是 `main`,那才是对的。**

~~原文「停在 PR #3、未合;合之前要她真机过一眼」~~ —— **已过期**:她在我写这段文档之前就合了。
留着是为了记住那条教训:**待办/状态这类话写下去的那一刻就可能过期,别只信文档,去 `git log` 看一眼。**
