...
| **`MSG_CAP`** | 否 | **(2026-09-14 新增)** 网页上往回翻得到多少条,默认 4000,**线上现值 20000**。⚠️ **它同时决定内存占用**——记录要能翻就得在内存里,**所以这是「机器内存够不够」的问题,不是「盘够不够」**(盘那头一年也才十几 MB)。**实测(2026-09-14,照真实比例造的记录)**:4000 条 ≈ +16 MiB ≈ 能翻 1250 轮 ≈ 两个月;**20000 条 ≈ +37 MiB ≈ 6250 轮 ≈ 十个月**;当时整机 `MemAvailable` **1.46 GiB**。**改值 + restart 即生效,不用部署**;现场再量法见《聊天记录怎么才不丢》 |
...
cd dwell-bridge
npm install
node test-dwell.mjs        # 单测，**2026-09-14 现场量 106 项**(拉过前端之后;没拉是 98 项，末尾那 8 条会跳过。数会变，以实际输出为准)
...
| `GET /api/health` | 存活 + 忙不忙 + 最后一次上游报错（**不过口令**，给排查用；不含任何密钥）。⚠️ **排查先看这两个字段**:`persisted`(记录落不落盘,见《聊天记录怎么才不丢》)、`ver`(这一版的指纹;线上那个值和本地算出来的一样 = 部署上去的就是这份代码) |
...
npx -y zeabur@latest service exec --id 6a81a118bdeaa87e2c52bec3 --env-id 6a53a9fcb6ce8edcb0163f97 -i=false \
  -- sh -c 'df -h /data | tail -1; grep MemAvailable /proc/meminfo; cat /sys/fs/cgroup/memory.current'
```
