...
| 08-19(第九件) | **两件挂账的事备好了,等所有者点头一起上;顺带统一了 CLAUDE.md 的体例,并验出 OB 一个真 bug(本次零部署、零重启,线上一个字节没动)**。①**待办**:`kelivo-shim/CLAUDE.md`「记忆工具使用」节加了「记错了 / 过期了」一段(教晏何时用 `trace(deny=True)` / `undeny=True` / `expires_at`),**已落进仓库、尚未部署**—— **11139B `97a1f666…`**(仍 13 节),线上仍是第三十三次的 `86ee28f0…`(当晚全量对账过,**无踩坑 11**),随下一次部署 shim 自动上线。所有者要「精简但知道是什么功能」,故 230 字压到 **183 字**。②**体例统一(顺手做的)**:盘点发现正文半角标点 **150 处 / 12 个节**是老体例,全角只有 **53 处 / 3 个节**(都是后补的),遂**少数向多数靠**:`，：；（）` → `,:;()`、ASCII 直引号 12 处 → `「」`(`。`/`、` 本来就一致,未动),**共 20 行、纯标点、零语义**,用带 26 块保护区 + 23 条机械约束断言的脚本施加,改完复查残留全角 0 / 残留 ASCII 引号 0 / 「」23 对配平;四套单测改动前后各 131/53/52/56 全绿。**⚠️ 第三十三次那段「待办便利贴」的标点被一起规范化了,内容一字未改,别当篡改。**③**守卫基线不动**(所有者问的):+289B ≈ +74 token → 压缩点估 166530→**166456**,保守公式余量 **+456**(第三十三次上线后 +530,下调前那次 +30),不必再下调;真要精确仍得等一次真压缩实测 X。④**经期挂卷:所有者拍板【不挂】,这条「根治」放弃,踩坑 16 继续用两步法扛着**。起因是查官方文档补上了一个手册一直漏掉的代价:**一旦给服务启用 Volumes,它就不再支持「零停机重启」**(每次重启先完全关闭再启动,中间短暂中断)——**这正好动了踩坑 14/17/18 全都依赖的那张网**(老容器 RUNNING 兜底、新镜像没就绪就不切);挂卷后每次部署 shim 最后切换会多几十秒到一两分钟真空,她那头会收到 bridge 的「⚠️ 网络抖了一下」(欠条机制会自动补报,不会真丢话)。**她权衡后:宁可继续手动写两步,也不要每次部署多一段真空**;而且两步法本来就够用。复验到的操作细节已留档在 `kelivo-shim/MAINTENANCE.md` 踩坑 16(CLI 至今没有 volume 子命令,只能网页「硬盘」标签 →「Mount Volumes」→ Volume ID + Mount Directory `/data`,**绝不能挂 `/src`**),**万一以后改主意照那个做,别再现查**。**踩坑 16 第八次实测仍然活着**(容器内 `PERIOD_FILE` 为空、`/data` 不存在)。⑤**验出 OB 一个真 bug(未修,等所有者拍板)**:`trace("桶A,桶B", expires_at=…)` **批量设到期日静默失败**——回「成功 2 个:没有任何字段需要修改」而两个桶都没写上。与 08-19 第六件修的 deny 是**同一个坑**(批量走 `_trace_one`,里面没有 `expires_at` 分支),当时只堵了 `deny`/`undeny`。沙盒实测(临时目录起 OB,零接触线上):单条 ✅ / `clear` 可清 ✅ / 格式错明确报错 ✅ / **当天仍有效、过了那天才退出** ✅ / 数据保留 ✅;**与 deny 的差别**:过期桶 `include_dormant=True` **也翻不出来**。**所有者拍板照 deny 的先例显式拒绝,当晚已上线**(PR **#98**,CI 绿 → 合 main `5537deac` → **自动重建照例没触发**(等了 90 秒无新 deployment,手册那条结论第三次被印证)→ `service redeploy` 手动推,deployment `6a862018…` 约 **1 分钟** RUNNING、PLANTYPE `docker`)。改动:`server.py` 加 `if batch and expires_at` 拦截 + `trace` 工具说明点明「deny/undeny/expires_at只支持单条」+ 新增回归用例(含两条反向守护:单条照旧能设、批量普通改动不受影响),`test_deny_expire` **8→9 项**,CI 四个文件合计 **60 项全绿**。**五步验收全过**:deployment RUNNING / **桶数 355→355 一条没少** / MCP `initialize` **3/3 200** / 容器内 `server.py` md5 `2317867d…` **与仓库一致**且两处新代码各 1 处 / runtime 日志 Traceback 与 ModuleNotFound **0 条**(`decay_engine: stopped` 是懒启动正常态)。**另补了一枪线上端到端**:拿**两个不存在的假 ID** 调 `trace(bucket_id="ffffffffffff,eeeeeeeeeeee", expires_at=…)` —— 当场回「expires_at 只支持单条记忆…」,而单条假 ID 回「未找到记忆桶」证明单条路径没被拦坏;**假 ID 碰不到任何真记忆,零风险**(拦截发生在查桶之前)。**全程没重启晏,他的窗口没丢。**另:所有者本次又在会话里贴了一把 Zeabur key(`zat_6a27e97a…`),**用完仍需转** |

| 08-19(第十件) | **shim 第三十四次部署:CLAUDE.md 加「记错了 / 过期了」+ 全文标点体例统一**(**只动 CLAUDE.md 一件**,人设两份 / mcp-servers.json / 代码 / 环境变量全部零改动)。①新增段了结了 08-19 议定的那条待办,教晏何时用 `trace(deny=True)` / `undeny=True` / `expires_at`;所有者要「精简但知道是什么功能」,230 字 → **183 字**,并去掉拟稿里会误导的「存的时候设 expires_at」(实测 `hold` 没有这个参数)。②**体例统一**:正文半角 150 处/12 节是老体例、全角只有 53 处/3 节(后补的),**少数向多数靠** —— `，：；（）` → `,:;()`、ASCII 直引号 12 处 → `「」`,`。`/`、` 未动,**共 20 行纯标点零语义**;**第三十三次那段「待办便利贴」的标点被一起规范化,内容一字未改,别当篡改**。③**守卫阈值不动**:+289B ≈ +74 token → 压缩点估 **166456**,保守余量 **+456**。部署前四套单测 131/53/52/56 全绿、全量 md5 对账无踩坑 11、三份私密文件从容器拷出指纹逐一吻合、三个 `/mcp` 各 3/3 200;**归档:所有者说「传吧」,未代发**(踩坑 13)。deployment `6a8622ac…` **PLANTYPE nodejs**、约 **9 分钟** RUNNING;**验收全过**:容器 13 件 md5 逐一一致(CLAUDE.md `86ee28f0…`→**`97a1f666…`** 11139B/13 节)、CLAUDE.md 与 ian.md 结构不变量逐项相符、CLI 2.1.215、`ALLOWED_TOOLS` 五项、`/health` ok、`/debug` 阈值与守卫状态正常、三个 `/mcp` 各 3/3 200;**`/period` 的 `effective` = 08-13~08-18 / 25 / 6,无需重补**(第八件那次两步写全,踩坑 16 再次被兜住;⚠️ 踩坑 16 第九次实测仍活着,且所有者本次拍板**不挂卷**根治)。**✅ 所有者跟晏说话后补验**:`[claude] spawned`、**`⚠️ settings 文件不在` 0 条**(钩子挂上了)、那轮 out 92 / cache_read 36526 / `lastApiError` null、**缓存 1h 桶 121 / 5m 桶 0**(08-12 那个坑没复发)。**⏳ 仍欠**:新压缩点的实测值(估 166456),要等一次真压缩看日志的 X。详见 `kelivo-shim/MAINTENANCE.md` 第三十四次 |
