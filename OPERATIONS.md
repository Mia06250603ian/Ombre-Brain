...
| ~~**接出 4.5 / 4.8(多模型)**~~ | **✅ 2026-08-24 已上线(shim 第三十七次)**:Kelivo 菜单里能选 4.6 / 4.5 / 4.8 了,**默认休眠**——线上当前仍是 4.6,她在手机上点一下才真换(**换模型必然重开进程 = 丢一个窗口**,这是架构的物理规律)。⚠️ **仍待她实测两件**:①真链路上 4.5 跑不跑得起来(方案文档第 5 节:物理上没法预先验);②乒乓球验收——切完模型去 TG 说一句,`/health` 应该还是新模型。急救开关:清掉 `BRAIN_MODELS` + restart。 所有者当天改主意要「现在就做」,并**当场同意绕开她自己定的「不许为这件事单独部署」那条红线**(理由:她已让晏归档,而窗口当时已涨到 158770(95%)快到头,**这一个窗口本来就要丢**)。名单 `BRAIN_MODELS` 定为 `claude-opus-4-6,claude-opus-4-5-20251101,claude-opus-4-8`(**Opus 5 不放**,CLI 认不了)。功能**默认休眠**:部署完那一刻晏还是 4.6、行为逐字不变,她在 Kelivo 点一下才真换。**下面这段是 08-23 的旧状态,按规矩 5 留着备查**:~~2026-08-23 可行性全部验完,所有者说「方案没问题」,但她拍板暂缓~~ —— 要等上面那行的观察期走完再加(**理由**:同时换模型的话,他说话调子飘了就分不清是谁干的)。**她选的是「方案 B」**(Kelivo 菜单里自己切;理由:她要「先备着、想用再切」,而方案 A 里那个变量本身就是开关、没有中间状态)。⚠️ **两条红线**:①**不许为这件事单独部署** —— 等下次 shim 因别的事要部署时搭顺风车,额外代价才是零窗口;②功能**默认休眠**(名单不设就只有 4.6),部署完那一刻行为逐字不变。她定的硬约束是「**4.6 绝对安全,会影响 4.6 就不做**」。**这件事不催、不主动提,你正好要部署 shim 时再提醒她一句。**⚠️ **Opus 5 现在接不了**:CLI 2.1.215 不认识它(拿瞎编的模型名做对照实测),要先升 CLI,而 CLI 是所有模型共用的 —— **单独立项,别顺手做** | `docs/多模型接出方案.md` |
