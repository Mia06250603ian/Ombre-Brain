...
| **iMessage 接晏**(新目录 `imessage-bridge/`) | **2026-09-26 代码写完、单测 108 项 + 演练 36 项全绿,尚未建服务、尚未部署**(检查单在该手册第 6 节;**所有者已注册 Photon、建好项目 `ian phone`**)。⚠️ **控制台「加手机号」+86 收不到验证码**(已找客服、转人工未回)→ 改成服务启动时用项目凭证调 Photon 的登记接口,**不走短信**,分到的号码显示在 `/health` 的 `line`;**Photon 收不收 +86 要上线才知道**。起因是所有者发来一份 Photon 教程问「我们的架构能做吗」;**没照教程用 Agent SDK 另起一个 claude**(那会冒出第二个晏),改成照 Telegram 桥的样子做一扇新门,**shim 零改动、晏的窗口不动**。**所有者知情接受的风险**:消息明文过第三方 Photon(隐私政策没写存不存)、线路可能说断就断 —— 所以 **Telegram 仍是主线**。⚠️ **还差**:建服务部署 → 她把 Project ID/Secret 自己填进 Zeabur 控制台(**别贴进聊天**)→ `/health` 出号码后她用 iPhone 往那个号码发一条(**气泡要蓝色**)。⚠️ **两件她没定、别自作主张**:①心跳(他主动找她)要不要也能走 iMessage(要改 shim 的推送出口 = 丢窗口);②要不要在晏的 `CLAUDE.md` 里告诉他「你也在 iMessage 里」(改人设,等 shim 顺风车) | `imessage-bridge/MAINTENANCE.md`(第 1、6、7 节) |
...
2. **Zeabur API key:用完就该作废** —— 她在会话里贴过明文,已进聊天记录。
   **累计十五次**(09-26 **第十五次**:她在问「内存还够吗」时顺手贴来,**这次没用它** —— 整机内存用 dwell `/api/health` 的 `mem.avail` 零权限量到了,已提醒作废;09-23 第十四次:她贴来让我部署第四十二次(顺风车 + 经期挂卷),已提醒用完删掉;09-23 第十三次:她贴来让我部署 5.5,已提醒用完去控制台删掉;**两串值都没写进任何文件、提交或文档**):一次日期没记下、08-16、08-28(两次)、08-30、09-01、09-02、09-03、09-10(两次)、09-12、**09-14**。
...
| 〃 | **imessage-bridge** | **尚未创建**(2026-09-26 代码写完,建服务的命令在 `imessage-bridge/MAINTENANCE.md` 第 6 节) | 未分配 | **iMessage ⇄ shim 的桥**:照 Telegram 桥的样子,无状态、不挂卷、**不碰晏**。中间过第三方 Photon(免费档、共享号码池)。空载内存本机实测约 103 MiB(2026-09-26,非线上数) |
