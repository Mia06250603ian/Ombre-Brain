...
> **怎么读这份手册(2026-08-19 起)**:急着干活就**读到第 4 节为止**(**约 40 KB**,2026-09-26 两轮瘦身后实测 40.5 KB;~~约 20 KB~~ 是早先的数,瘦身前已涨到 57.6 KB。现场再量:`awk '/^## 5\. /{exit} {print}' OPERATIONS.md | wc -c`),那已经够开工;
...
| **dwell 换壳(三栏)+ Home 重做** | **2026-09-19 已上线,但没做完**(dwell 部署到第二十二次时所有者额度用完;部署次数以 dwell 手册为准)。⏳ **还没做**:八间屋子的门面在新壳里重做、首页卡片接真数据、皮肤/透明度只存浏览器本地。**接着做之前先读 dwell 手册《怎么对着截图改 UI:七条》**(最要紧:**绝不要拿容器里的渲染当尺子量字号,也别照预览调字重**)。⚠️ 返回条要合成一条得动聊天页顶栏(碰「照官端」),**她没点头别动**。本行原文在 `TIMELINE.md`「附二」 | `dwell-bridge/MAINTENANCE.md`《怎么对着截图改 UI》《Home 页:怎么按参考图量,量到什么》《换布局》 |
...
**dwell UI 留下的四条**(碰 dwell 前端视觉之前必读;最要紧的是第 4 条「先找『照官端』标注再动」)**2026-09-26 已搬进 `dwell-bridge/MAINTENANCE.md`《碰前端视觉之前的四条》**,原文一字未改。
...
3. ~~**`REPORT_TOKEN` / `EARS_TOKEN` 要转**~~ —— 两把都泄露过进会话记录;✅ **2026-08-28 所有者拍板:都不换**(一把只能往手机活动记录里塞假数据,一把只锁 ears,后面都没有别的密钥)。
   ⚠️ **真要换,几处必须一起换,漏一处就静默坏、不报错**:
   `REPORT_TOKEN` = **她 iPhone 的快捷指令 + telegram-bridge + imessage-bridge**(漏手机 = 上报静默失效,几天后才发现晏不查岗;漏 imessage = iMessage 里查岗静默失效);
   `EARS_TOKEN` = **ears + telegram-bridge + imessage-bridge**(~~bridge 与 ears 两边~~ 09-26 起 iMessage 语音也用同一把,漏了 = 那扇门的语音直接坏)。
   原文在 `TIMELINE.md`「附二」。
4. (2026-09-26 并入第 3 条;编号保留,因为别的手册按「第 0 节第 6 条」引用下面那条)
