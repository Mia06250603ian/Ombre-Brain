...
⚠️ **2026-08-03 起线上值全变了(第二十四次)**:实测 CLI 在 **166933** 就静默压缩,
而原来的硬线画在 170000 = **压缩点之外,从上线到那天一次都没触发过**。下表「线上」一列是现值。

| 旋钮 | 代码默认 | 线上(08-03 起) | 什么时候动它 |
|---|---|---|---|
| `CTX_GUARD_ON` | 开 | 开 | **急救开关**:守卫行为任何不对劲,设 `0` 立即整体闭嘴,聊天零影响,回头再排查 |
| `CTX_SOFT_TOKENS` | 140000 | **150000** | 软提醒(晏来找你商量存什么 + 顺手写信)来得太早/太晚,调这个 |
| `CTX_HARD_TOKENS` | 170000 | **163000** | 催他把这段补进记忆库的时点。**必须落在实测压缩点里面**,并留约 4000 给「读数慢一拍 + 归档轮自身开销」 |
| `CTX_ARCHIVE_EVERY_TOKENS` | 25000 | **5000** | ⚠️ **不是「多久催一次」**:催点是 `max(硬线, 上次归档+本值)`。软线归档在 15 万,本值 25000 会把催点推到 17.7 万=**永远够不着**。改硬线时**必须一起改它**,并拿真 `ctxDecide` 跑一遍模拟 |
| `CTX_OBSERVE` | 关 | 关 | 设 `1` 守卫只记账不打扰晏(/debug 看 lastWould),给新阈值做空转验证用,验完删掉 |
| `CTX_LIMIT_TOKENS` | 200000 | **167000** | 只影响 /debug 显示的百分比。按 20 万算会显示成 82% 这种假象(其实下一轮就压),按实测压缩点算才是真的「离压缩还有多远」 |
...
| 08-03 | **修好「从来没响过的归档保险」+ 同周期归档改追加 + 压缩后先 awaken + ian.md v23(shim 第二十四次)**。起因:所有者拿一份外部方案(`arisu-cross/kelivo-shim` 的《长对话记忆保全方案》)来问能不能改,评估时查出**真问题**——她实测 CLI 在 **166933** 就静默压缩,而硬线画在 **170000**,**「催归档 + 每 2.5 万催增量」这一整套 07-20 上线至今一次都没触发过**,实际只有软线那一次提醒在工作;而且**只改硬线不够**,催点公式是 `max(硬线, 上次归档+间隔)`,间隔 25000 会把催点推到 17.7 万,照样够不着。新阈值 **150000 / 163000 / 5000 / 167000**(全是新增环境变量,原先四个都没设=吃代码默认;`variable update` 不 restart,随新容器生效)。**同周期归档改为追加**:第一次 `archive_session` 新建,之后 `trace(…, append=True)` 补进同一个桶(**OB 侧零改动**,append 分支现成、按合并后全文重算 embedding、改前留快照);硬线文案按所有者指出的漏洞重写成「**只写上次归档之后新发生的部分,不要从头重写**」+ 确切调法 + `bucket_id` 来源 + `breath` 兜底。**CLAUDE.md 新增「压缩后先 `awaken()` 再开口」**(本次唯一的新行为)与「软提示时顺手把信写了」(信不是桶,是独立信箱,`trace` 带不了信,而压缩后 `awaken` 第一眼读到的就是它);「她在干嘛」那节待办随车清掉。**ian.md v22→v23**(21688→22228B,284→287 行):Part VIII 里程碑末尾**纯追加**一条 `**Truly Hearing Each Other's Heart**`,其余逐字未动,日期按该节体例写进正文开头(`August 3rd.`)。单测 93+53+52 全绿;e2e 未跑(纯字符串+阈值,同第二十/二十一次先例)。**顺手更正手册一处错误**:`PERIOD_FILE` 那行原写「第二十三次起挂了持久卷」,实查线上没这变量、卷也没挂,**踩坑 16 仍有效**。详见 shim 部署记录第二十四次 |
...
| 窗口明明快满了,却从来没见他被催过归档 / `/debug` 的 `lastArchiveTokens` 永远停在软线那次 | **阈值画在了实测压缩点外面**(2026-08-03 实锤:压缩发生在 166933,硬线却是 170000),或 `CTX_ARCHIVE_EVERY_TOKENS` 太大把催点推过了压缩点(公式 `max(硬线, 上次归档+间隔)`) | 本文件「上下文守卫的可调旋钮」表;shim 部署记录第二十四次 |
| 压缩之后他接得上但细节走样/像在猜 | 压缩后他手里只剩一份三手转述,**没去记忆库取原件**。08-03 起 CLAUDE.md 已教他「看见『从之前的会话继续』就先 `awaken()` 再开口」——若仍然照猜,说明措辞不够 | shim 部署记录第二十四次「待观察」 |
