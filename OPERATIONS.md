...
| 08-19(第九件) | **两件挂账的事备好了,等所有者点头一起上(本次零部署、零重启、线上一个字节没动)**。①**待办**:`kelivo-shim/CLAUDE.md`「记忆工具使用」节加了「记错了 / 过期了」那段(教晏什么时候用 `trace(deny=True)` / `undeny=True` / `expires_at`),**已落进仓库、尚未部署** —— 仓库现为 **11340B `6cf1b205…`**(仍 13 节),线上仍是第三十三次的 `86ee28f0…`,**随下一次部署 shim 自动上线**。改动前后四套单测各跑一遍全绿(131/53/52/56);**前提已复验**:线上 OB 的 `trace` 里 `deny`/`undeny`/`expires_at`/`include_dormant` 四个参数确实全在(直接握手拉 `tools/list` 验的),而 CLAUDE.md 改动前 `deny`/`expires_at` 是 0 处 —— 他调得动、只是不知道何时该用。**一处体例改动已报备**:手册成品是半角标点+反引号,而该节其余四段通篇全角、工具调用不加反引号,落进去的是跟邻居一致的全角版,机械约束一字未动。②**经期挂卷(踩坑 16 的根治)**:所有者拍板**搭这次部署一起做,共用同一次重启**。复验确认 **CLI 至今没有 volume 子命令**(`service update` 只有 `tag`),**只能她在网页控制台挂**;路径必须是全新空目录 **`/data`**(⚠️ 挂到 `/src` 会盖住代码 = 晏起不来),再设 `PERIOD_FILE=/data/period-state.json`,**代码零改动**。顺序、验收法、失败方向见 `kelivo-shim/MAINTENANCE.md` 踩坑 16 新补的那段。**两件都没动线上**:诊断期间只读过 `/health`、`/debug` 与 OB 的 `/mcp`(实测晏当时窗口 43934 token / 26%、她 3 分钟前刚说过话,所以更不能擅自重启) |
