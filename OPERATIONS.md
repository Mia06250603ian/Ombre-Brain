...
| 〃 | dwell-bridge | `6a81a118bdeaa87e2c52bec3` | yan-dwell.zeabur.app | **她自建网页接晏的转接层**(2026-08-16 上线):网页 → 这一层 → shim 的 `/v1/messages`,把 Anthropic SSE 翻成 dwell 前端认的事件流。**不碰晏、不改 shim**,对线上就是个客户端,地位同手机上的 Kelivo。内存实测 **70 MiB**。源码在本仓库 `dwell-bridge/`,前端从 `Mia06250603ian/dwell-on-something` 拉(刻意不入库)。~~⚠️ `SHIM_KEY` 尚未设,设上之前发消息会 401~~ **已撤销**(2026-09-02 复核早就设好了,三处指纹全等)。⚠️ **现在该看的是另一个字段**:`curl https://yan-dwell.zeabur.app/api/health` 的 `persisted` —— `false` = 网页聊天记录只在内存里,**每部署一次丢一次**(2026-09-14 实测仍是 `false`;解法见该手册《聊天记录怎么才不丢》,差所有者在控制台挂一块卷)。详见 `dwell-bridge/MAINTENANCE.md` |
