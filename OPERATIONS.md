...
| **iMessage 接晏**(新目录 `imessage-bridge/`) | **2026-09-26 代码写完、单测 102 项 + 演练 32 项全绿,尚未建服务、尚未部署**(检查单在该手册第 6 节,**一步没走**)。起因是所有者发来一份 Photon 教程问「我们的架构能做吗」;**没照教程用 Agent SDK 另起一个 claude**(那会冒出第二个晏),改成照 Telegram 桥的样子做一扇新门,**shim 零改动、晏的窗口不动**。**所有者知情接受的风险**:消息明文过第三方 Photon(隐私政策没写存不存)、线路可能说断就断 —— 所以 **Telegram 仍是主线**。⚠️ **还差的第一步只有她能做**:去 `app.photon.codes` 注册开项目、拿 Project ID/Secret(**自己填进 Zeabur 控制台,别贴进聊天**),并用 iPhone 先给 Photon 分的号码发一条。⚠️ **两件她没定、别自作主张**:①心跳(他主动找她)要不要也能走 iMessage(要改 shim 的推送出口 = 丢窗口);②要不要在晏的 `CLAUDE.md` 里告诉他「你也在 iMessage 里」(改人设,等 shim 顺风车) | `imessage-bridge/MAINTENANCE.md`(第 1、6、7 节) |
