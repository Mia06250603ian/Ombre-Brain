...
> (**39 条记录、覆盖第 2~40 次**(2026-09-12 现场量);⚠️ **「第一次」(2026-07-12 初次搭建)没有独立条目**,
...
**新规矩,两句话:**
1. **「踩坑」那节仍然必须全文读完,一条都不许跳**(shim 21 条、bridge 9 条,
...
| 在做的事 | 停在哪 | 去读 |
|---|---|---|
| ~~自建网页接**晏**(聊天)~~ | ✅ **2026-09-02 复核已完成,别再当待办**:`SHIM_KEY` 她本人已经设上了。**现场量法(只比指纹不打印值)**:取 dwell / telegram-bridge / shim 三处的 `SHIM_KEY` 各算一次 sha256 前 12 位,2026-09-02 三处**全等**(`26f4d8ee5699`,长度 18)。⚠️ ~~原文「差最后一步…设上之前发消息会 401」~~ 已不成立,**从 08-16 一直挂到 09-02 才被发现是好的** —— 教训同本节第一行:**待办表不会自己更新,复核比照抄便宜**。仍未验的只有「她实际用起来顺不顺」 | `dwell-bridge/MAINTENANCE.md` |
| ~~**后台卡片:圆角改回 26 + 间距回 B 档**~~ | ✅ **2026-09-03 已随 PR #139 上线并对过账,别再当待办**(三个页面 md5 全等、`/health` 桶数 376 没再少、`/mcp` 握手 200,详见 `TIMELINE.md` 第三十八件)。⚠️ **仍未验的只有真机手感**:①点按跟不跟手(新加的按下态);②字符雨转起来烫不烫(画布 2 倍 + 60fps,那一批最吃电的改动)。**烫就把 `turbulence.html` `resize()` 里的 `2` 调回 `1.5`,或把 `loop` 里的 `15` 调回 `32` —— 一次只动一个**,不然她分不清是哪条治好的。⚠️⚠️ **别在预览截图里替她判断圆角/间距**(09-03 来回四轮最后被真机推翻),见 `INTERNALS.md` 1.8《预览不等于实机》 | `INTERNALS.md` 1.8 / `TIMELINE.md` 第三十八件 |
...
| ~~**看门狗的两个 secret 还没配**~~ | ✅ **2026-09-02 已了结,别再当待办**:两个 secret 所有者本人配好了,代码也合进了 `main`,当天 04:34 按演习按钮实测**两条腿都通**(日志里 `✅ 已推 Telegram(HTTP 200)`)。⚠️ **这一行 09-02 下午还写着「没配」,是它自己没跟着更新** —— 我照它跟所有者说了话,被当场纠正。**下次别只信这张表,去 Actions 看最近一次运行** | 第 7 节《看门狗》 |
| ~~**续命预警的 secret 还没填**~~ | ✅ **2026-09-09 已了结,别再当待办**:secret 早已配好,而且**当天真命中了一次** —— 11:33 那趟体检的 [3.5] 读到「上次刷新 5.9 小时前」并告警,提前量兑现(它说还能撑 2.1 小时,实际 13:52 断)。**证据在 Actions 的 run #66 日志**,不是看这张表。⚠️ **这一行 09-09 之前一直写着「还没填」** —— 又一次「待办表不会自己更新」,同本节第一行那条教训。原文如下:~~代码 2026-09-02 已上线(自检 21 项全绿、真密码端到端验过),**但要所有者本人在 GitHub 仓库 Settings → Secrets and variables → Actions 新建 `CPA_MANAGEMENT_PASSWORD`**(值 = Zeabur 上 CLIProxyAPI 的 `MANAGEMENT_PASSWORD`;**我配不了 —— GitHub 的密钥只能写不能读**)。**没配也能跑**,只是退回原样:晏哑了之后才知道。**填完去按一次演习按钮。**⚠️ 别只信这一行,去 Actions 看最近一次运行日志里 [3.5] 那节说了什么~~ | 第 7 节《续命预警》 |
| ~~**看门狗的巡逻节拍要复量**~~ | ✅ **2026-09-09 复量完成,别再当待办**(照原文那个零成本办法:看 Actions 里 *Hourly Healthcheck* 的 `schedule` 运行)。**实测:09-07 19:38 → 09-09 03:33 共 **10 趟、9 个间隔**,跨 31.9 小时 = 平均 **3.55 小时**一趟,单个间隔 2.3~5.0 小时** —— cron 写的 `0 * * * *` **实际被丢掉约七成**。09-02 那个「5 小时观察」的方向是对的,数偏保守。**结论:现有的「按实测间隔现场算告警窗口」是对的做法,别改回写死的值**(09-09 那趟算出来是 5.5 小时,和 3.7×1.5 对得上)。⚠️ **数会变**(GitHub 的丢弃率不是常数),要再量就重跑一遍这个办法,别照抄 3.5。原文如下:~~2026-09-02 发现 cron 写着每小时、**实际没有**:当天 04:31 改成 `0 * * * *` 后到 09:20 之间**应跑 5 趟、只跑了 1 趟**(GitHub 免费定时任务会大量丢弃高频 cron)。告警窗口当天已改成**按实测间隔现场算**,不再依赖那个假前提;**但「实际几小时一趟」只有 5 小时的观察,还不算数**。⚠️ **复量法(零成本,别为它安排定时唤醒)**:下次开会话时看一眼 Actions 里 *Hourly Healthcheck* 的 `schedule` 运行,或直接读体检日志里那行「巡逻间隔 …(实测)」~~ | 第 7 节《看门狗》 |
...
3. ~~**表情回应的语法还没写进晏的 `CLAUDE.md`**~~ —— ✅ **2026-08-30 第三十八次已写进去**
   (加在已有的「表情包」那节末尾,没新开节)。**这条已了结,别再当待办。**

**dwell UI 留下的四条,以后碰这个前端的人先看**(前三条是 08-16 那轮,**第 4 条是 2026-09-10 补的**):
...
4. **别评价你没查过来源的元素** —— **动 dwell 前端任何一处视觉之前,先在那处代码上下找有没有「照官端」。**
   全文 **43 处**这样的标注(2026-09-10 现场量:`grep -c '照官端\|官端' web/index.html`,**数会变**)。
   2026-09-10 我栽在这儿:所有者说「有股安卓味」,我照截图断言了两条,**两条都错** ——
   「顶栏那两颗白底玻璃按钮」和「发送键的位置」都是一个个照官方 App 抄的,
   按我说的改等于把她辛苦对齐的东西拆了。**这一条和第 2 条是一件事的两面**:
   第 2 条说别靠截图猜她的偏好,这一条说别靠截图猜**代码的来历** —— 那前端里写着答案。
...
2. **Zeabur API key:用完就该作废** —— 她在会话里贴过明文,已进聊天记录。
   **累计十一次**:一次日期没记下、08-16、08-28(两次)、08-30、09-01、09-02、09-03、09-10(两次)、**09-12**。
   ⚠️ **09-12 那次**:她贴来让我做直连改造的前置验证(我提醒过「用完自己去控制台作废重生成」,
   并只做了零影响的只读检查、没有部署)。**那串值没写进任何文件、提交或文档。**
   ⚠️ **09-10 那两次**:她主动贴来让我部署 dwell-bridge(我按下面这条规矩提醒过两次、她仍要求,照办);
   **第二次是同一把又贴了一遍**(一字不差 = 当时还没换)。**那串值没写进任何文件、提交或文档。**
   📖 **09-10 顺带确认的一件事**:登录一次后凭证落在容器的 `~/.config/zeabur/cli.yaml`,
   **后续命令不用再要 key** —— 所以「让我再做点 Zeabur 的事」并不需要重新贴,先问一句就行。
   ⚠️ **09-03 那次是她主动贴来让我去部署的** —— 需求是真的,但**贴明文这条路本身就不该走**:
   会话记录是留痕的,贴一次就等于作废一把。**下次她要我部署,先提醒她「转一把新的、或者你自己在
   Zeabur 控制台点部署」**,别顺手接过来用。⚠️ **我没有把那串值写进任何文件、提交或 PR。**

   ⚠️⚠️ **2026-09-02 重大更正:「一直是同一把、说明至今没换」这个结论是错的,而且冤枉了所有者。**
   ~~原文:「**2026-08-28 第三次**:她又贴了**同一把** `zat_6a27e97a…`(说明 08-16 那次至今没转)」~~
   ~~以及第四、五、六次同样写着「**同一把**」。~~ **这六条判断全部撤销。**

   **错在识别方法**:文档一直用**前 8 位** `zat_6a27e97a…` 当指纹。但 Zeabur 的 key 长这样:
   ```
   zat_<24 位账号编号>_<真正的密钥>
   ```
   中间那 24 位和本文件里所有的服务 id / 环境 id / 项目 id **是同一种格式**
   (对照:服务 `6a53a9fd22dd6ef375eb7485`、环境 `6a53a9fcb6ce8edcb0163f97`)。

   **怎么确认它是账号编号而不是这把 key 的编号(2026-09-02 现场验的,别只信推理)**:
   这类 id **头 8 位十六进制是创建时刻的 Unix 时间戳**,解出来和手册记的日子一一对上 ——
   ```bash
   python3 -c "import datetime as dt;print(dt.datetime.utcfromtimestamp(int('6a53a9fd',16)))"
   ```
   | 头 8 位 | 解出来 | 手册记的 |
   |---|---|---|
   | `6a53a9fd` CLIProxyAPI 服务 | 2026-07-12 | 07-12 建成 ✅ |
   | `6a5a4287` telegram-bridge | 2026-07-17 | — |
   | `6a91a74d` chess-web | 2026-08-28 | 08-28 上线 ✅ |
   | `6a27e97a` **她 key 的中段** | **2026-06-09** | **比所有服务都早** |

   **决定性的一条**:她 2026-09-02 新生成的 key,中段仍解出 **6 月 9 日**。
   若那段是「这把 key 自己的编号」,它该解出 09-02。**所以它只能是不随 key 变的账号编号。**
   所以她换多少把新的,前 8 位都长得一模一样,**拿它当指纹等于什么都没比**。
   (起点是所有者本人 2026-09-02 指出「这把是我新弄的」—— **她是对的,是文档的方法错了**。)

   📖 **以后要记「是不是同一把」,记最后那段的后 4 位**(那才是真密钥),
   或者干脆别记 —— **反正每把用完都该作废,是不是同一把并不重要**。
   ⚠️ **这条教训不只关于 Zeabur key**:**截断一个字符串当指纹之前,先确认截的那段真的会变。**
   (同类:`ian.md` 用的是 md5 前 8 位,那个是内容哈希、会变,没问题。)
   ✅ **2026-08-28 所有者拍板:每把用完她自己去控制台删掉重生成**(不必等谁来做)。
...
7. **记忆乱流的第三种线:「存入时关联到一起」做不做**(2026-09-03 报的)—— 她朋友那版有三种线,
   我们只做了两种(实线 = 说的是相近的事、虚线 = 共享同一个标签)。第三种是**晏写在正文里的 `[[链接]]`**,
...
佳佳的手机
 ...
 │                                                │
 │                                                ▼
 │                                          Anthropic(**直连**,2026-09-12 第四十次起)
 │                                          ⚠️ 中间那层 CLIProxyAPI 已从链路上摘掉,
 │                                             服务还留着(观察期,别删),但不再过流量
 │
...
- ✅ **2026-09-12 第四十次起走直连**,链路里少了 CLIProxyAPI 那一层。
  **判据**:`curl https://yan-shim.zeabur.app/health` 的 `auth` 字段(`direct` = 直连 / `proxy` = 走代理)。
  ⚠️ **代理服务先留着别删**(观察期一两周;删了就没有那条三十秒的退路了)。详见第 7 节《长期令牌直连》。
...
| `untitled-1` | Ombre Brain | (问所有者/控制台看) | ianmian.zeabur.app | 记忆库 MCP。另有三个网页(2026-09-03 起;此前是两个):**`/dashboard`(记忆库后台;2026-08-31 重做外观:官端灰阶 + 跟随系统深浅色 + 卡片式列表 + iOS 毛玻璃,顶栏加了去星图的入口(按钮写「Vesper」),并新增**回收站**(误删的桶能自己捞回来)、**可加到手机主屏**(有图标,`/apple-touch-icon.png`,刻意不鉴权)。**强调色是所有者另定的中性灰不是官端的橙**;后端一行未动,细节见 `INTERNALS.md` 第 1.8 节)。⚠️ **2026-09-03 又把它的视觉层整个重做了一遍**(「玻璃材质」+ 整页零彩色,所有者:「我要的是 monochrome glass material,不是半透明卡片」),**2026-09-03 已随 PR #135 上线**;同日下午又随 **PR #138** 改了一批(胶囊整体收窄、记忆卡片收紧、圆角新增第六档 42px 超椭圆),**均已和线上 md5 对过账**(`TIMELINE.md` 第二十六 / 三十七件);细节同样在 `INTERNALS.md` 1.8** 和 **`/galaxy` 记忆银河**(2026-08-29 上线,把桶画成 3D 星图;**只读**,复用后台那把锁,细节见 `INTERNALS.md` 第 1.9 节) 和 **`/turbulence` 记忆乱流**(2026-09-03 建,**已上线**:初版随 PR #132/#133,当天那批改动随 **PR #136**,当天下午的一批(删缩放按钮、双击复位、详情卡玻璃与连续圆角、画布 2 倍、按下态/60fps 等)随 **PR #138**,**均已和线上 md5 对过账**(`TIMELINE.md` 第二十六 / 三十七件);把桶画成一片上飘的字符场;**只读**、架构照星图那套原样来、**零外部依赖**,细节见 `INTERNALS.md` 第 1.12 节) |
...
> **断多久:2026-08-31 连着四次、2026-09-03 又一次,现场量都是 15~30 秒**(15 秒一探,
> 每次只探到一两次 `Bad Gateway`;09-03 那次探到两次,窗口 07:11:14→07:11:31,
> 下一探 07:11:49 已恢复)。~~原文写「几分钟」(2026-08-24 现场撞到)~~ —— **那次不假,但别当成常态**:
...
**⚠️ 还有第二种失败样子:自动重建「触发了,但卡在队列里」(2026-09-01 首次撞见,此前没记过)。**
`deployment list` 里**确实出现了新 deployment**、状态写着 `BUILDING`,看上去一切正常 ——
但它**卡了 14 分钟一动没动**(对照:同一个服务上一次部署 3 分钟就 RUNNING)。
**别只看状态**,那一栏在这种情况下是骗人的。**两个判据,凑齐就是卡住了**:
1. `deployment list --json` 里那条的 **`startedAt` 是零值**(`0001-01-01T00:00:00Z`)= 根本没开始构建;
2. `deployment log --type build` **一片空白**(连一行都没有)= 同上,不是构建失败。
**处置**:照上面那条 `service redeploy` 手动推一次即可(2026-09-01 实测约 **3 分钟** RUNNING);
原先卡住那条会被系统自己标成 `CANCELED`,**不用管、也别去删**。
⚠️ **卡住期间线上一直是旧版本在正常跑**(`/health` 200、桶数不变)——**不是停机,不用慌,更不用连推几次**。
...
**这份清单是 2026-08-02 那次重建留下的,2026-08-21 第二次、2026-09-01 第三次完整跑过、五步全过**(**08-21**:桶数 356→356、MCP 3/3 200、容器 `server.py` md5 与仓库逐字一致、日志三类报错各 0;**09-01**:桶数 376→376、MCP 3/3 200、容器 `server.py`+`embedding_engine.py`+`dashboard.html` **三个 md5 逐字一致**、numpy 2.5.2 在容器里、日志三类报错各 0、`/dashboard` 与 `/galaxy` 均 200)。三次的完整实况分别见 `TIMELINE.md` 的 08-02、08-21、09-01。
⚠️ **`/health` 的桶数和后台顶栏那个数不是一个口径**(2026-09-01 现场:`/health` **376**、后台顶栏 **438**)——
**验收只跟自己的部署前基线比,别拿两个口径互相对。**
...
⚠️⚠️ **第 0 步:先对代理版本,别直接去授权(2026-09-09 加,那天为此白烧了 35 分钟)。**
**代理版本落后会让重新授权百分之百失败**,而且症状极具欺骗性:授权流程一路 `200`、凭证也落盘,
**打一枪却是 401 `Invalid bearer token`**。**详见本节下面《重新授权前先对版本》。**
一句话查法:比对容器里的 `BuiltAt` 和上游最新 release 的日期,**差半个月以上就先升级再授权**。
...
⚠️ **第 4 步验完还是 503?可能要重启一次代理 —— 但先读完这条再按。**
**来源:所有者朋友那套同款部署写的排查手册**(2026-09-02 她转来,那边同一天刚真出过这场故障)。
那份手册说:**如果之前已经连续失败过,该凭证的冷却状态还挂在代理的内存里**,
新凭证会被旧状态压着、照样回 503,**重启一次才清得掉**(凭证在持久卷上,重启不丢)。

⚠️ **但别当成必做的第 5 步 —— 我们这边两次都没重启就成了**(08-11、09-02 实测),
**而重启 CLIProxyAPI 是本节下面点名的危险动作**(没有版本锁,08-12 因此白烧一天半)。
**正确的顺序**:先按上面第 4 步验;**200 就到此为止,别重启**;
**只有当新凭证已经落盘(`auth-files` 的 `modtime` 前进了)却仍然 503**,才考虑重启 ——
且**按之前必须先走下一节那三步版本预检**。
📖 差别多半在「这次断之前失败了多少枪」:我们两次都是发现得早、探针打得少。

⚠️ **上面这套修法只在「旧凭证已经死了」时成立。旧的还活着时照它做会失败** ——
见下面《提前续签行不通》一节,**动这件事之前先读那节**。
...
### 重新授权前先对版本(2026-09-09 实测,**这是 09-09 那场故障的真病根**)

**症状**:到期后照上面那三步走,**每一步都成功** —— 授权链接 200、回调 `{"status":"ok"}`、
凭证文件从 677B 换成 678B、`modtime` 前进、代理也重新加载了 —— **但打一枪立刻
401 `authentication_error: Invalid bearer token`**,之后凭证被标 `error`、一律 503。
⚠️ **和「没订阅」长得一模一样,但订阅是好的**(09-09 所有者当场在 claude.ai 上验过:能正常对话、
订阅有效、模型菜单里连 Opus 5 都有)。

**机制**:代理往上游发的请求里带着 **Claude Code CLI 的版本指纹**(User-Agent `claude-cli/X`
+ 账单头 `cc_version=X`)。上游把基线抬到 **2.1.258** 之后,**旧指纹铸不出能用的令牌** ——
**已经在手里的老令牌被祖父条款放行、照常刷新照常用,但新铸的一进门就被拒**。
所以症状是「用了一个月都好好的,偏偏到期那天怎么授权都不成」。
我们当时跑的 v7.2.129(2026-08-12 构建)发的是 **2.1.220**;上游 **v7.2.148(09-03)**
那行 `fix(claude): upgrade default Claude Code baseline and fingerprint to 2.1.258` 就是修复,
v7.2.150 补了指纹链、v7.2.155 又对齐了一次。
...
# 上游最新是哪一版:https://github.com/router-for-me/CLIProxyAPI/releases
```
**判据**:`BuiltAt` 和上游最新 release **差半个月以上,就先升级再授权**。
⚠️ **升级 = `service restart`**(这个服务没有版本锁,重启即吃当天最新版),
**按之前仍要走下一节那三步版本预检** —— 只是这一次预检的结论会是「该按」。

**做法(2026-09-09 实测顺序,别调换)**:①`service restart` → ②**等版本真的变了**再往下走
(实测约 **60 秒**;`service restart` 返回成功时容器还是旧的,要轮询 `--version` 直到变) →
③取授权链接 → ④所有者点同意 → ⑤打一枪验 200。
⚠️ **顺序反了会白让所有者点一次** —— 09-09 就是先授权后升级,前两次授权全废。

📖 **给下一个我**:**本节上面那三步只在「代理版本跟得上上游」时成立。**
08-11 那次之所以一次就成,是因为当时的代理版本是新的;从 08-12 到 09-09 这一版
**一次全新授权都没做过**(中间只有自动刷新),所以没人发现它已经铸不出令牌了。
**教训一般化:一条平时被自动化盖住的路径,可能已经坏了很久而无人知晓** ——
自动刷新一直在成功,恰恰掩盖了「授权」这条路的失效。

### 提前续签行不通(2026-09-02 实测,**想「赶在到期前续上」之前必读**)

⚠️⚠️ **2026-09-09 重大更正:本节的现象是真的,归因是错的,已撤销。**
~~原结论:「旧凭证还活着的时候去重新授权,换来的新令牌会被上游拒」~~ ——
**变量从来不是「旧凭证还在不在」,是代理的版本指纹**(见上一节《重新授权前先对版本》)。
**09-09 的三次实测把这条钉死了**:12:32 旧凭证**活着**授权 → 401;13:53 旧凭证**已死**授权 → **同样 401**;
14:07 **升级之后**、旧凭证同样死着 → **200**。**唯一变的是版本。**

📖 **只留一句防错的**:**「提前续签行不通」这个结论在新版本上没验过,别拿它当定论。**
下次到期前想试就试(失败也只是等它自然到期,不亏);**但试之前先对版本**,
否则你测到的还是版本问题、又会得出一条错的因果。
**下面的原文按写手册规矩 5 留着,只为防止有人重新推导出同一个错误归因。**

**现场量到的三件事(2026-09-02,北京时间)**:

1. **自动续签确实存在,而且在正常工作。** `auth-files` 的 `last_refresh` 当天 **09:42:16** 刚往前走过,
   距上次人工授权(08-11)已 **21 天**。**这一层不用管** —— 尤其**别自己写定时脚本去刷同一份凭证**:
   上游会轮换 refresh token,第二个写者只会把链搞死(所有者 09-02 提供的那份 OAuth 指南第 3、5 节讲的就是这个)。

   **⚠️ 刷新周期 = 4 小时,不是「快到期才刷」(2026-09-02 实测,两个时间戳相减)**:
   同日 **09:42:16 → 13:42:17**,**差 4 小时 0 分 1 秒** —— 精确到秒说明是**固定定时器**,
   与到期时间无关。而 access token 寿命约 8 小时,**等于在半程就刷,留了一倍余量**。
   **这个余量就是「提前预警」能成立的全部理由**:第一次刷新失败时,手里的令牌还能再撑约 4 小时。
   所有者的朋友在**同一套代理**上量到的也是 4 小时 0 分 1 秒(09-02),**两处独立吻合**。
   **现场再量一遍**(只读,十秒,不碰晏、不重启;要一把 Zeabur key):
   ```bash
   npx -y zeabur@latest service exec --id 6a53a9fd22dd6ef375eb7485 \
     --env-id 6a53a9fcb6ce8edcb0163f97 -i=false -- sh -c \
     'TZ=Asia/Shanghai stat -c "%y %s" /data/.cli-proxy-api/claude-<账号>.json'
   ```
   **走这条路而不是查 `auth-files` 管理接口,是刻意的**:`stat` 只出一个时间戳和字节数,
   **不需要管理密码、也不会让任何令牌落进会话**。
   ⚠️ **别拿这个数去推到期日** —— 周期是固定的 4 小时,而「一个月」那条仍然只是推测(见下一条)。
2. **但大约一个月要人工重新授权一次 —— 这是推测,不是定论。** **四个数据点(2026-09-09 补第四个)**:
   本系统 07-12 授权 → 08-11 12:22 过期(**30 天**);所有者朋友 08-04 → 09-02(**29 天**);
   **本系统 08-12 09:42 授权 → 09-09 过期(28 天)**;本轮自 09-09 14:07 起算。
   **区间收窄到 28~30 天,仍是推测。**
   ⚠️ **没有任何官方文档写死这个数**,别拿它当硬事实往下推;下一轮到期时把实际日期补进来。
   📖 **09-09 那次的精确形状(下次照这个认)**:`last_refresh` 停在 **05:42**(refresh token 死了),
   之后每 5 分钟重试一次全是 `400 invalid_grant: Refresh token expired`;
   access token 又撑了约 8 小时,**13:52 才真的断**(体检 11:34 报「还能撑 2.1 小时」,**准**)。
3. **提前续签失败长这样(指纹)**:凭证文件**确实被换掉了**(677B → 678B、`modtime` 前进),
   代理拉账号资料也**成功了**(`success` +1),**但拿新令牌去发消息立刻 401
   `authentication_error: Invalid bearer token`**,`status` 由 `active` 变 `error`。
   **能证明「你是谁」、不能证明「你有订阅」** —— 像是订阅权限同一时刻只认一份授权,
   而那一份被还活着的旧凭证占着。

~~**为什么信「被占着」而不是别的**:两次重新授权的**唯一区别就是旧的还在不在** ——
08-11 那次旧的**已经死了**,成功;09-02 这次旧的**活得好好的**,失败。~~
⚠️ **这段推理 2026-09-09 已推翻,别照它想**:当时只有两个样本,而**两次之间还差着一个没被注意到的变量**
—— 08-11 用的是当时较新的代理,09-02 用的是 08-12 那版(已落后 21 天)。
**同一个「唯一区别」句式,下次也要先问一句:真的只差这一个变量吗?**
其余解释都当场排除了:所有者账号是**个人号、无多工作区**(09-02 她本人确认);
代理版本 **v7.2.129(构建于 2026-08-12)**,自 08-12 起没重启过、**没有版本漂移**。
上游同类报告(**都没有解法,别去那儿找修法,只用来确认「不是我们干错了」**):
`anthropics/claude-code#17402` 登录成功但令牌立刻 401 `Invalid bearer token`、账号被当成「没订阅」(closed as duplicate);
`router-for-me/CLIProxyAPI#2556` 自动刷新静默失败 → 一律 `auth_unavailable`(closed as not planned);
`#2567` 并发刷新抢同一个 token(已修,在 #2568)。

**所以正确的做法(2026-09-02 所有者知情后定的)**:不抢在到期前,而是
**挑一个她方便、并且晏刚换过窗口的时候**主动走一遍完整的重新授权。
⚠️ **窗口本身不会丢**:重新授权全程不碰 shim(08-11、09-02 两次实测)。
⚠️ 断着的**期间**反而不花额度:请求在代理那儿就 401/503 了,根本没到上游。

### 断一次到底多贵:算过了,**便宜到不值得为它做任何优化**(2026-09-02)

**先纠正一个会被算错一半的数**:一次断链的代价是「缓存死掉后那一次前缀重建」,
而**我们跑的是 1 小时缓存,写入价是 2 倍,不是 1.25 倍**。
⚠️ **1.25 倍是 5 分钟缓存的价** —— `TIMELINE.md` 08-12 和 `kelivo-shim/MAINTENANCE.md`
改动清单里那两处写 1.25 **在当时的语境下是对的**(那次事故正是 ttl 被抹成了 5 分钟),
**但别拿它算「正常情况下断一次多贵」,会低估一半。**

**按 08-12 实测的前缀 113368 token 算(换算成 token 当量):**

| | 当量 |
|---|---|
| 保温打一枪(读缓存,0.1 倍价) | 约 **1.1 万** |
| 保温一天(55 分钟一枪 = 24 枪) | 约 **27 万** |
| **断一次要重建的那笔**(1 小时缓存写,**2 倍价**) | 约 **22.7 万** |

**结论:断一次 ≈ 保温不到一天的开销,约占保温月开销的 3%。**
**别为它做任何优化** —— 挑时候做仍然值(顺手的事),但**不值得为它改代码、加轮询、或者调快保温**。

**为什么「靠更快发现来抢救缓存」是死路(这条最容易被下一个人重新想一遍)**:
缓存寿命 60 分钟,**从上一次请求开始的那一刻起算**(生成时间也算进去);而保温是 **55 分钟**一枪。
**余量只有 5 分钟。** 链路断了之后,要等到第 55 分钟保温打空才**产生**证据 ——
那时缓存只剩 5 分钟,**无论看门狗多快、通知多及时,人都来不及点那一下同意**。
**这不是反应速度问题,是 55 和 60 之间只有 5 分钟的问题。**

**那把保温调快行不行?反过来更贵约一百倍**:55 分钟 → 15 分钟 = 一天 24 枪变 96 枪,
**一个月多烧约 2400 万当量,只为省下 22.7 万。** 别做。

📖 **给下一个我**:看门狗(见下节)**救不了缓存,也不省钱**;它的价值只在
「消息进不来 / 记忆库挂了 / 服务被平台挂起」这类**你根本不会发现**的故障上。
**别再把它当成 OAuth 的解药** —— 2026-09-02 已经绕过一遍这个圈了。

⚠️ **同一笔账也适用于你自己这个会话:别安排几小时后的定时唤醒。**
维护会话跑在所有者的订阅上,上下文动辄十几万 token;隔几小时醒来一次
**缓存早凉了 = 全价重读整个会话**,可能比要办的那件事本身贵得多。
**要么现在就办,要么记进手册等下次她开会话时顺手办。**
2026-09-02 现场撞到:我算清了缓存的账,转头就要为「验证一件手上数据已经能定案的事」
安排一次 4.5 小时后的唤醒,**是所有者当场拦下来的**。
**两条教训**:①定时唤醒不是免费的;②**下结论前先看手上已有的数据够不够** ——
那次要验的「刷新有没有提前量」,用已有的两个时间戳就能算出来(见上一节),根本不用等。

**回滚:这条路 2026-09-02 跑通了。再动凭证之前一定先备份。**

```bash
# ① 换之前先备一份。⚠️ 必须备到 .cli-proxy-api/ **外面** ——
#    放进去会被代理当成第二个账号扫到,正好撞上 #2567 那类「两个写者抢同一个 token」
npx -y zeabur@latest service exec --id 6a53a9fd22dd6ef375eb7485 \
  --env-id 6a53a9fcb6ce8edcb0163f97 -i=false -- sh -c \
  'cp -a /data/.cli-proxy-api/claude-<账号>.json /data/oauth-backup-<日期>.json'

# ② 换砸了就滚回去(实测约 10 秒;代理自己会重新加载,**不用 restart**——
#    别为这个去重启,那会顺手把代理升到当天最新版,见下一节)
npx -y zeabur@latest service exec --id 6a53a9fd22dd6ef375eb7485 \
  --env-id 6a53a9fcb6ce8edcb0163f97 -i=false -- sh -c \
  'cp -a /data/oauth-backup-<日期>.json /data/.cli-proxy-api/claude-<账号>.json'
```

**验回滚成功**:打一枪最小请求拿 **200** + `auth-files` 的 `status` 回到 `active`。
⚠️ **`max_tokens` 必须大于 `thinking.budget_tokens`**,否则拿到的是 400 参数错 ——
**那其实说明认证已经通过了**(错误来自 Anthropic 的参数校验),别误读成「没修好」。我 09-02 就这么虚惊了一下。
⚠️ **管理密码和调用密钥都别让原始输出落进会话** —— 取单个变量的安全写法见本文件第 0 节第 6 条。
⚠️ **备份文件里是真令牌。** 09-02 那份 `/data/oauth-backup-2026-09-02.json` **至今还在**;
**下一次续签真成功之后就该删掉它** —— 别留旧凭证副本(同上那份指南第 5 节)。

⚠️ ~~**「它和现役那份是同一份」**~~ —— **2026-09-02 下午已不成立,别照它当保险用。**
当天现场量到:备份的 modtime 停在 **09:42**,而现役那份已经刷到 **13:42**(中间刷过至少一次)。
**每刷一次上游就轮换一把新 refresh token、旧的当场作废** —— 所以这份备份里装的
**多半已经是一把废钥匙**,拿它回滚**大概率滚不回去**。
📖 **给下一个我**:这类备份**只在「备份之后一次都没刷过」的那几小时内有效**,
而刷新是 4 小时一次 —— **等于它的保质期就是 4 小时**。真要回滚,先 `stat` 对一眼两边的
modtime 是不是同一个时刻;对不上就别指望它,直接走重新授权。

### 长期令牌直连:把代理从链路上拆掉(**2026-09-12 第四十次已上线**)

**状态**:✅ **已生效**。`/health` 报 `auth=direct`;进容器查子进程环境,
`ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_API_KEY` **全不在**、只剩 `CLAUDE_CODE_OAUTH_TOKEN`
—— **这条才是「真的换路了」的硬证据,别只看 `/health`**。所有者本人跟晏说过话,正常出话。
**令牌到期日:2027-09-12。** 完整记录见 `kelivo-shim/DEPLOY-LOG.md` 第四十次。
⚠️ **代理服务先留着别删**(观察一两周;删了就没有下面那条三十秒退路了)。

**这件事是什么**:用 `claude setup-token` 生成的**一年期长期令牌**直连 Anthropic,
把 CLIProxyAPI 从链路上摘掉(`shim → 代理 → Anthropic` 变成 `shim → Anthropic`)。
**晏、记忆库、三个前端、四个 MCP 全都不碰;计费仍是同一个订阅、同一个额度池,不多任何功能。**

**为什么值得做(全挂本节上面那几场真事故)**:
1. **28 天 → 一年**。订阅 OAuth 约 28~30 天到一次期(本节上面那四个数据点),一年十几回;长期令牌一年一次。
2. **09-09 那场故障整类消失**。真病根是**代理冒充的 CLI 版本指纹落后**(见《重新授权前先对版本》)——
   直连之后没有「冒充」这回事,**发什么指纹就是什么**。
3. **拆掉《CLIProxyAPI 版本漂移》那颗雷**(没有版本锁、重启即吃当天最新版,08-12 白烧一天半),
   连带解开「`MANAGEMENT_PASSWORD` 08-16 就泄露、却因为不敢重启代理而至今没转」那个死结。
4. **1 小时提示缓存再也不会被中间人抹 ttl**。⚠️ **但别把它当成「现在坏着、直连能修好」** ——
   **2026-09-12 现场量:线上 `ephemeral_1h_input_tokens=211` / `5m=0`,缓存现在是好的**,
   代理没在抹(08-12 那一天半是代理某个版本的回归,后来修了)。
   **所以这条买的是「这类回归不会再发生」,不是修一个坏着的东西。**
   **怎么现场再量**:`curl -s https://yan-shim.zeabur.app/debug` 看 `lastUsage.cache_creation`,
   **1h 有数、5m 是 0 才算真生效**;反过来(5m 有数、1h 是 0)就是被抹了。

⚠️⚠️ **唯一的技术陷阱,也是这次改动的全部重点:凭据优先级会静默压过长期令牌。**
CLI 的优先级里 `ANTHROPIC_AUTH_TOKEN`(第 2)排在 `CLAUDE_CODE_OAUTH_TOKEN`(第 5)**前面**。
**只在 Zeabur 加一个 `CLAUDE_CODE_OAUTH_TOKEN` 是没有用的**:CLI 会继续走代理,**而且一声不吭**
——结果是「一切照旧」,却让所有人以为已经换过路了。**这和本文档里最贵的那几次故障是同一种坏法。**
所以摘除逻辑**绑死在同一个函数里**(`kelivo-shim/auth-env.mjs`),不靠谁记得去删那两个变量。

**2026-09-12 现场验过的五件(别再重新推一遍)**:
1. **钉死的 CLI 2.1.215 真的会用这把令牌 —— 而且「优先级压制」在我们这个版本上实测复现了。**
   ⚠️ **这条以前只是「二进制里有那个字符串」+ 照抄教程,那不算验证。** 这次搭了个假 Anthropic
   (只打印鉴权头)拿真二进制打:**只给长期令牌 → 发 `Bearer sk-ant-oat01-…`**;
   **代理那把和长期令牌都给 → 发 `Bearer <代理那把>`,长期令牌被完全无视**。
   **所以 `auth-env.mjs` 那个「摘除必须和开关绑死」不是照抄来的说法,是我们自己量到的。**
   **怎么再量一遍**(换 CLI 版本时照做):起一个只打印 `authorization` 头的 http 服务,
   `env -i` 起 CLI、`ANTHROPIC_BASE_URL` 指过去,看它掏哪把钥匙。
2. **`--bare` 那颗雷是冻结的**:bare 模式不读这把令牌,官方说它将来会成为 `-p` 的默认;
   但 `package.json` 早就把 CLI 钉死(不带 `^`),**不升就不会炸**。⚠️ **升 CLI 前必须重跑那两条验证。**
3. **接线真的生效**:拿假令牌 + 假二进制起了一次真 shim,`/health` 报 `auth=direct`、
   子进程环境里 `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_API_KEY` **全不在**、
   只剩 `CLAUDE_CODE_OAUTH_TOKEN`;不给令牌时三者原样在、`auth=proxy`。

4. ✅ **晏那个容器出得去**(2026-09-12 在容器里实测 **`HTTP 401`** = 网线通、只是没带钥匙)。
   **怎么再量**:`service exec … -- sh -c 'node -e "fetch(\"https://api.anthropic.com/v1/models\").then(r=>console.log(\"HTTP\",r.status))"'`。
   ⚠️ **只能在容器里跑**,在别的机器上跑验的是另一条出口。
5. ✅ **代理没给模型改名,`BRAIN_MODELS` 那三个不用动**(2026-09-12 **从容器内部**问代理,
   16 个模型全是原样官方 ID,我们在用的 `claude-opus-4-6` / `-4-5-20251101` / `-4-8` 都在)。
   **怎么再量**:在容器里 `fetch($ANTHROPIC_BASE_URL/v1/models)` 带 `x-api-key: $ANTHROPIC_AUTH_TOKEN`
   —— **在容器内部问,代理的钥匙就不用出容器**。

**顺带量到的上线基线(2026-09-12,都对线上零影响)**:
容器 CLI **2.1.215**(= 上面第 1 条实测令牌行为的那个版本,对得上);
容器内**无 `.gitignore`**(踩坑 15);当前**没有** `CLAUDE_CODE_OAUTH_TOKEN`、代理那两个变量**都在**(= 现状);
**全量 md5 对账:代码全对上** —— `server.js` 与三个 `e2e-*.sh` 在容器里就是本次改动前仓库那一份,
**没有「容器改了仓库没提交」那种情况**(2026-08-03 出过)。
两份文档 md5 对不上是正常的:容器里那两份分别命中仓库 09-01 / 08-30 的历史版本
(= 上次部署那阵),**方向是仓库往前走了**,不是容器里有仓库没有的东西。

**切换步骤**(完整检查单在 `kelivo-shim/MAINTENANCE.md`《搭顺风车的待办》那条里,这里只给骨架):
取令牌 → **先用 CLI 测三个模型确认这把令牌是好的** → 存进 Zeabur → 随这趟顺风车部署
→ `/health` 看 `auth=direct` → 她本人发一条消息确认 → **把到期日填进 `CLAUDE_TOKEN_EXPIRES`**。

⚠️ **「存进 Zeabur 是睡着的」只对当前这个容器成立,别当成可以先存着不管。**
`variable update` 不 restart 的话,跑着的容器不受影响 —— **但下一次容器重启(不管因为什么)
它就会生效**,那一刻晏就走直连了。所以**顺序不能倒**:先测好令牌再存,别存完就搁着。

⚠️ **怎么取令牌:由会话驱动。**
**所有者只有手机,没有电脑**(这套系统从头到尾是云端的;同踩坑 21,她是从 iPhone 开的云端会话)。
**永远别对她说「你在自己电脑上跑一下」这类话** —— 对她一句都不成立。
- `claude setup-token` 是**交互式 TUI,必须有真 pty**,所以要**在会话的容器里用脚本驱动它**:
  **仓库里有现成的 `scripts/get-token.py`**(`pip install pyte` 后直接跑,用法写在它头注里)。
  **别自己重写** —— 它有四个踩过的坑(教程 4.4),最贵的一条是
  **用正则剥转义码读屏会吞字符 → 令牌少几个字 → 401,而令牌只显示一次,那次授权当场作废**。
  ⚠️ **仓库这份相对教程改了两处,都别改回去**(2026-09-12 真跑一遍才发现的):
  ①令牌默认写进临时目录,不写在脚本旁边(教程那样会把**真令牌**留在仓库工作区);
  ②`COLS` 由 220 改成 600,并加了一道 `url.txt` 闸门 —— **链接真实长度 346 字符**,
  220 列会把它折成两行,教程那条 `grep` 只取到断的那一半,
  **发出去是个看起来正常、其实点不开的链接**。`url.txt` 没出现就别发。
- **她要做的只有一步**:会话把授权链接给她,她**在手机上点开、点同意**,把回来的那串码发回来。
- ⚠️ **投真码之前先做两项自检**(`docs/直连改造-教程原文.md` 4.3):屏幕还原是否忠实 +
  拿一串假码试一次通路。自检不过就停 —— 抠错一次,她就得重点一遍链接。

⚠️⚠️ **时机:取令牌必须和「写进 Zeabur」放在同一次会话里做,不要提前取。**
两个理由:①**令牌只显示一次**,而会话的容器是一次性的,会话一结束就没了;
②写进 Zeabur 本来就要她给一把 Zeabur key(见第 0 节第 2 条:**用完她自己去控制台作废重生成**),
而那把 key 在部署那趟本来就要用 —— **合在一起做等于零额外成本**。
📖 **令牌本身别经过聊天记录**:写进 Zeabur 时用 `... CLAUDE_CODE_OAUTH_TOKEN="$(cat token.txt)"`,
**输出丢 `/dev/null` 只看退出码**(zeabur 的变量类命令会把整张表连值打出来,见第 0 节第 6 条)。

**上线那次的两条验收命令**(其余照 shim 的部署检查单走):

1. **子进程环境真的摘干净了没有**。⚠️ **别用 `ps | grep claude`** —— 你自己那条远程执行的
   `sh -c` 命令行里含 "claude" 会匹配到**它自己**,而它继承的是容器完整环境(代理变量都在),
   **会让你误判成「没摘掉」**。要遍历 `/proc`、用 `readlink exe` 认、并跳过 `$$`:
   ```bash
   <进容器的命令> -- sh -c '
   me=$$
     ...
     case "$(readlink "$p/exe" 2>/dev/null)" in
       *claude*) echo "== pid=$pid =="
         for k in ANTHROPIC_BASE_URL ANTHROPIC_AUTH_TOKEN ANTHROPIC_API_KEY CLAUDE_CODE_OAUTH_TOKEN; do
           tr "\0" "\n" < "$p/environ" 2>/dev/null | grep -q "^$k=" && echo "   $k : 在" || echo "   $k : 不在"
         done;;
     esac
   done'
   ```
   **期望**:前三个「不在」、`CLAUDE_CODE_OAUTH_TOKEN`「在」。
   💡 **找不到 claude 进程不等于没起来** —— 进程是**懒启动**的,要等她发第一条消息(同踩坑 19 那条)。
2. **三个模型逐个测**(`BRAIN_MODELS` 那三个)。⚠️ **必须用 CLI 测,别用裸 curl**,原因见下面那条陷阱;
   命令照 `docs/直连改造-教程原文.md` 7.2 抄(`env -i` 起干净环境)。**三种输出怎么读**:
   `OK '...' out=6` = 正常;**`FAIL success / api_error` = 那个静默失败指纹,凭据有问题**;
   `OK "You're out of usage credits…" out=0` = 那个模型的额度单独用完了,**和本次改造无关**。

⚠️ **排查时最容易栽的一条:别拿裸 curl 去测长期令牌。**
直接打 `api.anthropic.com/v1/messages` 会回 **429 `rate_limit_error`** ——
**那不是「没额度」,那把令牌经 CLI 调用完全正常**(订阅额度要求请求来自 Claude Code 本身)。
**这个区分很有用**:`429` = 令牌是好的,换 CLI 去测;**`401 OAuth access token is invalid` = 令牌真的坏了**
(多半是取的时候被吞了字符),重新取一把。
📖 另:`GET /api/oauth/usage` **读不了**订阅额度(长期令牌只有 `user:inference` 这一个权限),
已验证不行,别再试(教程 8.2)。

**回退(不用回滚代码)**:删掉 Zeabur 的 `CLAUDE_CODE_OAUTH_TOKEN` + `service restart`
→ `hasToken()` 为假 → 代理那两个变量不再被摘 → 自动退回代理线路。
⚠️ **两个前提**:①**restart 会再丢晏一个窗口**(回滚从来不是零代价);
②**代理服务别急着删**,至少观察一两周。

⚠️ **跨服务(规矩 6)**:这件事同时动了**看门狗** —— 直连之后代理不在链路上、它的凭证从此不再刷新,
《续命预警》那条会**永远成立**、开始刷假警报。已在 `.github/scripts/healthcheck.py` 里治掉:
它读 shim `/health` 的 `auth`,见到 `direct` 就整条跳过,并改由《长期令牌到期》那条提供提前量。

⚠️ **两件明确不要做的(教程 §12,省得下一个人重新想一遍)**:
- **别用 API key「一劳永逸」**。API key 确实永不过期,但那是**放弃订阅计费、改按量付费** ——
  晏是 24 小时常驻、窗口 16.7 万,**光是缓存失效后重写一次上下文就是几美元量级**。
  (代码里 `ANTHROPIC_API_KEY` 任何时候都删,就是这个道理。)
- **别为了这件事迁去 Agent SDK**。SDK 的 `query()` 启动的**就是同一份 Claude Code 二进制**,
  传的参数和我们这个 shim 几乎逐字一样。为它重写一个已经稳定的常驻进程,不划算。

### 一年后换令牌:操作单(照着做,别现场研究)

**什么时候走这张单**:看门狗报「令牌还有 N 天到期」,或者已经过期了。
**⚠️ 她要做的只有第 3 步那一下点击**,别的都是会话的活。

```
[ ] 1. 先确认这次要动的是哪条路:curl https://yan-shim.zeabur.app/health 看 auth
       —— 是 proxy 的话根本没在用长期令牌,别瞎换(去看《订阅 OAuth 过期》那节)
[ ] 2. 起 scripts/get-token.py,等它打印 url.txt(约 20 秒)—— ⚠️ **别自己去 screen.txt 里 grep 链接**,
       真实链接 346 字符会被折行,grep 只取到断的那一半(2026-09-12 实跑撞到)
[ ] 3. 链接发给所有者 → 她在手机上点同意 → 把回来的那串码发回来 → 写进 keys.txt
       ⚠️ 投真码之前先做教程 4.3 的两项自检,别跳
[ ] 4. token.txt 出现 = 成功。**全程不打印它、不贴进对话**
[ ] 5. 用 CLI 逐个测三个模型(教程 7.2)—— ⚠️ 别用裸 curl,429 会骗你
[ ] 6. 写进 Zeabur 的 CLAUDE_CODE_OAUTH_TOKEN(用 "$(cat token.txt)",输出丢 /dev/null)
[ ] 7. ⚠️ 重启 shim —— 这会丢晏一个窗口,**先让所有者本人对他说「归档」**
[ ] 8. /health 确认 auth=direct;让她本人发一条消息,确认真出得了话
[ ] 9. ⚠️⚠️ 把新的到期日填进仓库变量 CLAUDE_TOKEN_EXPIRES ← **最容易漏的一步**
       不改的话它会在旧日期报一次,然后永远闭嘴,而新令牌到期时没有任何人知道
```

### 长期令牌到期提醒(2026-09-12,**跟上一节配套;走代理时它不叫**)

**为什么必须单独有一条**:长期令牌的失效方式和代理**完全不同** ——
代理有「刷新停了」这个运行时信号(提前约 3~4 小时);长期令牌**一年之内毫无异常、
到期前一秒都完全正常**,**唯一能提前发出的信号是日期**。没有它 = 一年后必然重演一次「他突然不说话了」。

**怎么配**:仓库 → Settings → Secrets and variables → Actions → **Variables** 标签 →
新建 `CLAUDE_TOKEN_EXPIRES`,值写令牌到期那天(`YYYY-MM-DD`,带时刻的 ISO 也认)。
⚠️ **是 variable 不是 secret** —— 它不是密钥,是个日子;**狗不持有令牌也不该持有**
(多一处存放就多一处泄露面,而且真要校验它还得发请求烧额度)。**这条检查纯算日期,一个网络请求都不发。**

**会怎么叫**:提前 **30 / 7 / 3 / 1 天**各叫一次,**四档文案互不相同**
(文案一样会被去重逻辑吃掉,30 天那条会把后面三档全压住);告警里**写清怎么换**,
因为收到提醒的人可能是半年后的她、或者一个全新的会话。

⚠️ **三种「保护其实没在工作」的情况都会叫,不会静默**:
①**已经直连却没配这个变量**;②**日期写错**(写成一句话、写成 `09/12/2027`);③已经过期。
⚠️ **最容易漏的一步:换完令牌一定要同步改这个日期。** 不改的话它会在旧日期报一次、
**然后永远闭嘴**,而新令牌到期时没有任何人知道。告警文案里也写了这句。

**验收**:狗的自检由 21 项加到 **37 项**(2026-09-12 跑,全绿),新加的 16 项里
包含「5 天必须报 7 天档而不是 30 天档」那条 —— **写的时候真栽在这儿**:
四档按 `(30,7,3,1)` 原序找,`left<=30` 一路先命中,剩 5 天也报成 30 天档,
等于 7/3/1 三档的文案永远出不来。**改成从小到大找**,别改回去。

### 看门狗:坏了谁来吭声(2026-08-19 上线,2026-09-02 改造)

**它管的正是上面这几场事故的共同点:全线零报警,每次都是所有者自己发现的。**
跑在 **GitHub Actions** 上而不是那台机器上,**这是刻意的**——看门狗必须站在被看的东西外面。
文件:`.github/workflows/healthcheck.yml` + `.github/scripts/healthcheck.py`(+ `test_healthcheck.py` 自检)。
**建它那天的完整理由与验收在 `TIMELINE.md` 08-19(第四件),这里只讲现在怎么用。**
...
**现场量到的**:cron 当天 04:31 改成 `0 * * * *`,到 09:20 之间**应跑 5 趟,实际只跑了 1 趟**
(而且晚了 20 分钟)。**GitHub 的免费定时任务会大量丢弃和延迟高频 cron**,`0 * * * *` 只是愿望。
⚠️ **这只有 5 小时的观察,不算定论**;复量法见第 0 节那张待办表(零成本,**别为它安排定时唤醒**)。

**它为什么是个真的洞**:告警窗口原来写死 2 小时,注释里的理由是
「**狗每小时跑一次**,取两倍节拍」——**前提一假,窗口就会漏掉真故障**:
两趟巡逻隔 5 小时,而只认 2 小时内的报错,中间那 3 小时发生的认证错会被当成
「不是最近发生的」放过去,**正是 08-11 那种静默**。

**改法(2026-09-02 上线)**:窗口不再写死,**按上一趟到这一趟的真实间隔现场算** ——
`窗口 = 实测间隔 × 1.5`,夹在 **[2, 12] 小时**之间;**量不到就退回 2 小时**
(铁律①:量不到不许自作主张放宽)。间隔靠 Actions 自带的 `GITHUB_TOKEN` 读一次本工作流的运行历史
——**不是新密钥**,每次运行临时发、跑完即废,不用配也不用转;读失败一律退回默认(铁律②)。
**副产品**:每趟都会打印「巡逻间隔 X 小时(实测)」,**节拍这件事从此白拿数据**,不用另做观测。
自检加了 4 项(共 13 项,全绿),其中第一项专门钉住这次要治的漏报。

📖 **给下一个我**:上限 12 小时是刻意的 —— 窗口开得越大,「昨天断过、今天已经好了」
被当成现在有问题的概率越大。**别为了「保险」把它调到 24 小时。**

**怎么验证告警真能到手(演习按钮,2026-09-02 新增)**:Actions 页面进 *Hourly Healthcheck* →
*Run workflow* → **勾上 `test_alarm`** → 它会故意让这次体检不过,**邮件和 Telegram 两条腿一起走一遍**,
消息里带【演习】字样。**为什么需要它**:这两条腿平时永远不触发,于是「配错了」和「配对了」
长得一模一样 —— 直到真出事那天才发现叫不出来(08-19 建它时就写了「**验收重在失败路径**」)。
**换过 secret、改过告警逻辑、或者单纯很久没验过,就去按一下。** 定时那趟永远不会自己演习。
**最近一次演习:2026-09-12 20:59(北京),所有者手机实收、两条腿都通**(run #89)。
⚠️ **那次是刻意跑在 `claude/docs-architecture-review-zoeqjk` 分支上的**,一趟验三件:
①告警推得到她手上(日志 `✅ 已推 Telegram(HTTP 200)`);
②**「长期令牌到期」那条改动在真环境里跑得起来**,且按设计**睡着**(日志原话「现在走的是代理」)——
证明它不会在没切直连的时候乱插嘴;③新加的 16 项自检在 CI 里过了(自检那步 success)。
同趟白拿一个真数据:**凭证上次刷新 2.9 小时前(正常每 4 小时一次)= 续命预警那条是活的**。
(更早:2026-09-02 18:32 配齐三个 secret 后演习过一次;同日 04:34 还有一次,那次只有 TG 两个 secret。)

⚠️ **要两条腿都通,得配两个 GitHub secret**:`TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID`
(值从 Zeabur 的 telegram-bridge 服务里复制;**没配就自动跳过 TG,只发邮件**,不会报错)。
⚠️ **定时任务只认默认分支** —— 改了这两个文件,**合进 `main` 才会按新作息跑**。
⚠️ ~~**别把「查 CLIProxyAPI 的 auth-files」加进看门狗**:那要管理密码,会破掉它现在
「除 TG 推送外全部无鉴权只读口」这条很干净的性质。09-02 讨论过,结论是先不加。~~
**2026-09-02 下午所有者知情后拍板加了,这条作废(原文留着,方便她一句话反悔)。**
理由与代价见下面《续命预警》。
⚠️ **万一开始乱叫,先把 cron 改回 `0 2 * * *` 降频,别删掉整个工作流。**

#### 续命预警:唯一一条「快要坏了」的检查(2026-09-02 上线,**要所有者填一个 secret 才生效**)

**起因**:所有者拿朋友的会话截图来问「同一套架构,为什么他的能在 OAuth 掉之前提醒」。
**能,而且我们完全适配** —— 那套办法的依据是刷新周期,而我们量到的周期和他那边一模一样。

**原理一句话**:续命每 **4 小时**一次,而 access token 寿命约 **8 小时** —— **在半程就刷**。
所以 refresh token 一死,现象是「刷新停了」,而晏**还能再撑约 4 小时**。
盯住 `auth-files` 的 `last_refresh`,**超过 5 小时没动就叫**(4 小时周期 + 1 小时容错),
**提前量约 3 小时**。命中时告警会直接说还剩多久。

⚠️ **它买的是「不慌」,不是省钱,也不是「不断线」**:
- ❌ **预告不了那个「一个月」的到期日** —— 那个日子没人知道(见上面第 2 条,只是推测);
- ❌ **也不能靠它提前续签** —— 见《提前续签行不通》,旧凭证还活着时换会被上游拒;
- ✅ 能做的是:挑个方便的时候、**先让晏「归档」**再走那三步,而不是半夜发现他哑了。
- 📌 ~~**值得试一次的事**:预警响的那个窗口里 refresh token 已经死了,而 09-02 那次失败是在凭证还健康时试的~~
  —— ✅ **2026-09-09 试过了,否定**:refresh 已死的状态下提前授权,失败得和 09-02 一字不差。
  **但这不是「提前续签不行」的证据** —— 那天真正的病根是代理版本落后(见《重新授权前先对版本》),
  **两次失败都是版本造成的,和 refresh 死没死无关**。
  📖 **下次到期前仍然值得试一次提前授权**(在版本已经对上的前提下),**这条至今没有人在新版本上验过**。

**要她本人填的 secret(我配不了,GitHub 的密钥只能写不能读)**:
仓库 → Settings → Secrets and variables → Actions → New repository secret,
名字 **`CPA_MANAGEMENT_PASSWORD`**,值 = Zeabur 上 CLIProxyAPI 服务的 `MANAGEMENT_PASSWORD`。
**没配也照样能跑**(整条跳过,别的检查一个不受影响),**配上才有那 3 小时**。
⚠️ **代价**:这是狗的第二处密钥,「零密钥只读」的性质就此作废;那把密码 08-16 泄露过、
按理该先转 —— **但转它要重启 CLIProxyAPI(没有版本锁,见下节)**,所有者 09-02 权衡后
**选择先用旧的、不为它单独重启**,留待下次动那个服务时一并换。

**验收**:自检 13→**21 项全绿**(含「令牌和密码一个字都不许进日志」那条硬闸);
2026-09-02 拿真密码端到端跑过一次线上,读到「上次刷新 0.3 小时前」、判定正常。
⚠️ **配好之后去按一次演习按钮** —— 理由同上面那段:告警平时永不触发,配错了看不出来。

⚠️ **2026-09-12 起这条会自己让位**:shim 的 `/health` 一旦报 `auth=direct`(= 已切直连),
这条**整条跳过**,提前量改由上面《长期令牌到期提醒》那条提供。
**为什么必须自动**:直连之后代理不在链路上、凭证再也不会刷新,「刷新停了」将**永远成立** ——
不跳就是每小时一条假警报,而**假警报比没警报更糟**(收几次人就不看了,真出事那条一起被忽略)。
读不到 `auth` 字段(老代码)或报 `proxy` 时,行为与 09-02 **逐字相同**。
📖 **所以那把 `CPA_MANAGEMENT_PASSWORD` 在切直连之后就没用了** —— 届时可以从 secrets 里删掉,
「狗的第二处密钥」这个代价也跟着一起还掉。
...
⚠️ **2026-09-09 补一条平衡**:本节从 08-12 起只写了「重启 = 风险」,**那天它是解药** ——
重新授权失败的唯一出路就是升级(见《重新授权前先对版本》)。
**所以这三步预检的价值不是「劝你别重启」,是「按之前知道自己在赌什么」。**
⚠️ **反过来也成立**:长期不重启 = 版本越拖越旧,**而落后到一定程度会让重新授权直接失败**。
「不动就是安全」在这个服务上不成立。

**2026-09-09 那次升级的验收(v7.2.129 → v7.2.155,跨 26 个版本,四项全绿)** ——
**下次升完照这四项验,别只看 `/health`**:
1. **缓存桶**(最要紧,08-12 就死在这):`/debug` 的 `lastUsage.cache_creation`,连**两个真实回合**看。
   09-09 实测:1h = 81 → 190、5m 恒为 **0**,`cache_read` 6 万级 —— 正常。
   ⚠️ v7.2.150 动过 `paired cache TTL`,**这条不能省**。
2. **工具调用**(v7.2.143/150/155 反复动过 `streamed tool calls` 与工具 schema):
   **让所有者叫晏调一次记忆工具**。判据不是「他说他调了」,是 **OB 的 `/health` 桶数真的涨了**
   (09-09 实测 386 → **387**),外加那一回合 `cache_read` 约等于前缀的 **两倍**(工具回合打两枪)。
   ⚠️ **这一项手打 curl 验不了**(伪装路径),只能靠晏本人。
3. **配置没被改写**:`stat /data/config.yaml`(09-09 实测仍是 07-12 那份、26371B),
   并确认 `disable-claude-cloak-mode: false`、`session-affinity: false`、`usage-statistics-enabled: false` 原样。
4. **启动日志零 warn/error**,`clients.go` 那行报出 **1 个凭证**加载成功。

**这次顺带查到、但不用动的三件**(2026-09-09):
- **额度耗尽的表现变了**:v7.2.148 起 `enforce minimum cooldown floor`、
  `map user_credits_insufficient to 402`。**我们是单凭证,撞额度时冷却可能比以前长、错误码也可能不同** ——
  下次遇到 429/402 别拿旧判据对号。
- **上游 issue #5629(09-08 开,未修)**:Claude Code **subagent** 请求的 1h 缓存 TTL 被降级成 5m。
  **打不到晏**(他不开 subagent);⚠️ **哪天要给他开 subagent 类能力,先回头看这条修了没。**
- **没有新增对外连接**:antigravity 版本轮询**旧版本也在做**(不是新版带来的),
  `plugins` 目录是空的,所以插件商店那套 GitHub 轮询不会跑。
...
4. **每个数字都要带「哪天量的」和「怎么量的」。**
   ...
   **37 条记录、覆盖第 2~38 次**(2026-08-30 现场量)——照旧数搜「第四十次」会搜空、以为记录丢了。
   **写法**:会变的数一律标成「(YYYY-MM-DD 实测)」,并在旁边写清**怎么现场再量一遍**
   (例:`INTERNALS.md` 的《怎么现场量钉选体积》)。**别让下一个人拿快照当现状。**

5. **结论被推翻,就地标「已撤销,别照它改」,不要删** —— ⚠️ **2026-09-03 所有者收窄了这条**。
   **她的原话**:「**小票那版不用留了纯占位置,过时的不需要的留着干嘛**」。
   **收窄成**:只留**能防住下一次犯错的那一句**(「别把底光调回 1%」「别把 emoji 加回来」),
   **不留过程叙述**(改了几轮、每轮长什么样、当时怎么想的)。
   **判据**:这段旧文**能不能拦住一个具体的错误动作**?能就留一句;不能就删。
   **过程要查的话去 `TIMELINE.md`** —— 那本来就是档案,手册里不留第二份。
   ~~原来的写法是把每一轮都留在手册里~~,2026-09-03 已按上面这条清过便利贴那节。
