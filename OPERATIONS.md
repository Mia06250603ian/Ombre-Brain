...
| **接出 4.5 / 4.8(多模型)** | **2026-08-23 可行性全部验完,所有者说「方案没问题」,但她拍板暂缓** —— 要等上面那行的观察期走完再加(**理由**:同时换模型的话,他说话调子飘了就分不清是谁干的)。**推荐的做法是「方案 A」:只改 `BRAIN_MODEL` 一个变量 + restart,零代码、不部署,对 4.6 零风险**;她定的硬约束是「**4.6 绝对安全,会影响 4.6 就不做**」。⚠️ **Opus 5 现在接不了**:CLI 2.1.215 不认识它(拿瞎编的模型名做对照实测),要先升 CLI,而 CLI 是所有模型共用的 —— **单独立项,别顺手做** | `docs/多模型接出方案.md` |
