...
2. **Zeabur API key:用完就该作废** —— 她在会话里贴过明文,已进聊天记录。
   **累计十五次**(09-26 **第十五次**:她在问「内存还够吗」时顺手贴来,起初没用它(整机内存用 dwell `/api/health` 零权限量到了),**后来她说「刚才那一把还能用」、要我部署,就用它建了 imessage-bridge 并部署了 shim 第四十三次**,已提醒用完去控制台删;09-23 第十四次:她贴来让我部署第四十二次(顺风车 + 经期挂卷),已提醒用完删掉;09-23 第十三次:她贴来让我部署 5.5,已提醒用完去控制台删掉;**两串值都没写进任何文件、提交或文档**):一次日期没记下、08-16、08-28(两次)、08-30、09-01、09-02、09-03、09-10(两次)、09-12、**09-14**。
