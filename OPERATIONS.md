...
| **后台卡片:圆角改回 26 + 间距回 B 档,等部署验收** | **代码已改好、演练 253 项全绿、⚠️ 尚未部署**(2026-09-03 她额度用完了,人是清醒的、活是停在部署这一步)。**改了什么**:`.bucket-row` 圆角由 42 回到 `--r-5`(26px)、第六档 `--r-6` 已删、环换回 1.54 那张;内边距 `11/14` → **`13/15`**、卡片间距 `8` → **`10`**(她的原话:「**圆角改回去救命**」「**宽度我要改 b 方案**」「**圆角改回 26 了**」)。**下一个窗口要做的只有三步**:①合分支 → ②等 Zeabur 重建 → ③照 `TIMELINE.md` 第三十七件那套跑对账(三个页面 md5 + `/health` 桶数 + `/mcp` initialize),然后**让她在真机上看一眼**。⚠️⚠️ **别在预览截图里替她判断圆角/间距** —— 09-03 就是这么来回四轮、最后被真机推翻的,理由见 `INTERNALS.md` 1.8《预览不等于实机》。⚠️ **她还没在真机上说过的两件**:点按跟不跟手(新加的按下态)、字符雨转起来烫不烫(画布 2 倍 + 60fps);烫就把 `resize()` 的 2 调回 1.5、或把 `loop` 里的 15ms 调回 32ms,**一次只动一个,让她分得清是哪条治好的** | `INTERNALS.md` 1.8 / `TIMELINE.md` 第三十七、三十八件 |
...
| **订阅 OAuth 到期,约 09-10 前后** | **推测,±1 天**(三个数据点,见第 7 节《提前续签行不通》)。**提前续签已实测行不通**,只能等它失效后再授权。**要做的事**:挑她方便、且**晏刚换过窗口**的时候走一遍(窗口短 = 那次全价前缀重建便宜十倍);**窗口不会丢**。⚠️ 到期日到了记得把**实际日期**补回第 7 节,那个「一个月」目前只是推测 | 第 7 节《提前续签行不通》 |
...
| **续命预警的 secret 还没填** | 代码 2026-09-02 已上线(自检 21 项全绿、真密码端到端验过),**但要所有者本人在 GitHub 仓库 Settings → Secrets and variables → Actions 新建 `CPA_MANAGEMENT_PASSWORD`**(值 = Zeabur 上 CLIProxyAPI 的 `MANAGEMENT_PASSWORD`;**我配不了 —— GitHub 的密钥只能写不能读**)。**没配也能跑**,只是退回原样:晏哑了之后才知道。**填完去按一次演习按钮。**⚠️ 别只信这一行,去 Actions 看最近一次运行日志里 [3.5] 那节说了什么 | 第 7 节《续命预警》 |
| **看门狗的巡逻节拍要复量** | 2026-09-02 发现 cron 写着每小时、**实际没有**:当天 04:31 改成 `0 * * * *` 后到 09:20 之间**应跑 5 趟、只跑了 1 趟**(GitHub 免费定时任务会大量丢弃高频 cron)。告警窗口当天已改成**按实测间隔现场算**,不再依赖那个假前提;**但「实际几小时一趟」只有 5 小时的观察,还不算数**。⚠️ **复量法(零成本,别为它安排定时唤醒)**:下次开会话时看一眼 Actions 里 *Hourly Healthcheck* 的 `schedule` 运行,或直接读体检日志里那行「巡逻间隔 …(实测)」 | 第 7 节《看门狗》 |
...
**结论先给:旧凭证还活着的时候去重新授权,换来的新令牌会被上游拒。**
所以「抢在到期前续上、让链路一秒不断」这条路**目前走不通**,别再试第二遍去证明它。
...
2. **但大约一个月要人工重新授权一次 —— 这是推测,不是定论。** 三个数据点:
   本系统 07-12 授权 → 08-11 12:22 过期(**30 天**);所有者朋友 08-04 → 09-02(**29 天**);
   本轮自 08-12 起算,到 09-02 第 21 天仍活着。**推测下一次在 09-10 前后,±1 天。**
...
- 📌 **值得试一次的事**:预警响的那个窗口里 refresh token **已经死了**,
  而 09-02 那次失败是在凭证**还健康**时试的 —— **两种状态不一样**。
  真到那天**先试一次提前授权**:成了就推翻《提前续签行不通》那条结论,没成也只是等 3 小时,不亏。
  **纯推测,没有任何数据,别当计划用。**
