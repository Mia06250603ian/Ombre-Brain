...
4. **1 小时提示缓存不再被中间人抹 ttl**(08-12 那一天半就是被抹了)。⚠️ **这条是推理,尚未实测** ——
   上线后看 `/debug` 的 `lastUsage.cache_creation.ephemeral_1h_input_tokens` 有没有数。
...
取令牌 → 存进 Zeabur(**此时它是「睡着的」,零影响**,因为代理那两个变量还在)→ 随下一趟顺风车部署
