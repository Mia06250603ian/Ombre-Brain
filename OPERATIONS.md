...
- 刻意**不在仓库**的文件(shim 手册「缺的三个文件」一节有取法):
  - `ian.md`(人设本体,私密)——从运行中容器 base64 拷出,当前 **v23**(22228B,md5 `db3204b9…`,
    2026-08-03 第二十四次部署后;287 行。**该次部署当事会话没写手册,是 2026-08-04 会话从容器
    反查补记的,v22→v23 具体改了哪 3 行已无从得知**,详见 shim 手册第二十四次。
    **注意所有者自己的文件名编号一直比本手册多 1**)。v18 起体例改为 `**Part N · 标题**` 粗体行
...
| `CTX_ARCHIVE_EVERY_TOKENS` | 25000 / **5000** | **嫌他归档太勤就调大**;设 `0` 只归一次不再催。调小=压缩时丢的尾巴更短,但更费额度。线上取 5000 是「宁可多存也别被压缩蒸掉」的取向 |
...
⚠️ **部署 shim 前,把容器里的 `CLAUDE.md` 和仓库那份 md5 对一遍**(不只是 ian.md 那三份私密文件)。
CLAUDE.md 虽然入库,但**它是跟着容器走的**:2026-08-03 第二十四次就出现过「容器改了、仓库没提交」,
谁按常规从仓库目录部署都会把那三处改动静默滚回去(踩坑 11)。
**对不上时以容器为准**,拿容器那份当基线改,上线后再补提交进仓库。
