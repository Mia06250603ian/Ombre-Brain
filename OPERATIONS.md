...
| **表情回应(telemood 借鉴的 A)** | **2026-08-28 代码已写完、单测 302 项全绿,⚠️ 尚未部署**(等所有者点头)。他写 `[回应:❤️]` → 桥剥掉标记、`setMessageReaction` 贴到她那条消息上。**只动 bridge,不部署 shim、不丢窗口**;急救开关 `REACTION_ON=0`。⚠️ **教他语法要改晏的 `CLAUDE.md` = 要部署 shim = 丢窗口**,所以照语音功能当初的办法:桥先支持、她在对话里口头教他、永久教学等下次 shim 部署搭顺风车。⚠️ **单测证明不了 Telegram 收不收**(静态检查不是真投递,同 telemood 的告诫),上线后要真打一枪;失败的后果只是表情不出现,正文照发。**部署后记得回来把本行改成已上线,并补 bridge 手册的设计要点/环境变量表** | `telegram-bridge/MAINTENANCE.md`、`TIMELINE.md` 08-28(第二件) |
