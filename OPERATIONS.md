...
| **dwell 前端手感校准(五个数)** | **PR 已开、未合、未部署、未真机确认**:`Mia06250603ian/dwell-on-something` **PR #3**(分支 `claude/ui-feel-calibration`)。五个数是所有者 2026-09-10 自己在校准尺上拉的:正文 16→**15px**、消息间距 18→**16px**、气泡圆角 18→**23px**、输入卡圆角 24→**27px**、发送键赤陶→**近黑**。⚠️ **发送键那条半推翻了 08-16「赤陶统一成一支」**(菊花仍是赤陶)。⚠️ **合之前要她在真机上过一眼**(预览不等于实机)。上线还要走 `dwell-bridge` 第 5 节的 `fetch-frontend.sh` + 部署,**Zeabur 那一步要她本人点**。⚠️ **还有一件没做**:校准尺里思考行和模型胶囊会跟着字号缩,真页面里它们写死 14.5/14px 不缩,**要一起缩得她另外点头** | `dwell-bridge/MAINTENANCE.md`《手感校准》 |
