...
| `untitled-1` | Ombre Brain | (问所有者/控制台看) | ianmian.zeabur.app | 记忆库 MCP。另有两个网页:**`/dashboard`(记忆库后台;2026-08-31 换成 Claude 官端配色 + 跟随系统深浅色 + 卡片式列表,顶栏加了去银河的入口,后端一行未动,细节见 `INTERNALS.md` 第 1.8 节)** 和 **`/galaxy` 记忆银河**(2026-08-29 上线,把桶画成 3D 星图;**只读**,复用后台那把锁,细节见 `INTERNALS.md` 第 1.9 节) |
