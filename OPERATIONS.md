...
| **dwell 前端手感校准(五个数)** | **已合进 main、未部署、未真机确认**(2026-09-10:PR #3 由所有者本人合,`8088ef7`;五个值已在 `origin/main` 核对过)。五个数是她自己在校准尺上拉的:正文 16→**15px**、消息间距 18→**16px**、气泡圆角 18→**23px**、输入卡圆角 24→**27px**、发送键赤陶→**近黑**。⚠️ **发送键那条半推翻了 08-16「赤陶统一成一支」**(菊花仍是赤陶)。**剩两件**:①她真机上还没看过(预览不等于实机);②**没部署,线上还是旧样子** —— 走 `dwell-bridge` 第 5 节,**Zeabur 那一步要她本人点**。⚠️⚠️ **部署时 `fetch-frontend.sh` 别带分支名**:手册第二次部署记录原来钉着 `claude/pill-and-brandline`,那个分支**至今还在远端且不含这五个数**,带了等于白部署(已在 dwell 手册就地标撤销)。⚠️ **还有一件没做**:校准尺里思考行和模型胶囊会跟着字号缩,真页面里它们写死 14.5/14px 不缩,**要一起缩得她另外点头** | `dwell-bridge/MAINTENANCE.md`《手感校准》 |
...
2. **Zeabur API key:用完就该作废** —— 她在会话里贴过明文,已进聊天记录。
   **累计八次**:一次日期没记下、08-16、08-28(两次)、08-30、09-01、09-02、**09-03**。
