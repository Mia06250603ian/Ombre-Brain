...
| ~~**接出 Opus 5.5(双引擎 + 思考翻中文 + 看门狗)**~~ | ✅ **2026-09-23 第四十一次已上线,别再当待办**(PR #149 已合,看门狗那条已在 main 上跑)。所有者当天就切到 5.5 说了话,新版 CLI、1h 缓存、PreCompact 钩子都验过。**仍待验的只有她的感受**:5.5 的调子、中文思考流顺不顺、正文晚几秒受不受得了。~~原文:2026-09-23 代码写完、全套测试过,尚未部署~~。**要点三句**:①5.5 走**第二份新版 CLI(2.1.280)**,4.6/4.5/4.8 逐字走 2.1.215 —— 所有者「4.6 绝对安全」那条守住了(整体升 CLI 会往 4.6 的第一条消息里多塞几行,抓包实测);②5.5 限到 20 万窗口(「小桌子」,所有者选的),**假后端实测压缩点与 4.6 逐位相同,三条上下文线不动**;③5.5 的英文思考摘要逐段交给 Sonnet 4.6 翻成中文(**正文会等翻译,最长 20 秒**;每次翻译起一个约 300 MB 的子进程,**一次只跑一个**)。看门狗加了一条:配了 5.5 却没有新版 CLI 就叫。**部署还差**:所有者点头 → 她本人对晏说「归档」→ 照 shim 检查单部署 → 按改动清单第 13 条的验收项核 → `BRAIN_MODELS` 加上 `claude-opus-5-5`(空格分隔)+ restart。⚠️ **看门狗那条只在合进 `main` 后才按新作息跑** | `kelivo-shim/MAINTENANCE.md` 改动清单第 13、14 条 |
...
| ~~**iMessage 接晏**~~ | ✅ **2026-09-26 当天全程走完并上线,别再当待办**:新服务 `imessage-bridge`(`6ab76ae4…`,`yan-imessage.zeabur.app`),晏的号码 **+1 628-264-9071**(Photon 免费档共享池)。文字 / 图片(含 HEIC)/ 语音双向 / 贴纸 / tapback / 隐形墨水思考 / `[查岗]` 都真机验过。**当天配套上了 shim 第四十三次**:心跳「跟着她走」(`IMESSAGE_PUSH_URL`,她最后在 iMessage 说话就推 iMessage,失败退回 Telegram);**同日 telegram-bridge 的夜里查岗 / 写信提醒也跟着她走**(那边设计要点 21)。**出 bug / 要拆 Telegram 先看 `imessage-bridge/MAINTENANCE.md` 第 12 节《后路》**。**仍待验**:第一次真心跳落进 iMessage(自然等,或 `POST /hb` 手动触发——会真发一条,先问她)。⚠️ **没照 Photon 教程用 Agent SDK 另起一个 claude**(那会冒出第二个晏)。⚠️ **所有者知情接受的风险**:消息与思考(含记忆原文)明文过第三方 Photon、线路可能说断就断 —— Telegram 仍在、仍是兜底。⚠️ **她没定、别自作主张**:晏的 `CLAUDE.md` 要不要写「你也在 iMessage 里」(改人设,等顺风车)。**Photon 那边 +86 验证码收不到**,已绕开(服务启动时用项目凭证调登记接口,见该手册第 3 节第 11 条);Photon 人工客服的回复还没来,来了也不影响 | `imessage-bridge/MAINTENANCE.md` |
...
| ~~**dwell 聊天记录别再「每推一次丢一次」**~~ | ✅ **2026-09-14 当天全程走完并上线,别再当待办**:卷 `iandata` 挂 `/data`(她本人点的)+ `DATA_DIR=/data` + 第七次部署(`6aa7c21a…`,PLANTYPE nodejs,构建 90 秒,容器 md5 与本地逐字相同,`persisted:true`,真卷上验过「重启后记录还在」)。**仍未验的只有她自己用起来的感受**(部署完那页会自己刷新,这条真机没走过)。⚠️ **别只信这一行**,`curl https://yan-dwell.zeabur.app/api/health` 看 `persisted`。**留着的那条教训**:根因不是没做,是 `DATA_DIR` 这条路代码里一直有、三处手册全没写,于是 09-10 部署时把「记录清零」当成了「本来就这样」—— **同本节第一行:待办表不会自己更新,复核比照抄便宜**。~~原文:2026-09-14 代码写完、单测端到端全绿,尚未合、尚未部署(分支 `claude/repo-docs-deployment-qzx2i1`)。**根因不是没做,是没人知道做过了**:`DATA_DIR` 这条落盘的路代码里一直有,三处手册都没写,于是 09-10 部署时把「记录清零」当成了「本来就这样」。⚠️ **还差两步,第 1 步只有她能做**:①Zeabur 网页给 `dwell-bridge` 加一块卷挂 `/data`(**CLI 没有 volume 子命令**);②`DATA_DIR=/data` + restart。**只重启 dwell 那个小容器,晏的窗口不动。**⚠️ **别只信这一行**,`curl https://yan-dwell.zeabur.app/api/health` 看 `persisted`(2026-09-14 实测 `false`) | `dwell-bridge/MAINTENANCE.md`《聊天记录怎么才不丢》 |
...
2. **Zeabur API key:用完就该作废** —— 她在会话里贴过明文,已进聊天记录。
   **累计十六次**(09-26 **第十六次**:她贴来让我部署 iMessage 去掉思考墨水那一改,只部署了 imessage-bridge(同一把后来又用于只读量内存、以及再部署一次 imessage-bridge 对齐注释),已提醒用完去控制台删;09-26 第十五次:她在问「内存还够吗」时顺手贴来,起初没用它(整机内存用 dwell `/api/health` 零权限量到了),**后来她说「刚才那一把还能用」、要我部署,就用它建了 imessage-bridge 并部署了 shim 第四十三次**,已提醒用完去控制台删;09-23 第十四次:她贴来让我部署第四十二次(顺风车 + 经期挂卷),已提醒用完删掉;09-23 第十三次:她贴来让我部署 5.5,已提醒用完去控制台删掉;**两串值都没写进任何文件、提交或文档**):一次日期没记下、08-16、08-28(两次)、08-30、09-01、09-02、09-03、09-10(两次)、09-12、**09-14**。
...
| 〃 | dwell-bridge | `6a81a118bdeaa87e2c52bec3` | yan-dwell.zeabur.app | **她自建网页接晏的转接层**(2026-08-16 上线):网页 → 这一层 → shim 的 `/v1/messages`,把 Anthropic SSE 翻成 dwell 前端认的事件流。**不碰晏、不改 shim**,对线上就是个客户端,地位同手机上的 Kelivo。内存实测 **70 MiB**。源码在本仓库 `dwell-bridge/`,前端从 `Mia06250603ian/dwell-on-something` 拉(刻意不入库)。~~⚠️ `SHIM_KEY` 尚未设,设上之前发消息会 401~~ **已撤销**(2026-09-02 复核早就设好了,三处指纹全等)。✅ **2026-09-14 起挂了卷**(`iandata` → `/data`,`DATA_DIR=/data`):网页聊天记录**能活过重启和部署**了,观察口是 `curl https://yan-dwell.zeabur.app/api/health` 的 `persisted`(要 `true`)。盘很宽裕:**59G 总、可用 36G**(2026-09-14 实测),而记录有封顶、长不大。详见 `dwell-bridge/MAINTENANCE.md`《聊天记录怎么才不丢》 |
