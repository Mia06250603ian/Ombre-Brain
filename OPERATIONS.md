...
⚠️ **怎么取令牌:必须由会话驱动,别叫所有者自己跑(2026-09-12 当场更正)**

~~原文写「令牌不绑机器,在任何能开浏览器的机器上跑一次 `claude setup-token` 即可,
那种用 pty 驱动 TUI 的脚本我们不需要」~~ —— **这句错了,当天被所有者当场纠正:她没有电脑。**
**这套系统从头到尾是云端的,她只有手机**(同踩坑 21:她是从 iPhone 开的云端会话)。
**别再写「你在自己电脑上跑一下」这类话** —— 对她一句都不成立。

**所以取令牌这件事的正确形态**:
- `claude setup-token` 是**交互式 TUI,必须有真 pty**,所以要**在会话的容器里用脚本驱动它**
  (驱动脚本见所有者给的那份《把 Claude Code 订阅从中转代理改成直连》第 4.2 节,
  **直接用那份,别自己重写** —— 它有四个踩过的坑,其中最贵的一条是
  **用正则剥转义码读屏会吞字符 → 令牌少几个字 → 401,而令牌只显示一次,那次授权当场作废**;
  必须用 `pyte` 这类真终端模拟器)。
- **她要做的只有一步**:会话把授权链接给她,她**在手机上点开、点同意**,把回来的那串码发回来。
- ⚠️ **投真码之前先做那份文档 4.3 的两项自检**(屏幕还原是否忠实 + 拿假码试一次通路)。
  自检不过就停 —— 抠错一次,她就得重点一遍链接。
