...
| **长期令牌直连**(把 CLIProxyAPI 从链路上拆掉) | **代码写完了,线上一行没生效**(2026-09-12)。所有者点头做到「写完放着」这一步;部署 shim = 丢窗口,已挂进 `kelivo-shim/MAINTENANCE.md`《搭顺风车的待办》等顺风车。**已验**:钉死的 CLI 2.1.215 支持长期令牌(不用升 CLI)、接线探针两个方向都对、七套单测 + 三套 e2e + 狗的 37 项自检全绿。**上线前那两件 2026-09-12 已验完**:容器能直连(`HTTP 401`)、代理没给模型改名;另加全量 md5 对账(代码全对上)。**现在只差真正上线那一趟。****要她做的**:上线那天点一下会话发给她的授权链接、把回来的码发回去(⚠️ **她只有手机,别叫她自己跑命令**),再把到期日填进仓库变量 `CLAUDE_TOKEN_EXPIRES` | 第 7 节《长期令牌直连》 |
...
- ⚠️ **CLIProxyAPI 那一节将来可能消失**:「用一年期长期令牌直连、把代理拆掉」的代码
  2026-09-12 已进仓库,**但线上一行都还没生效**(等下一趟顺风车)。**上面这张图现在仍然是准的**;
  切换之后图里少一层,判据是 `curl https://yan-shim.zeabur.app/health` 的 `auth` 字段
  (`direct` / `proxy`)。详见第 7 节《长期令牌直连》。
...
### 长期令牌直连:把代理从链路上拆掉(2026-09-12 **代码已写完,尚未上线**)

**状态先说清**:代码在仓库里躺着,**线上一行都还没生效**,挂在
`kelivo-shim/MAINTENANCE.md` 的《搭顺风车的待办》里等下一趟顺风车
(部署 shim = 丢晏一个窗口,不值得为它单开一次)。**所有者 2026-09-12 已点头做到「写完放着」这一步。**
