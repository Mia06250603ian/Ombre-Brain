...
2. **Zeabur API key:用完就该作废** —— 她在会话里贴过明文,已进聊天记录。
   **累计十次**:一次日期没记下、08-16、08-28(两次)、08-30、09-01、09-02、09-03、**09-10(两次)**。
...
**2026-09-12 现场验过的三件(别再重新推一遍)**:
1. **钉死的 CLI 2.1.215 支持长期令牌,不用为这件事升 CLI。** 实测 `claude setup-token --help`
   打得出用法、二进制里 `CLAUDE_CODE_OAUTH_TOKEN` 出现 **72** 次。
   **怎么再量一遍**(换 CLI 版本时照做,量法与警告都在 `auth-env.mjs` 头注)。
...
**还没验、上线前必须先验的两件**(都对线上零影响,可以随时做):
- **晏那个容器出不出得去**:必须**在容器里**跑
  `node -e "fetch('https://api.anthropic.com/v1/models').then(r=>console.log('HTTP',r.status))"`,
  **期望 `HTTP 401`**(401 = 网络通、只是没带钥匙)。⚠️ 在别的机器上跑没有意义,那验的是另一条出口。
- **模型 ID 会不会变**:确认代理没给模型改名(`curl -H "x-api-key: <代理key>" https://miaianhome.zeabur.app/v1/models`)。
  列出来是原样官方 ID 就照抄,`BRAIN_MODELS` 那三个不用动。
