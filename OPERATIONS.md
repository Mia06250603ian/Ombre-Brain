...
| **dwell 换壳(三栏)** | **2026-09-19 已上线**(第十二~十六次部署,两个 PR 都已合):`index.html` 是底部三栏 + 首页卡片流 + 设置页,Chats 点一行用 iframe 装 `chat.html`(聊天页)。**⏳ 还没做完**:日记/待办/日历/日报/共读/健康/仓库/悄悄话**八间屋子的门面还没在新壳里重做**(后端接口一条没动,原样在 dwell 仓库的 `8088ef7`);首页卡片还是写死的假数据;设置里的皮肤/透明度只存浏览器本地。⚠️ **返回条是第二条栏**,要合成一条得动聊天页顶栏(碰「照官端」),**她没点头别动**。⚠️ **字号是按大写字高反推的(±3%),真机截图才能锁死** | `dwell-bridge/MAINTENANCE.md`《换布局》《那套数》 |
