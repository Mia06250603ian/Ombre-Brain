...
> **怎么读这份手册(2026-08-19 起)**:急着干活就**读到第 4 节为止**(**约 44 KB**,2026-09-26 瘦身后实测;~~约 20 KB~~ 是早先的数,瘦身前已涨到 57.6 KB。现场再量:`awk '/^## 5\. /{exit} {print}' OPERATIONS.md | wc -c`),那已经够开工;
...
```
先读仓库根目录的 START-HERE.md(很短,讲清这仓库里躺着十个服务、各看各的手册),
...
⚠️ **2026-08-24 起模型不再是固定的 4.6** —— 她能在 Kelivo 菜单里自己切(4.6 / 4.5 / 4.8,**2026-09-23 起加上 5.5**),
当天已切到 `claude-opus-4-5-20251101`。**2026-09-23 第四十一次起菜单里多了 `claude-opus-5-5`**(走第二份新版 CLI,见 shim 手册改动清单第 13 条),当天她就切过去了。**「他现在跑的是哪个」以 `curl https://yan-shim.zeabur.app/health`
...
| 在做的事 | 停在哪 | 去读 |
|---|---|---|
| 自建网页接**维护 agent** | **2026-09-12 代码写完、全套演练通过,尚未建服务、尚未部署**(检查单在 `agent-bridge/MAINTENANCE.md` 第 6 节,**一步都没走过**)。⚠️ **她当天拍板:等前端优化做完再一起部署** —— 那样第一次上线就带上确认按钮和暂停键,**这是刻意等,不是漏了**。⚠️⚠️ **「必须先升 2C8G」这个前提已撤销,别再照它筹备**:所有者当天定的用法是「一个窗口干完写完手册,下次找下一个窗口」——**那本来就不需要常驻**,开工位才起进程、收工就退。于是**不用升机器、不用重启、晏的窗口不动**。~~原文:方案已成型尚未实施;机器她已拍板升 2C8G,但升级会重启机器、晏的窗口会丢~~ | `agent-bridge/MAINTENANCE.md`(**新的唯一可信手册**)/ `docs/维护Agent接出方案.md`(风险清单仍有效,**只有「常驻」那条前提废了**) |
| **dwell 换壳(三栏)+ Home 重做** | **2026-09-19 已上线**(换壳第十二~**十五**次、Home 重做第十六~十七次,三个 PR 都已合)⚠️ **「第十二~十六次」这个写法 2026-09-19 已纠正**:dwell 手册的部署记录里换壳只到第十五次,**以那份为准**(规矩:单服务细节以专属手册为准):`index.html` 是底部三栏 + 首页卡片流 + 设置页,Chats 点一行用 iframe 装 `chat.html`(聊天页)。**✅ Home 这一屏已照新参考图整屏重做并反复收敛**(2026-09-19 当天部署到第二十二次;排版 / Spring 入场 / PWA 全屏 / 环·爱心·柱子逐个对齐,每次都和容器 md5 逐字对过账)。⏳ **仍未收敛完,明天接着改** —— 所有者额度用完停在第二十二次;**接着做之前先读 `dwell-bridge/MAINTENANCE.md`《怎么对着截图改 UI:2026-09-19 栽出来的七条》**,那七条全是当天返工换来的(其中最要紧的一条:**绝不要拿容器里的渲染当尺子量字号** —— 本机 DejaVu vs 她手机 New York,墨高比 0.880 vs 0.938,照预览调会往反方向走)。**⏳ 还没做完**:日记/待办/日历/日报/共读/健康/仓库/悄悄话**八间屋子的门面还没在新壳里重做**(后端接口一条没动,原样在 dwell 仓库的 `8088ef7`);首页卡片还是写死的假数据;设置里的皮肤/透明度只存浏览器本地。⚠️ **返回条是第二条栏**,要合成一条得动聊天页顶栏(碰「照官端」),**她没点头别动**。⚠️ **仍未真机确认的只剩字重**:容器里 `ui-serif` 落到 DejaVu Serif、**只有两档字重**(实测 400=500、600=700=800),所以那套 400/600/700 在预览里必然塌成两档 —— **别照预览调字重**,详见 dwell 手册那节 | `dwell-bridge/MAINTENANCE.md`《Home 页:怎么按参考图量,量到什么》《换布局》 |
...
**2026-09-26 第二批撤下来的(十二行)**:原文一字未改在 `TIMELINE.md` 末尾「附二」。**「仍待」那半句是原文里还没了结的事,照旧有效。**
...
| dwell 前端手感校准(五个数) | 09-10 | `dwell-bridge/MAINTENANCE.md`《第六次》。⚠️ 重部署时 `fetch-frontend.sh` **别带分支名**;思考行/模型胶囊跟着缩她决定不做,别再捡起来 |
| 订阅 OAuth 到期(真病根是代理版本落后) | 09-09 | 第 7 节《重新授权前先对版本》/ `TIMELINE.md` 09-09。原文推测下一轮到期 **10-07 ~ 10-09**(仍是推测,别当日程;09-12 起晏已直连,这条只关系到留作退路的代理,见第 2 节 CLIProxyAPI 那行) |
| 看门狗的两个 secret | 09-02 | 第 7 节《看门狗》。别只信表,去 Actions 看最近一次运行 |
| 续命预警的 secret | 09-09 | 第 7 节《续命预警》(证据在 Actions run #66 日志) |
| 看门狗巡逻节拍复量 | 09-09 | 第 7 节《看门狗》。实测平均约 3.55 小时一趟(09-09 量);**按实测间隔现场算告警窗口是对的,别改回写死的值** |
...
4. **别评价你没查过来源的元素** —— **动 dwell 前端任何一处视觉之前,先在那处代码上下找有没有「照官端」。**
   ⚠️ **2026-09-19 现场量:那 43 处如今在 `web/chat.html` 里,`index.html` 只剩 2 处** —— 09-19 换壳把聊天页整个搬走了。~~原文的命令量的是 `index.html`~~,照它量会得出「只有 2 处、随便改」的错结论,**正是这一条要防的事**。**现场再量**:`grep -c '照官端\|官端' web/chat.html web/index.html`(**数会变**)。
...
2. **Zeabur API key:用完就该作废** —— 她在会话里贴过明文,已进聊天记录。**累计十六次**(最近一次 09-26;逐次流水与下面几条的来历原文在 `TIMELINE.md` 末尾「附二」)。**几条规矩**:
   ...
   - 📖 登录一次后凭证落在 `~/.config/zeabur/cli.yaml`,同一个容器里**后续命令不用再要 key**(09-10 确认)—— 先问一句,别让她重贴。
   - ⚠️ **别拿 key 的前 8 位当指纹**(2026-09-02 更正,原判「一直是同一把」冤枉了她):格式是 `zat_<24 位账号编号>_<真密钥>`,**中间那段是不随 key 变的账号编号**
     (这类 id 头 8 位十六进制是创建时刻的 Unix 时间戳,她新生成的 key 中段仍解出 06-09)。要记就记最后那段的后 4 位,或者干脆别记。
     **通用教训:截断一个字符串当指纹之前,先确认截的那段真的会变。**
3. ~~**`REPORT_TOKEN` 要转**~~ —— 08-06 截图泄露;**2026-08-28 第二次**被
   ...
   **要换的话必须两边一起改。** ⚠️ **2026-09-26 起是三处**:`imessage-bridge` 也配了同一把(iMessage 里的 `[查岗]` 要用),漏改它 = iMessage 查岗静默失效。
...
佳佳的手机
 ...
 │                                             服务还留着(观察期,别删),但不再过流量
 │
 └─(2026-09-12 新增,**尚未部署**)自建网页还有第二间屋:
      dwell-bridge ──/api/gong──▶ agent-bridge ──▶ 一个临时的 claude 子进程 = **维护工位**
      **和晏完全无关**:另一个容器、另一个进程、另一套权限,开工才起、收工就退。
      详见 `agent-bridge/MAINTENANCE.md`

 └─(2026-09-26 新增**并上线**;~~尚未部署~~ 当天已上线)iPhone「信息」:
      Photon(第三方) ──▶ imessage-bridge ──/v1/messages──▶ kelivo-shim(**同一个晏**,第四扇门,同 Telegram 那样)
      详见 `imessage-bridge/MAINTENANCE.md`
...
| 〃 | **agent-bridge** | **尚未创建**(2026-09-12 代码写完,建服务的命令在 `agent-bridge/MAINTENANCE.md` 第 6 节) | 未分配 | **维护 agent 的工位**:她从自建网页派活,本层起一个临时 `claude` 子进程干活,**随时能叫停插话**,收工即退。**不常驻**(所以不必升 2C8G)。⚠️ 权限闸是「容器里没有 Zeabur key」,动线上要她当场给 |
| 〃 | **imessage-bridge** | `6ab76ae430cd08c7e96bca5d` | yan-imessage.zeabur.app | **iMessage ⇄ shim 的桥**(2026-09-26 上线):照 Telegram 桥的样子,无状态、不挂卷、**不碰晏**。中间过第三方 Photon(免费档、共享号码池,晏的号码 +1 628-264-9071)。线上 `mem.self` 113~129 MiB(2026-09-26,`/health` 读的 RSS)。**心跳的第二个出口**(shim 的 `IMESSAGE_PUSH_URL`) |
| `untitled-1` | Ombre Brain | (问所有者/控制台看) | ianmian.zeabur.app | 记忆库 MCP。另有三个网页:**`/dashboard`**(记忆库后台,含回收站、可加到手机主屏;视觉「玻璃材质、整页零彩色」,**强调色是所有者定的中性灰**)、**`/galaxy`** 记忆银河(3D 星图,只读)、**`/turbulence`** 记忆乱流(字符场,只读、零外部依赖)。三页细节依次见 `INTERNALS.md` 1.8 / 1.9 / 1.12;改版经过见 `TIMELINE.md` 第二十六 / 三十七件(本行原文在 `TIMELINE.md`「附二」) |
...
- **Mia06250603ian/Ombre-Brain**(本仓库,Gitea 备份见 README)。
  **根目录 `START-HERE.md` 是这个仓库的门口地图**(2026-08-21 新增):**十个服务**(八个有代码的目录 + 两个只有手册的;~~五个目录~~ 是 08-21 的数)分别是什么、
  ...
  - `imessage-bridge/` = iMessage 桥源码 + **`MAINTENANCE.md`** + `stickers/`(**telegram-bridge 贴纸的转换副本**,
    那边加新图要在这边重跑 `tools/build-stickers.mjs`)。2026-09-26 新增。
