...
| **dwell 换壳(三栏)+ Home 重做** | **2026-09-19 已上线**(换壳第十二~**十五**次、Home 重做第十六~十七次,三个 PR 都已合)⚠️ **「第十二~十六次」这个写法 2026-09-19 已纠正**:dwell 手册的部署记录里换壳只到第十五次,**以那份为准**(规矩:单服务细节以专属手册为准):`index.html` 是底部三栏 + 首页卡片流 + 设置页,Chats 点一行用 iframe 装 `chat.html`(聊天页)。**✅ Home 这一屏已照新参考图整屏重做**(排版三档字重 / Spring 入场 / PWA 全屏,容器 md5 逐字对过账)。**⏳ 还没做完**:日记/待办/日历/日报/共读/健康/仓库/悄悄话**八间屋子的门面还没在新壳里重做**(后端接口一条没动,原样在 dwell 仓库的 `8088ef7`);首页卡片还是写死的假数据;设置里的皮肤/透明度只存浏览器本地。⚠️ **返回条是第二条栏**,要合成一条得动聊天页顶栏(碰「照官端」),**她没点头别动**。⚠️ **仍未真机确认的只剩字重**:容器里 `ui-serif` 落到 DejaVu Serif、**只有两档字重**(实测 400=500、600=700=800),所以那套 400/600/700 在预览里必然塌成两档 —— **别照预览调字重**,详见 dwell 手册那节 | `dwell-bridge/MAINTENANCE.md`《Home 页:怎么按参考图量,量到什么》《换布局》 |
