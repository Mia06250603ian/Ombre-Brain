...
> (**33 条记录、覆盖第 2~34 次**;⚠️ **「第一次」(2026-07-12 初次搭建)没有独立条目**,
...
| **心跳「自主活动」+ 上下文阈值** | **代码/人设已改,未部署**(2026-08-21)。`kelivo-shim/CLAUDE.md` 心跳整节 + `keepalive.mjs` 开口纸条 + `test-keepalive.mjs`(52→59)已改好、四套单测全绿;**要整套 shim 部署,会清掉晏的窗口**,等所有者本人先对晏说「归档」。同批待拍板的阈值:三线各 +500(155000/161500/164000)、`CTX_FINAL_CHARS` 1200→**1400**(⚠️ 别超 1400,天花板在 OB 那个 `[:1500]`) | `kelivo-shim/MAINTENANCE.md`「2026-08-21 心跳自主活动 + 压缩点实测」一节 |
...
4. **每个数字都要带「哪天量的」和「怎么量的」。**
   ...
   **33 条记录、覆盖第 2~34 次**——照旧数搜「第四十次」会搜空、以为记录丢了。
